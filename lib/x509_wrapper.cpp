...
void X509Wrapper::load_certificate(const std::string& data, const EncodingFormat encoding) {
    ...
    if (encoding == EncodingFormat::PEM) {
        x509 = X509_ptr(PEM_read_bio_X509(bio, nullptr, nullptr, nullptr), ::X509_free);
    } else if (encoding == EncodingFormat::DER) {
        x509 = X509_ptr(d2i_X509_bio(bio, nullptr), ::X509_free);
    } else {
        BIO_free(bio);
    ...
    ASN1_TIME* notBefore = X509_get_notBefore(x509.get());
    ASN1_TIME* notAfter = X509_get_notAfter(x509.get());
    int day, sec;
...
X509Wrapper::X509Wrapper(const std::string& certificate, const EncodingFormat encoding): x509(nullptr, ::X509_free) {
    load_certificate(certificate, encoding);
}

X509Wrapper::X509Wrapper(const std::filesystem::path& path, const EncodingFormat encoding): x509(nullptr, ::X509_free) {
...
X509Wrapper::X509Wrapper(X509* x509) : x509(x509, ::X509_free) {
    // Handle validity
...
X509Wrapper::X509Wrapper(const X509Wrapper& other) : x509(nullptr, ::X509_free) {
...
std::vector<X509Wrapper> X509Wrapper::split() {
    // isn't there any helper function for this already?  I can imagine that it is hard to make this function
    // bullet proof
    ...
    while (std::regex_search(search_start, this->str.cend(), match, cert_regex)) {
        ...
        try {            
            certificates.emplace_back(cert_data, EncodingFormat::PEM);
...
std::string X509Wrapper::get_common_name() const {
    X509_NAME* subject = X509_get_subject_name(this->x509.get());
...
std::string X509Wrapper::get_issuer_name_hash() const {
    ...
    X509_NAME* name = X509_get_issuer_name(this->x509.get());
...
std::string X509Wrapper::get_serial_number() const {
    ASN1_INTEGER* serial_asn1 = X509_get_serialNumber(this->x509.get());
...
std::string X509Wrapper::get_issuer_key_hash() const {
    ...
    X509_pubkey_digest(this->x509.get(), EVP_sha256(), tmphash, NULL);
    std::stringstream ss;
    for (int i = 0; i < SHA256_DIGEST_LENGTH; i++) {
...
std::string X509Wrapper::get_responder_url() const {
    const auto ocsp = X509_get1_ocsp(this->x509.get());
