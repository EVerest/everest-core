...
from valuegenerator import AcrossFileStructGenerator
...
def write_tests(generator):
    ...
    os.system("clang-format -i " + generator.get_file_path("hpp") + " " + generator.get_file_path("cpp"))
...
for root, dirs, files in os.walk(code_path):
    if name in files:
        ...
        generator = FileTestGenerator(file_to_test, write_path, across_file_struct_generator, code_path, license_header, get_single_deny_list(deny_lists, file_to_test))
        generator.generate_code()
        generators.append(generator)
if not api_files_found:
    raise FileNotFoundError("API files were not found in include directory\nExpected for example: {include}/api_types/auth/API.hpp\nInclude Path that was given is: " + code_path)
