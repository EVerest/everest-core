...
TariffMessage_External to_external_api(TariffMessage_Internal const& val) {
    ...
    std::vector<text_message::MessageContent_External> vec;
    for (auto const& item : val.messages) {
        vec.push_back(text_message::to_external_api(item));
    }
    result.messages = vec;
    result.ocpp_transaction_id = val.ocpp_transaction_id;
    result.identifier_id = val.identifier_id;
    if (val.identifier_type) {
        result.identifier_type = display_message::to_external_api(val.identifier_type.value());
    }
    return result;
}
...
IdlePrice_External to_external_api(IdlePrice_Internal const& val) {
    ...
    result.grace_minutes = val.grace_minutes;
    if (val.hour_price) {
        result.hour_price = money::to_external_api(val.hour_price.value());
    }
    return result;
}
...
ChargingPriceComponent_External to_external_api(ChargingPriceComponent_Internal const& val) {
    ...
    if (val.price) {
        result.price = money::to_external_api(val.price.value());
    }
    return result;
}
...
SessionCost_Internal to_internal_api(SessionCost_External const& val) {
    ...
    if (val.cost_chunks) {
        std::vector<SessionCostChunk_Internal> vec;
        for (auto const& item : val.cost_chunks.value()) {
            vec.push_back(to_internal_api(item));
        }
        result.cost_chunks.emplace(vec);
    }
    result.idle_price = optToInternal(val.idle_price);
    if (val.charging_price) {
        std::vector<ChargingPriceComponent_Internal> vec;
        for (auto const& item : val.charging_price.value()) {
            vec.push_back(to_internal_api(item));
        }
        result.charging_price.emplace(vec);
    }
    result.next_period = to_internal_api(val.next_period.value());
    if (val.message) {
        std::vector<text_message::MessageContent_Internal> vec;
        for (auto const& item : val.message.value()) {
            vec.push_back(text_message::to_internal_api(item));
        }
        result.message.emplace(vec);
    }
...
SessionCost_External to_external_api(SessionCost_Internal const& val) {
    ...
    result.id_tag = auth::to_external_api(val.id_tag.value());
    if (val.cost_chunks) {
        std::vector<SessionCostChunk_External> vec;
        for (auto const& item : val.cost_chunks.value()) {
            vec.push_back(to_external_api(item));
        }
        result.cost_chunks.emplace(vec);
    }
    result.idle_price = to_external_api(val.idle_price.value());
    result.charging_price = vecToExternal(val.charging_price.value());
    result.next_period = to_external_api(val.next_period.value());
    if (val.message) {
        std::vector<text_message::MessageContent_External> vec;
        for (auto const& item : val.message.value()) {
            vec.push_back(text_message::to_external_api(item));
