...
ninja \
    -C "$EXT_MOUNT/build" \
    everest-framework_gcovr_coverage
retValHTML=$?

ninja \
    -C "$EXT_MOUNT/build" \
    everest-framework_gcovr_coverage_xml
retValXML=$?

# Copy the generated coverage report and xml to the mounted directory in any case
cp -R "$EXT_MOUNT/build/everest-framework_gcovr_coverage" "$EXT_MOUNT/gcovr-coverage"
cp "$EXT_MOUNT/build/everest-framework_gcovr_coverage_xml.xml" "$EXT_MOUNT/gcovr-coverage-xml.xml"
