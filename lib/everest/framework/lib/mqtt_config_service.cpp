...
void ConfigServiceClient::register_config_change_handler(const std::string_view name, ConfigChangeHandler handler) {
    ...
    // store handler
    change_callbacks.emplace(name, std::move(handler));
}
...
void ConfigServiceClient::mqtt_set_request(const nlohmann::json& data) {
    ...
    try {
        ...
        if (const auto it = change_callbacks.find(name); it != change_callbacks.end()) {
