...
bool overlap(const DateTime& start_time, const DateTime& end_time, const ChargingProfile profile) {
    DateTime latest_start(start_time);
    DateTime earliest_end(end_time);
...
DateTime get_period_end_time(const int period_index, const DateTime& period_start_time,
                             const ChargingSchedule& schedule, const std::vector<ChargingSchedulePeriod>& periods) {

    boost::optional<DateTime> period_end_time;
    ...
    if (period_index + 1 < periods.size()) {
        ...
        const auto dt = DateTime(period_start_time.to_time_point() + seconds(period_diff_in_seconds));
        return dt;
    } else if (schedule.duration) {
        period_diff_in_seconds = schedule.duration.value() - periods.at(period_index).startPeriod;
        return DateTime(period_start_time.to_time_point() + seconds(period_diff_in_seconds));
    } else {
        return DateTime(date::utc_clock::now() + hours(std::numeric_limits<int>::max()));
    }
}
...
PeriodDateTimePair SmartChargingHandler::find_period_at(const DateTime& time, const ChargingProfile& profile,
    ...
    if (period_start_time) {
        ...
        for (int i = 0; i < periods.size(); i++) {
            ...
            if (time >= period_start_time.value() && time < period_end_time) {
                return {periods[i], DateTime(period_end_time)};
            }
            period_start_time.emplace(DateTime(period_end_time));
        }
    }

    return {boost::none, DateTime(date::utc_clock::now() + hours(std::numeric_limits<int>::max()))};
}
...
ChargingSchedule SmartChargingHandler::calculate_composite_schedule(
    std::vector<ChargingProfile> valid_profiles, const DateTime& start_time, const DateTime& end_time,
    ...
    DateTime last_period_end_time(end_time);
...
SmartChargingHandler::get_valid_profiles(const DateTime& start_time, const DateTime& end_time, const int connector_id) {
...
boost::optional<DateTime> SmartChargingHandler::get_profile_start_time(const ChargingProfile& profile,
                                                                       const DateTime& time, const int connector_id) {

    const auto schedule = profile.chargingSchedule;
    boost::optional<DateTime> period_start_time;
    if (profile.chargingProfileKind == ChargingProfileKindType::Absolute) {
        if (schedule.startSchedule) {
            period_start_time.emplace(DateTime(floor<seconds>(schedule.startSchedule.value().to_time_point())));
        } else {
        ...
        if (this->connectors.at(connector_id)->transaction != nullptr) {
            period_start_time.emplace(DateTime(floor<seconds>(
        ...
        const auto start_schedule = DateTime(floor<seconds>(schedule.startSchedule.value().to_time_point()));
        ...
        period_start_time.emplace(DateTime(time.to_time_point() - seconds(static_cast<int>(seconds_to_go_back))));
    }
    return period_start_time;
}

DateTime SmartChargingHandler::get_next_temp_time(const DateTime temp_time,
                                                  const std::vector<ChargingProfile>& valid_profiles,
                                                  const int connector_id) {
    auto lowest_next_time = DateTime(date::utc_clock::now() + hours(std::numeric_limits<int>::max()));
    for (const auto& profile : valid_profiles) {
        const auto schedule = profile.chargingSchedule;
        const auto periods = schedule.chargingSchedulePeriod;
