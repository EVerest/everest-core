...
.. tip::
    ...
    You can find the API reference pages in the left sidebar. Click on the
    link "EVerest API" to get there. TODO: Link to sidebar.
...
creates MQTT topics according to its specification
(TODO: Link to specification) and by this provides
...
*everest/api/1/power_supply_DC/ps_dc_1/*.
...
https://www.asyncapi.com\ {:target=_blank}.

All EVerest API modules are located in *everest-core/modules/API* and each
module contains the API definition in its *docs/asyncapi.yaml* file.
These files can be used to generate the HTML-based documentation via the
*everest_api_docs* compilation target. **TODO: This target is missing!**.

In order to build the documentation, the
`asyncapi/cli <https://www.asyncapi.com/en/tools/cli>`__
(https://www.asyncapi.com/en/tools/cli) tool has to be available on the
development machine.

Another possible way is to use the file in
`AsyncAPI Studio <https://https://studio.asyncapi.com/>`_.

The *asyncapi.yaml* is defined for the client implementing the API and
...
are prefixed with *everest/api/1/{api_type}/{module_id}* - with *1*
...
.. code-block:: text
   ...
   everest/api/1/power_supply_DC/ps_dc_1/m2e/voltage_current
...
In order to use the EVerest API, load the needed API module in the
...
EVerest comes with a set of examples demonstrating the usage of the
APIs. They are located in the folder *everest/examples/API* and
available for different programming languages. The intention is to
demonstrate the minimal effort required to use the API.

List of EVerest APIs
====================

See here for a complete list of EVerest APIs: **TODO**
