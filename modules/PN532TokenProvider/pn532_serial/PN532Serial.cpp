            ...
            if (data_received < packet_length) {
                ...
                if (data.back() == 0x00 && data.size() >= 2) {
                    ...
                    if (this->debug) {
                        ...
                        EVLOG_info << std::setfill('0') << std::setw(2) << std::right << std::hex
                                   << "Packet data checksum: 0x" << (int)packet_data_checksum << " checksum result: 0x"
                                   << (int)sum;
                        for (auto& element : data) {
                            data_stream << "0x" << std::setfill('0') << std::setw(2) << std::right << std::hex
                                        << (int)element << " ";
                        }
                    ...
                    if (sum == 0x00) {
                        if (this->debug) {
                            EVLOG_info << "Packet data checksum valid, command code: 0x" << std::setfill('0')
                                       << std::setw(2) << std::right << std::hex << (int)command_code;
                        }

                        if (command_code == COMMAND_GET_FIRMWARE_VERSION_RESPONSE &&
                            this->get_firmware_version_promise) {
                            ...
                            if (data.size() >= 6) {
                                ...
                                if (in_list_passive_target_response->nbtg == 1) {
                                    ...
                                    if (this->debug) {
                                        EVLOG_info << "Target data: " << std::setfill('0') << std::setw(2) << std::right
                                                   << std::hex                                               //
                                                   << "\ntg: 0x" << (int)target_data.tg                      //
                                                   << "\nsens_res_msb: 0x" << (int)target_data.sens_res_msb  //
                                                   << "\nsens_res_lsb: 0x" << (int)target_data.sens_res_lsb  //
                                                   << "\nsens_res: 0x" << (int)target_data.sens_res          //
                                                   << "\nsel_res: 0x" << (int)target_data.sel_res            //
                                                   << "\nnfcid_length: 0x" << (int)target_data.nfcid_length; //
                                    }
                                    ...
                                    if (data.size() >= 6 + target_data.nfcid_length) {
                                        ...
                                        if (this->debug) {
                                            ...
                                            for (auto& element : target_data.nfcid) {
                                                nfcid_stream << "0x" << std::setfill('0') << std::setw(2) << std::right
                                                             << std::hex << (int)element << " ";
                                            }
                                        ...
                                        if (data.size() >= 6 + target_data.nfcid_length + 1) {
                                            ...
                                            if (data.size() >= 6 + target_data.nfcid_length + ats_length) {
                                                for (size_t i = 6 + target_data.nfcid_length;
                                                     i < 6 + target_data.nfcid_length + ats_length; i++) {
                                            ...
                                            if (this->debug) {
                                                ...
                                                for (auto& element : target_data.ats) {
                                                    ats_stream << "0x" << std::setfill('0') << std::setw(2)
                                                               << std::right << std::hex << (int)element << " ";
                                                }
