...
void isolation_monitor_API::ready() {
    invoke_ready(*p_if_isolation_monitor);
...
void isolation_monitor_API::generate_api_var_self_test_result() {
    subscribe_api_var("self_test_result", [=](std::string const& data) {
        auto value = generic::deserialize<bool>(data);
        p_if_isolation_monitor->publish_self_test_result(value);
        return true;
    });
}
...
void isolation_monitor_API::generate_api_var_raise_error() {
    subscribe_api_var("raise_error", [=](std::string const& data) {
        ...
        if (deserialize(data, error)) {
            ...
            auto ev_error = p_if_isolation_monitor->error_factory->create_error(error_str, sub_type_str, message_str,
                                                                                Everest::error::Severity::High);
            p_if_isolation_monitor->raise_error(ev_error);
            return true;
        }
        return false;
    });
...
void isolation_monitor_API::generate_api_var_clear_error() {
    subscribe_api_var("clear_error", [=](std::string const& data) {
        ...
        if (deserialize(data, error)) {
            ...
            if (error.sub_type) {
                p_if_isolation_monitor->clear_error(error_str, error.sub_type.value());
            } else {
                p_if_isolation_monitor->clear_error(error_str);
            }
            return true;
        }
        return false;
...
std::string isolation_monitor_API::make_error_string(ns_types_ext::Error const& error) {
