...
public:
    ...
    auth_token_provider_API(const ModuleInfo& info, Everest::MqttProvider& mqtt_provider,
                            std::unique_ptr<auth_token_providerImplBase> p_auth_token_provider, Conf& config) :
        ModuleBase(info),
        mqtt(mqtt_provider),
        ...
        comm_check("generic/CommunicationFault", "Bridge to implementation connection lost",
                   this->p_auth_token_provider){};

    Everest::MqttProvider& mqtt;
    const std::shared_ptr<auth_token_providerImplBase> p_auth_token_provider;
    const Conf& config;

    // ev@1fce4c5e-0ab8-41bb-90f7-14277703d2ac:v1
    // insert your public definitions here
    const ns_ev_api::Topics& get_topics() const;
    ns_ev_api::CommCheckHandler<auth_token_providerImplBase> comm_check;
...
private:
    ...
    using ParseAndPublishFtor = std::function<void(const json&)>;
