...
static int callback_ws(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len) {
    WebSocketServer *server = reinterpret_cast<WebSocketServer *>(lws_context_user(lws_get_context(wsi)));
...
std::string WebSocketServer::ssl_configuration() const {
}
...
bool WebSocketServer::start_server() {
    ...
    std::cout << "WebSocket Server running on port " << m_info.port;
    if (m_ssl_enabled) std::cout << " with TLS";
    std::cout << std::endl;
