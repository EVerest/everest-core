...
void EvseManager::ready() {
    ...
    error_handling =
        std::make_unique<ErrorHandling>(r_bsp, r_hlc, r_connector_lock, r_ac_rcd, p_evse, r_imd, r_powersupply_DC,
...
void EvseManager::ready_to_start_charging() {
    ...
    auto supported_operation_modes =
        utils::convert_operation_mode_csv_to_vector(this->config.supported_operation_modes);
    if (supported_operation_modes.empty()) {
        supported_operation_modes.reserve(2);
        supported_operation_modes.push_back(types::evse_manager::OperationModeEnum::Idle);
        supported_operation_modes.push_back(types::evse_manager::OperationModeEnum::ChargingOnly);
    }
    this->p_evse->publish_supported_operation_modes(supported_operation_modes);
    this->p_evse->publish_supported_energy_transfer_modes(supported_energy_transfers);
