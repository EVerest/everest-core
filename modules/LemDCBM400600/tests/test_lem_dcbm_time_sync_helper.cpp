...
protected:
    ...
    const std::string expected_tz_sync_request_regex{
        R"(\{"time": \{"tz":""\}\})"};

    const std::string expected_dst_sync_request_regex{
        R"(\{"time": \{"dst":\}\})"};
...
TEST_F(LemDCBMTimeSyncHelperTest, test_sync_success_system_time) {
    ...
    EXPECT_CALL(*this->http_client,
                put("/v1/settings", testing::ContainsRegex(this->expected_dst_sync_request_regex)))
        .Times(1)
        .WillOnce(testing::Return(HttpResponse{200, this->put_settings_response_success}));
    EXPECT_CALL(*this->http_client,
                put("/v1/settings", testing::ContainsRegex(this->expected_tz_sync_request_regex)))
        .Times(1)
        .WillOnce(testing::Return(HttpResponse{200, this->put_settings_response_success}));
    EXPECT_CALL(*this->http_client,
