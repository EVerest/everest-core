...
bool CarSimulation::iso_start_v2g_session(const CmdArguments& arguments, bool three_phases) {
    ...
    if (energy_mode == constants::AC) {
        if (three_phases == false) {
            r_ev[0]->call_start_charging(types::iso15118_charger::EnergyTransferMode::AC_single_phase_core);
        } else {
            r_ev[0]->call_start_charging(types::iso15118_charger::EnergyTransferMode::AC_three_phase_core);
        }
    } else if (energy_mode == constants::DC) {
        r_ev[0]->call_start_charging(types::iso15118_charger::EnergyTransferMode::DC_extended);
    } else {
        return false;
    }
    return true;
