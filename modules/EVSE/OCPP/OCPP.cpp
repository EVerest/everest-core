...
#ifdef DATABASE_CONFIG
    // use OCPP 2.x device model storage
    std::unique_ptr<ocpp::v2::DeviceModelStorageInterface> device_model;
    auto configuration = std::make_unique<ocpp::v16::ChargePointConfigurationBase>(std::move(device_model));
#else
    // use OCPP 1.6 JSON file storage
    auto configuration = std::make_unique<ocpp::v16::ChargePointConfiguration>(json_config.dump(),
                                                                               this->ocpp_share_path, user_config_path);
#endif
    this->charge_point = std::make_unique<ocpp::v16::ChargePoint>(
        std::move(configuration), std::filesystem::path(this->config.DatabasePath), sql_init_path,
