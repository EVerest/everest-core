    ...
    if (hlc_enabled) {
        ...
        if (config.charge_mode == "AC") {
            ...
            r_hlc[0]->subscribe_dc_ev_target_voltage_current([this](types::iso15118::DcEvTargetValues v) {
                ...
                if (almost_eq(v.dc_ev_target_voltage, latest_target_voltage) and
                    almost_eq(v.dc_ev_target_current, latest_target_current)) {
                    // no change
                    return;
                }
...
void EvseManager::apply_new_target_voltage_current() {
    double target_voltage = latest_target_voltage;
    double target_current = latest_target_current;

    bool car_breaks_limit = false;

    // Apply Limit voltage/current for broken EV implementations
    const auto ev = get_ev_info();
    if (ev.maximum_current_limit.has_value() and target_current > ev.maximum_current_limit.value()) {
        target_current = ev.maximum_current_limit.value();
    }

    if (ev.maximum_voltage_limit.has_value() and target_voltage > ev.maximum_voltage_limit.value()) {
        target_voltage = ev.maximum_voltage_limit.value();
    }

    // Apply HLC Limits
    const auto hlc_limits = charger->get_evse_max_hlc_limits();
    if (target_current > hlc_limits.evse_maximum_current_limit) {
        target_current = hlc_limits.evse_maximum_current_limit;
        car_breaks_limit = true;
    }

    // Apply HLC power limit
    const auto actual_voltage = ev_info.present_voltage.has_value() ? ev_info.present_voltage.value() : target_voltage;

    const auto target_power = target_current * actual_voltage;
    if (target_power > hlc_limits.evse_maximum_power_limit) {
        target_current = hlc_limits.evse_maximum_power_limit / actual_voltage;
        car_breaks_limit = true;
    }

    if (car_breaks_limit) {
        EVLOG_warning << "EV ignores new EVSE max limits. Target current was set to new EVSE max limits";
    }

    if (target_voltage > 0) {
        powersupply_DC_set(target_voltage, target_current);
    }
}
