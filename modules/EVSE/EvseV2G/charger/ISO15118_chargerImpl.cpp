    ...
    if (not v2g_ctx->hlc_pause_active) {
        if (payment_options.size() > 0) {
            ...
            for (size_t i = 0; i < max_payment_options; i++) {
                v2g_ctx->evse_v2g_data.payment_option_list.push_back(
                    static_cast<iso2_paymentOptionType>(payment_options.at(i)));
            }

        } else {
            dlog(DLOG_LEVEL_ERROR, "No valid PaymentOptions configured, falling back to ExternalPayment");
            v2g_ctx->evse_v2g_data.payment_option_list = {iso2_paymentOptionType_ExternalPayment};
        }
    }
