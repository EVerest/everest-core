...
ISO15118_vasImpl::~ISO15118_vasImpl() {
    ...
    if (internet_service_thread.joinable()) {
        internet_service_thread.join();
    }
}
...
void ISO15118_vasImpl::start_script(const std::string& script_name, const std::vector<std::string>& args) {
    ...
    everest::run_application::run_application(script_path, args, [](const std::string& message) {
        EVLOG_info << message;
