...
const PowerStackMockConfig default_power_stack_mock_config_without_tls =
    PowerStackMockConfig{
        .eth = "veth1",
        .port = 8502,
        .hmac_key = {0x67e4, 0x2656, 0x0a70, 0xca4a, 0x833c, 0x44b3,
                     0x1270, 0xca93, 0x55d8, 0x7b02, 0x0f57, 0x8e1e,
                     0x9d19, 0x74c0, 0x2fa6, 0xf680, 0x4c2f, 0xcbdf,
                     0x735e, 0x711c, 0xec08, 0x5b93, 0x8147, 0x16ad},
        .tls_config = nullopt,
    };
...
DispenserTestBase::DispenserTestBase(DispenserTestParams params)
    : dispenser_config(params.dispenser_config),
      ...
      dispenser_connector_upstream_voltage(
          params.dispenser_connector_upstream_voltage),
...
void DispenserTestBase::SetUp() {
  ...
  dispenser->start();
  power_stack_mock = std::shared_ptr<PowerStackMock>(
      PowerStackMock::from_config(power_stack_mock_config));
...
DispenserWithTlsTest::get_last_power_requirement_request() {
  return power_stack_mock->get_last_power_requirement_request(
      global_connector_number);
}

uint32_t DispenserWithTlsTest::get_stop_request_counter() {
  return power_stack_mock->get_stop_charge_request_counter(
      global_connector_number);
}
...
float DispenserWithTlsTest::get_maximum_rated_charge_current() {
  return power_stack_mock->get_maximum_rated_charge_current(
...
std::optional<fusion_charger::goose::PowerRequirementRequest>
dispenser_fixture::DispenserWithoutTlsTest::
    get_last_power_requirement_request() {
  return power_stack_mock->get_last_power_requirement_request(
      global_connector_number);
}
...
std::shared_ptr<Connector> DispenserWithMultipleConnectors::get_connector(
...
void DispenserWithMultipleConnectors::connect_car(
    uint16_t local_connector_number) {
  get_connector(local_connector_number)->on_car_connected();
  sleep_for_ms(10);
}
void DispenserWithMultipleConnectors::send_hmac_key(
    uint16_t local_connector_number) {
  power_stack_mock->send_hmac_key(local_connector_number);
  sleep_for_ms(10);
}

void DispenserWithMultipleConnectors::set_export_values(
    uint16_t local_connector_number, float voltage, float current) {
  get_connector(local_connector_number)
      ->new_export_voltage_current(voltage, current);
  sleep_for_ms(10);
}

void DispenserWithMultipleConnectors::set_mode_phase(
    uint16_t local_connector_number, ModePhase mode_phase) {
  get_connector(local_connector_number)->on_mode_phase_change(mode_phase);
  sleep_for_ms(10);
}

std::array<uint32_t, 4>
DispenserWithMultipleConnectors::get_stop_request_counter() {
  auto result = std::array<uint32_t, 4>();
  for (int i = 0; i < 4; i++) {
    auto counter = power_stack_mock->get_stop_charge_request_counter(
        connector_configs[i].global_connector_number);
...
void DispenserWithMultipleConnectors::disconnect_car(
    uint16_t local_connector_number) {
  get_connector(local_connector_number)->on_car_disconnected();
  sleep_for_ms(10);
}

void DispenserWithMultipleConnectors::assert_working_status(
    std::array<WorkingStatus, 4> expected_status) {
  for (int i = 0; i < 4; i++) {
    auto actual_status = get_connector(i + 1)->get_working_status();
    EXPECT_EQ(actual_status, expected_status[i])
        << "Regarding connector " << i + 1;
  }
}

void DispenserWithMultipleConnectors::assert_requirement_type(
    std::array<std::optional<fusion_charger::goose::RequirementType>, 4>
        expected_types) {
  for (int i = 0; i < 4; i++) {
    auto actual_status = power_stack_mock->get_last_power_requirement_request(
        connector_configs[i].global_connector_number);

    if (actual_status == nullopt && expected_types[i] == nullopt) {
      continue;
    } else if (actual_status == nullopt) {
      FAIL() << "Actual Status is NULL , but expected was: "
             << (uint16_t)expected_types[i].value() << " regarding connector "
             << i + 1;
...
void DispenserWithMultipleConnectors::
    assert_stop_request_counter_greater_or_equal(
        std::array<uint32_t, 4> expected) {
  for (int i = 0; i < 4; i++) {
    auto actual = power_stack_mock->get_stop_charge_request_counter(
        connector_configs[i].global_connector_number);
