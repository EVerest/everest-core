...
class DispenserTestBase : public ::testing::Test {
 ...
 protected:
  ...
  virtual void sleep_for_ms(uint32_t ms);
};
...
class DispenserWithTlsTest : public DispenserTestBase {
 public:
  ...
  const uint16_t global_connector_number =
      connector_configs[0].global_connector_number;
  const uint16_t local_connector_number = 1;

  std::shared_ptr<Connector> connector();
  std::optional<fusion_charger::goose::PowerRequirementRequest>
  get_last_power_requirement_request();
  uint32_t get_stop_request_counter();
  uint32_t get_power_requirements_counter();
...
class DispenserWithMultipleConnectors : public DispenserTestBase {
 public:
  uint16_t local_connector_number1 = 1;
  uint16_t local_connector_number2 = 2;
  uint16_t local_connector_number3 = 3;
  uint16_t local_connector_number4 = 4;

  DispenserWithMultipleConnectors();

  std::shared_ptr<Connector> get_connector(uint16_t local_connector_number);
  void set_up_psu_for_operation();
  void connect_car(uint16_t local_connector_number);
  void send_hmac_key(uint16_t local_connector_number);
  void set_export_values(uint16_t local_connector_number, float voltage,
                         float current);
  void set_mode_phase(uint16_t local_connector_number, ModePhase mode_phase);
  std::array<uint32_t, 4> get_stop_request_counter();
  void disconnect_car(uint16_t local_connector_number);

  void assert_working_status(std::array<WorkingStatus, 4> expected_status);
  void assert_requirement_type(
