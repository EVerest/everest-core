...
public:
    ...
    const uint16_t global_connector_number = connector_configs[0].global_connector_number;
...
public:
    ...
    void set_export_values(uint16_t local_connector_number, float voltage, float current);
    ...
    void assert_stop_request_counter_greater_or_equal(std::array<uint32_t, 4> expected);
};
