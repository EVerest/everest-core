...
TEST_F(DispenserWithTlsTest, StateCarDisconnected) {
  ...
  EXPECT_EQ(connector()->get_output_port_availability(),
            PsuOutputPortAvailability::NOT_AVAILABLE);
  EXPECT_EQ(dispenser->get_psu_communication_state(),
            DispenserPsuCommunicationState::INITIALIZING);
...
TEST_F(DispenserWithTlsTest, CarConnectedAndReadyToCharge) {
  ...
  EXPECT_EQ(connector()->get_working_status(),
  ...
  sleep_for_ms(5);
  EXPECT_EQ(connector()->get_working_status(),
            WorkingStatus::CHARGING_STARTING);
...
TEST_F(DispenserWithTlsTest, ChargingACarUpToRegularDisconnect) {
  ...
  EXPECT_EQ(
      get_last_power_requirement_request()->requirement_type,
      fusion_charger::goose::RequirementType::InsulationDetectionVoltageOutput);
  EXPECT_EQ(get_last_power_requirement_request()->mode,
            fusion_charger::goose::Mode::ConstantCurrent);
  ...
  EXPECT_EQ(stop_request_counter_before_charge_complete,
            stop_request_counter_before_charging);
  ...
  EXPECT_EQ(connector()->get_working_status(),
            WorkingStatus::CHARGING_COMPLETE);
  EXPECT_GT(get_stop_request_counter(),
            stop_request_counter_before_charge_complete);
...
TEST_F(DispenserWithTlsTest, ReadDispenserInformationAndConfiguration) {
  auto expected_dispenser_information =
      DispenserInformation{.manufacturer = 0x0002,
                           .model = 0x0080,
                           .protocol_version = 1,
                           .hardware_version = 3,
                           .software_version = "v1.2.3+456"};

  EXPECT_EQ(power_stack_mock->get_dispenser_information(),
            expected_dispenser_information);
...
TEST_F(DispenserWithTlsTest,
       PsuCommunicationStateIsFailedAfterModbusHeartbeatTimeout) {
  sleep_for_ms(2'100);
...
TEST_F(
    ...
    CarConnectWithoutModePhaseChangeContinuesSendingModulePlaceholderRequests) {
  power_stack_mock->set_psu_running_mode(PSURunningMode::RUNNING);
  power_stack_mock->send_mac_address();
  ...
  auto post_connect_power_requirement_counter =
...
TEST_F(DispenserWithTlsTest, TimeSyncEverySecond) {
  ...
  EXPECT_NEAR(time1, (uint32_t)std::time(NULL), 1);
...
TEST_F(DispenserWithTlsTest, ErrorReportingChargingPowerUnit) {
  power_stack_mock->write_registers(0x4008, {(uint16_t)AlarmStatus::ALARM});
...
TEST_F(DispenserWithTlsTest, String) {
  ...
  e.error_subcategory = {.cooling_section =
                             ErrorSubcategoryCoolingSection::CoolingUnit1};
...
TEST_F(DispenserWithTlsTest,
       module_placeholder_allocation_timeout_then_normal) {
  power_stack_mock->set_psu_running_mode(PSURunningMode::RUNNING);
  power_stack_mock->send_mac_address();
...
TEST_F(DispenserWithTlsTest, acquire_hmac_post_start) {
  ...
  auto thread = std::thread([this]() {
    while (connector()->get_working_status() !=
           WorkingStatus::STANDBY_WITH_CONNECTOR_INSERTED) {
...
TEST_F(DispenserWithTlsTest, acquire_hmac_post_start_timeout) {
  ...
  auto ms_needed =
      std::chrono::duration_cast<std::chrono::milliseconds>(time_needed)
          .count();
