...
TEST_F(DispenserWithMultipleConnectors, ChargingSession) {
  ...
  assert_working_status({WorkingStatus::STANDBY, WorkingStatus::STANDBY,
                         WorkingStatus::STANDBY, WorkingStatus::STANDBY});
  ...
  assert_requirement_type(
      {nullopt, RequirementType::ModulePlaceholderRequest, nullopt, nullopt});

  set_mode_phase(2, ModePhase::ExportCharging);
  assert_working_status(
      {WorkingStatus::STANDBY_WITH_CONNECTOR_INSERTED, WorkingStatus::CHARGING,
       WorkingStatus::STANDBY_WITH_CONNECTOR_INSERTED, WorkingStatus::STANDBY});
  ...
  assert_requirement_type(
      {nullopt, RequirementType::Charging, RequirementType::Charging, nullopt});
  assert_working_status({WorkingStatus::STANDBY_WITH_CONNECTOR_INSERTED,
                         WorkingStatus::CHARGING, WorkingStatus::CHARGING,
  ...
  disconnect_car(2);
  assert_working_status({WorkingStatus::STANDBY_WITH_CONNECTOR_INSERTED,
                         WorkingStatus::STANDBY, WorkingStatus::CHARGING,
                         WorkingStatus::STANDBY});
}
