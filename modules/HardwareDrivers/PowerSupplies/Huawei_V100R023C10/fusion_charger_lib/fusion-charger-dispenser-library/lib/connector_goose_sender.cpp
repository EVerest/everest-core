...
ConnectorGooseSender::ConnectorGooseSender(
    std::shared_ptr<goose_ethernet::EthernetInterfaceIntf> intf, bool secure,
    logs::LogIntf log)
    : secure(secure),
      logs(log),
      goose_sender(
...
void ConnectorGooseSender::start() { goose_sender.start(); }

void ConnectorGooseSender::stop() { goose_sender.stop(); }

void ConnectorGooseSender::on_new_hmac_key(std::vector<uint8_t> hmac_key) {
  this->hmac_key = hmac_key;
}

void ConnectorGooseSender::on_new_mac_address(
...
void ConnectorGooseSender::send_goose_frame(goose::frame::GoosePDU pdu,
                                            uint16_t appid) {
  if (secure && !hmac_key) {
  ...
  if (secure) {
    ...
    frame.vlan_id = 0;
    frame.priority = 5;
    packet = std::make_unique<goose::sender::SendPacketSecure>(
        frame, hmac_key.value());
  } else {
  ...
  // if (!secure) {
  //   frame = std::make_unique<goose::frame::GooseFrame>();
  // } else {
  //   frame = std::make_unique<goose::frame::SecureGooseFrame>();
  // }
  ...
  // if (dynamic_cast<goose::frame::GooseFrame*>(frame.get())) {
  //   goose_sender.send(new goose::sender::SendPacketNormal(
  //       *dynamic_cast<goose::frame::GooseFrame*>(frame.get())));
  // } else {
  //   goose_sender.send(new goose::sender::SendPacketSecure(
  //       *dynamic_cast<goose::frame::SecureGooseFrame*>(frame.get()),
  //       *hmac_key));
  // }
}
