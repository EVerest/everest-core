...
std::tuple<SSL*, SSL_CTX*> tls_util::init_mutual_tls_client(
    int socket, MutualTlsClientConfig config) {
  ...
  if (SSL_CTX_use_certificate_file(ctx, config.client_cert.c_str(),
                                   SSL_FILETYPE_PEM) != 1) {
    throw std::runtime_error("Could not load client certificate");
  }

  printf("Client key: %s\n", config.client_key.c_str());
  if (SSL_CTX_use_PrivateKey_file(ctx, config.client_key.c_str(),
                                  SSL_FILETYPE_PEM) != 1) {
...
std::tuple<SSL*, SSL_CTX*> tls_util::init_mutual_tls_server(
    int socket, MutualTlsServerConfig config) {
  SSL_load_error_strings();
  SSL_library_init();
  OpenSSL_add_all_algorithms();
  ...
  if (SSL_CTX_use_certificate_file(ctx, config.server_cert.c_str(),
                                   SSL_FILETYPE_PEM) != 1) {
    throw std::runtime_error("Could not load server certificate");
  }

  if (SSL_CTX_use_PrivateKey_file(ctx, config.server_key.c_str(),
                                  SSL_FILETYPE_PEM) != 1) {
