...
void ConnectorFSM::transition(std::optional<States> new_state,
  ...
  if (new_state.has_value() && current_state != new_state.value()) {
    log.info << "New state: " + state_to_string(current_state) + " -> " +
...
void Connector::set_working_status(WorkingStatus status) {
  log.info << "Set working status for connector " +
                  std::to_string(local_connector_number) +
