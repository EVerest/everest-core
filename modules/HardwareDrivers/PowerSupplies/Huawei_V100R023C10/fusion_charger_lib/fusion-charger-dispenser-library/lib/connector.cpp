...
bool mode_phase_is_export_mode(ModePhase mode_phase) {
  return mode_phase == ModePhase::ExportCableCheck ||
         mode_phase == ModePhase::ExportPrecharge ||
...
void ConnectorFSM::transition(std::optional<States> new_state,
                              std::optional<ModePhase> new_mode_phase) {
...
void ConnectorFSM::on_mode_phase_change(ModePhase mode_phase) {
  ...
  } else if (current_state == States::Completed &&
             mode_phase_is_export_mode(mode_phase)) {
...
Connector::Connector(ConnectorConfig connector_config,
    ...
    : connector_config(connector_config),
      ...
      eth_interface(std::make_shared<goose_ethernet::EthernetInterface>(
...
void Connector::on_state_transition(States state) {
  ...
  if (state == States::ConnectedNoAllocation) {
    ...
    log.verbose << log_prefix +
                       "Starting module placeholder allocation timeout thread";
    module_placeholder_allocation_timeout.thread = std::thread([this]() {
      std::cv_status wait_resp;
      {
        std::unique_lock<std::mutex> lock(
            module_placeholder_allocation_timeout.received_mutex);
      ...
      log.verbose << log_prefix +
                         "MPAC Timeout thread: timeout reached, checking state";
...
void Connector::on_state_mode_phase_transition(States state,
...
void Connector::start() {
  ...
  connector_registers.psu_port_available.add_write_callback(
      [this](PsuOutputPortAvailability value) {
        log.debug << log_prefix + "PSU port available changed to " +
                         std::to_string((uint16_t)value);
      });

  connector_registers.rated_output_power_psu.add_write_callback(
      [this](float value) {
        if (rated_output_power_psu.has_value() and
            rated_output_power_psu.value() == value) {
          return;  // no change
        }

        rated_output_power_psu = value;
        log.info << log_prefix + "PSU Rated output power changed to " +
                        std::to_string(value) + " kW";
      });

  connector_registers.max_rated_psu_voltage.add_write_callback(
      [this](float value) {
        if (max_rated_psu_voltage.has_value() and
            max_rated_psu_voltage.value() == value) {
          return;  // no change
        }

        max_rated_psu_voltage = value;
        log.info << log_prefix + "PSU Max rated voltage changed to " +
                        std::to_string(value) + " V";
      });

  connector_registers.max_rated_psu_current.add_write_callback(
      [this](float value) {
        if (max_rated_psu_current.has_value() and
            max_rated_psu_current.value() == value) {
          return;  // no change
        }
...
void Connector::set_total_historical_energy_charged_per_connector(
    double energy_charged) {
...
void Connector::on_car_disconnected() { fsm.on_car_disconnected(); }
...
void Connector::send_needed_goose_frame(States state, ModePhase mode_phase) {
  switch (state) {
    ...
    case States::NoKeyYet:
      ...
      break;
    case States::ConnectedNoAllocation:
      goose_sender.send_module_placeholder_request(
          connector_config.global_connector_number);
      break;
    ...
    case States::Running: {
      switch (mode_phase) {
        case ModePhase::Off:
          ...
          break;
        case ModePhase::ExportCableCheck:
          goose_sender.send_insulation_detection_voltage_output(
              connector_config.global_connector_number,
              current_requested_voltage_current.voltage,
              current_requested_voltage_current.current);
          break;
        case ModePhase::OffCableCheck:
          goose_sender.send_insulation_detection_voltage_output_stoppage(
              connector_config.global_connector_number);
          break;
        case ModePhase::ExportPrecharge:
          goose_sender.send_precharge_voltage_output(
              connector_config.global_connector_number,
              current_requested_voltage_current.voltage,
              current_requested_voltage_current.current);
          break;
        case ModePhase::ExportCharging:
          ...
          goose_sender.send_charging_voltage_output(
              connector_config.global_connector_number,
              current_requested_voltage_current.voltage,
              current_requested_voltage_current.current);
          break;
...
void Connector::set_working_status(WorkingStatus status) {
  ...
  log.info << log_prefix +
                  "Set working status to: " + working_status_to_string(status);
...
void Connector::on_module_placeholder_allocation_response(bool success) {
  if (fsm.get_state() != States::ConnectedNoAllocation) {
    log.debug << log_prefix +
                     "Module placeholder allocation response received, but "
  ...
  if (success) {
    log.info << log_prefix +
                    "Module placeholder allocation received response, SUCCESS";
  } else {
    log.warning
        << log_prefix +
               "Module placeholder allocation received response, FAILED";
  }
...
void Connector::on_module_placeholder_allocation_timeout() {
  if (fsm.get_state() != States::ConnectedNoAllocation) {
    log.debug << log_prefix +
                     "Module placeholder allocation timeout, but not in "
...
void Connector::cancel_module_placeholder_allocation_timeout() {
  if (this->module_placeholder_allocation_timeout.thread.joinable()) {
    log.verbose
        << log_prefix +
               "Cancelling module placeholder allocation timeout thread";
...
void Connector::car_connect_disconnect_cycle(
  ...
  while (fsm.get_state() != States::ConnectedNoAllocation &&
         fsm.get_state() != States::Running) {
...
std::vector<uint8_t> Connector::get_hmac_key() {
