  ...
  while (psu_communication_is_ok()) {
    ...
    try {
      goose::frame::SecureGooseFrame frame(
          packet);  // todo: verify hmac (give hmac key via constructor)
      pdu = frame.pdu;
      decoded = true;
...
void Dispenser::init() {
  ...
  psu_registers->manufacturer.add_write_callback([this](uint16_t value) {
    log.info << "PSU Manufacturer changed to " + std::to_string(value);
  });
  psu_registers->protocol_version.add_write_callback([this](uint16_t value) {
    log.info << "PSU Protocol version changed to " + std::to_string(value);
  });
  psu_registers->esn_control_board.add_write_callback(
      [this](const std::string& value) {
        log.info << "PSU ESN Control Board changed to " + value;
      });
  psu_registers->software_version.add_write_callback(
      [this](const std::string& value) {
        log.info << "PSU Software version changed to " + value;
      });
  psu_registers->hardware_version.add_write_callback([this](uint16_t val) {
    log.info << "PSU HW version changed to " + std::to_string(val);
  });

  psu_registers->psu_running_mode.add_write_callback(
      [this](SettingPowerUnitRegisters::PSURunningMode value) {
        log.info << "Running mode changed to " +
