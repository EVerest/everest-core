  ...
  while (psu_communication_is_ok()) {
    try {
      ...
      if (req.has_value()) {
        ...
        modbus_server::transport_exceptions::ConnectionClosedException& e) {
      log.error
          << "Unsolicitated reporter noticed an closed connection; exiting...";
      break;
    } catch (std::runtime_error& e) {
      log.error << "Unsolicitated reporter thread error: " +
                       std::string(e.what());
      break;
    }
  ...
  while (psu_communication_is_ok()) {
    ...
    try {
      goose::frame::SecureGooseFrame frame(
          packet);  // note: hmac is verified below (only if configured)
      pdu = frame.pdu;
      decoded = true;
      log.verbose << "Received secure goose frame";
    } catch (std::runtime_error& e) {
      log.verbose << "Could not parse goose frame as secure: " +
                         std::string(e.what());
    }
    ...
    if (this->dispenser_config.allow_unsecured_goose && !decoded) {
      try {
        ...
        log.verbose << "Could not parse goose frame as non-secure: " +
                           std::string(e.what());
      }
    }
    ...
    if (strcmp(pdu.go_id, "CC/0$GO$PowerRequestReply") != 0) {
      log.info << "Received goose frame with weird go_id: " +
                      std::string(pdu.go_id);
      continue;
    }
    ...
    for (auto& c : connectors) {
      if (c->connector_config.global_connector_number ==
          response.charging_connector_no) {
        corresponding_connector_found = true;

        // verify hmac if configured
        if (dispenser_config.verify_secure_goose_hmac) {
          try {
            goose::frame::SecureGooseFrame secure_frame(packet,
                                                        c->get_hmac_key());
...
void Dispenser::modbus_event_loop_thread_run() {
  auto timeout =
      std::chrono::steady_clock::now() + dispenser_config.modbus_timeout_ms;
  ...
  try {
    while (psu_communication_is_ok()) {
      ...
      if (data_received) {
        timeout = std::chrono::steady_clock::now() +
                  dispenser_config.modbus_timeout_ms;
      } else {
        if (timeout < std::chrono::steady_clock::now()) {
          throw std::runtime_error(
              "No Modbus data received for " +
              std::to_string(dispenser_config.modbus_timeout_ms.count()) +
              " ms");
        }
    ...
    log.error
        << "Poll thread noticed an closed connection; exiting... Error: " +
               std::string(e.what());
  } catch (modbus_ssl::OpenSSLTransportException& e) {
    log.error << "Poll thread noticed an OpenSSL error; exiting... Error: " +
                     std::string(e.what());
  } catch (std::runtime_error& e) {
...
Dispenser::Dispenser(DispenserConfig dispenser_config,
  ...
  if (connector_configs.size() > MAX_NUMBER_OF_CONNECTORS) {
    throw std::runtime_error(
        "Too many connectors: " + std::to_string(connector_configs.size()) +
        "Max: " + std::to_string(MAX_NUMBER_OF_CONNECTORS));
  }

  // number connectors from 1 to n
  for (size_t local_connector_number = 1;
       local_connector_number <= connector_configs.size();
...
const int Dispenser::connect_with_retry(const char* ip, uint16_t port,
                                        int retries) {
  for (int i = 0; i < retries; i++) {
    try {
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds((int)(10 * std::pow(2, i))));
      if (i == retries - 1) {
        throw;
      }
    }
...
void Dispenser::init() {
  ...
  modbus_socket = connect_with_retry(dispenser_config.psu_host.c_str(),
                                     dispenser_config.psu_port, 10);
  ...
  if (dispenser_config.tls_config.has_value()) {
    ...
    transport = std::make_shared<modbus_server::ModbusSocketTransport>(
        modbus_socket.value());
  }

  protocol =
      std::make_shared<modbus_server::ModbusTCPProtocol>(transport, 0, 0);
  ...
  psu_registers->psu_mac.add_write_callback(
      [this](const uint8_t* value) { this->psu_mac_received = true; });

  // Callbacks for common power unit registers
  psu_registers->manufacturer.add_write_callback([this](uint16_t value) {
    log.debug << "Dispenser   : PSU Manufacturer changed to " +
                     std::to_string(value);
  });
  psu_registers->protocol_version.add_write_callback([this](uint16_t value) {
    log.debug << "Dispenser   : PSU Protocol version changed to " +
                     std::to_string(value);
  });
  psu_registers->esn_control_board.add_write_callback(
      [this](const std::string& value) {
        log.debug << "Dispenser   : PSU ESN Control Board changed to " + value;
      });
  psu_registers->software_version.add_write_callback(
      [this](const std::string& value) {
        log.debug << "Dispenser   : PSU Software version changed to " + value;
      });
  psu_registers->hardware_version.add_write_callback([this](uint16_t val) {
    log.debug << "Dispenser   : PSU HW version changed to " +
                     std::to_string(val);
  });

  psu_registers->psu_running_mode.add_write_callback(
      [this](SettingPowerUnitRegisters::PSURunningMode new_value) {
        if (psu_running_mode.has_value() and
            psu_running_mode.value() == new_value) {
          return;  // no change
        }

        psu_running_mode = new_value;
        log.info << "Dispenser   : PSU Running mode changed to " +
                        SettingPowerUnitRegisters::psu_running_mode_to_string(
                            new_value);
      });

  psu_registers->psu_mac.add_write_callback([this](const uint8_t* value) {
    char mac_str[18];
    sprintf(mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", value[0], value[1],
            value[2], value[3], value[4], value[5]);
    log.debug << "Dispenser   : 🍔 PSU (Big) MAC changed to " +
                     std::string(mac_str);
  ...
  server->set_read_holding_registers_request_cb(
      [this](const modbus_server::pdu::ReadHoldingRegistersRequest& req) {
  ...
  server->set_write_single_register_request_cb(
      [this](const modbus_server::pdu::WriteSingleRegisterRequest& req) {
        registry->on_write(req.register_address,
                           {(uint8_t)(req.register_value >> 8),
                            (uint8_t)(req.register_value & 0xff)});
        return modbus_server::pdu::WriteSingleRegisterResponse(req);
      });

  modbus_unsolicitated_event_thread =
      std::thread([this]() { modbus_unsolicitated_event_thread_run(); });

  goose_receiver_thread =
      std::thread([this]() { goose_receiver_thread_run(); });
}
...
bool Dispenser::is_stop_requested() {
  return psu_communication_state ==
         DispenserPsuCommunicationState::UNINITIALIZED;
}
