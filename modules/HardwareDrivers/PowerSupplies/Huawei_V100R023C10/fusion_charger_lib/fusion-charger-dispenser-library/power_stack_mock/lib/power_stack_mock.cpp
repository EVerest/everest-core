...
using fusion_charger::modbus_driver::raw_registers::
    offset_from_connector_number;
...
bool ConnectorCallbackResults::operator==(
    const ConnectorCallbackResults &rhs) const {
  return connector_upstream_voltage == rhs.connector_upstream_voltage &&
         output_voltage == rhs.output_voltage &&
         output_current == rhs.output_current &&
         contactor_status == rhs.contactor_status &&
...
std::ostream &operator<<(std::ostream &os,
                         const ConnectorCallbackResults &results) {
  os << "ConnectorCallbackResults{" << std::endl;
  os << "connector_upstream_voltage: " << results.connector_upstream_voltage
     << std::endl;
  ...
  os << "electronic_lock_status: " << (uint32_t)results.electronic_lock_status
     << std::endl;
  os << "}";
...
PowerStackMock::PowerStackMock(
    int client_socket, std::optional<std::tuple<SSL *, SSL_CTX *>> openssl_data,
...
PowerStackMock *PowerStackMock::from_config(PowerStackMockConfig config,
  ...
  if (config.tls_config.has_value()) {
    auto openssl_data =
        init_mutual_tls_server(client_socket, config.tls_config.value());
    SSL *ssl = std::get<0>(openssl_data);
    return new PowerStackMock(
        client_socket, openssl_data,
        std::make_shared<modbus_ssl::OpenSSLTransport>(ssl), config);
  }

  return new PowerStackMock(
      client_socket, std::nullopt,
      std::make_shared<modbus_server::ModbusSocketTransport>(client_socket),
      config);
}

std::vector<uint16_t> PowerStackMock::read_and_check(uint16_t start_address,
                                                     uint16_t quantity) {
  auto registers = client.read_holding_registers(start_address, quantity);
  if (registers.size() != quantity) {
    fail_printf(
        "Holding register at 0x%X, reading %d registers returned vector "
        "of length: %d",
...
std::unique_ptr<goose::frame::GooseFrameIntf> parse_goose_frame(
    const goose_ethernet::EthernetFrame &frame,
    std::optional<std::vector<uint8_t>> hmac_key = std::nullopt) {
  ...
  while (running) {
    ...
    try {
      if (config.verify_hmac) {
        frame = parse_goose_frame(
            packet,
            std::vector<uint8_t>(config.hmac_key, config.hmac_key + 48));
      } else {
    ...
    if (strcmp(pdu.go_id, "CC/0$GO$PowerRequest") == 0) {
      ...
      if (new_request.requirement_type ==
              fusion_charger::goose::RequirementType::
        ...
        response.result =
            fusion_charger::goose::PowerRequirementResponse::Result::SUCCESS;
        ...
        memcpy(response_frame->destination_mac_address,
               frame->source_mac_address, 6);
        ...
        if (config.enable_hmac) {
          std::vector<uint8_t> hmac_key(config.hmac_key, config.hmac_key + 48);
          frame = ((goose::frame::SecureGooseFrame *)response_frame.get())
                      ->serialize(hmac_key);
        } else {
          frame =
              ((goose::frame::GooseFrame *)response_frame.get())->serialize();
        }
    ...
    if (strcmp(pdu.go_id, "CC/0$GO$ShutdownRequest") == 0) {
      ...
      if (stop_charge_request_counter.find(global_connector_number) ==
          stop_charge_request_counter.end()) {
...
void PowerStackMock::on_pdu(const modbus_server::pdu::GenericPDU &pdu) {
  // psu_printf("Received PDU: %s", pdu.to_string().c_str());
...
std::vector<uint16_t> PowerStackMock::get_unsolicited_report_data(
    uint16_t start_address, uint16_t quantity) {
  std::lock_guard<std::mutex> guard(pdu_registers_mutex);

  if (pdu_registers.find(start_address) == pdu_registers.end()) {
    // fail_printf("No segment found at address 0x%X", start_address);
    return {};
  }
  ...
  if (segment.registers_count != quantity) {
    throw std::runtime_error("Expected " + std::to_string(quantity) +
                             " registers, got " +
  ...
  for (int i = 0; i < quantity; i++) {
    ...
    registers.push_back(segment.registers[i * 2] << 8 |
                        segment.registers[i * 2 + 1]);
  }

  return registers;
}

std::vector<uint16_t> PowerStackMock::read_registers(uint16_t start_address,
                                                     uint16_t quantity) {
  return client.read_holding_registers(start_address, quantity);
}

void PowerStackMock::write_registers(uint16_t start_address,
...
void PowerStackMock::send_hmac_key(uint16_t local_connector_number) {
  std::vector<uint16_t> hmac_key_vec;

  for (int i = 0; i < 48; i += 2) {
    hmac_key_vec.push_back((config.hmac_key[i] << 8) |
                           (config.hmac_key[i + 1] & 0xFF));
  }

  client.write_multiple_registers(
      0x2115 + (uint16_t)offset_from_connector_number(local_connector_number),
      hmac_key_vec);
}

void PowerStackMock::send_max_rated_current_of_output_port(
    float current, uint16_t local_connector_number) {
  client.write_multiple_registers(
      0x2102 + (uint16_t)offset_from_connector_number(local_connector_number),
      float_to_uint16_vec(current));
}

void PowerStackMock::send_min_rated_current_of_output_port(
    float current, uint16_t local_connector_number) {
  client.write_multiple_registers(
      0x2107 + (uint16_t)offset_from_connector_number(local_connector_number),
      float_to_uint16_vec(current));
}

void PowerStackMock::send_max_rated_voltage_of_output_port(
    float voltage, uint16_t local_connector_number) {
  client.write_multiple_registers(
      0x2100 + (uint16_t)offset_from_connector_number(local_connector_number),
      float_to_uint16_vec(voltage));
}

void PowerStackMock::send_min_rated_voltage_of_output_port(
    float voltage, uint16_t local_connector_number) {
  client.write_multiple_registers(
      0x2105 + (uint16_t)offset_from_connector_number(local_connector_number),
      float_to_uint16_vec(voltage));
}

void PowerStackMock::send_rated_power_of_output_port(
    float power, uint16_t local_connector_number) {
  client.write_multiple_registers(
      0x212D + (uint16_t)offset_from_connector_number(local_connector_number),
...
PowerStackMock::get_last_power_requirement_request(
...
uint32_t PowerStackMock::get_power_requirements_counter(
    uint16_t global_connector_number) {
...
uint32_t PowerStackMock::get_stop_charge_request_counter(
    uint16_t global_connector_number) {
...
float PowerStackMock::get_maximum_rated_charge_current(
    uint16_t local_connector_number) {
  auto read_result = client.read_holding_registers(
      0x1105 + (uint16_t)offset_from_connector_number(local_connector_number),
      2);
...
ConnectorCallbackResults PowerStackMock::get_connector_callback_values(
    uint16_t local_connector_number) {
  auto output_voltage = registers_to_float(client.read_holding_registers(
      0x1107 + (uint16_t)offset_from_connector_number(local_connector_number),
      2));

  auto output_current = registers_to_float(client.read_holding_registers(
      0x1109 + (uint16_t)offset_from_connector_number(local_connector_number),
      2));

  auto contactors_upstream_voltage =
      registers_to_float(client.read_holding_registers(
          0x1113 +
              (uint16_t)offset_from_connector_number(local_connector_number),
          2));

  auto contactors_status = (ContactorStatus)client.read_holding_registers(
      0x1154 + (uint16_t)offset_from_connector_number(local_connector_number),
      1)[0];

  auto electronic_lock_status =
      (ElectronicLockStatus)client.read_holding_registers(
          0x1156 +
              (uint16_t)offset_from_connector_number(local_connector_number),
          1)[0];
...
int PowerStackMock::client_socket() { return client_sock; }

void PowerStackMock::set_enable_answer_module_placeholder_allocation(
    bool enable) {
