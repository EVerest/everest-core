...
static void print(std::ostream& stream, const char* fmt, const char* prefix,
                  va_list args) {
  std::stringstream ss;
  ss << prefix;
  ss << ": ";
...
double uint16_vec_to_double(std::vector<uint16_t> vec) {
  uint8_t v0[8] = {
      (uint8_t)(vec[3] & 0xFF), (uint8_t)(vec[3] >> 8),
      (uint8_t)(vec[2] & 0xFF), (uint8_t)(vec[2] >> 8),
      (uint8_t)(vec[1] & 0xFF), (uint8_t)(vec[1] >> 8),
      (uint8_t)(vec[0] & 0xFF), (uint8_t)(vec[0] >> 8),
  };
