...
typedef fusion_charger::modbus_driver::raw_registers::
    SettingPowerUnitRegisters::PSURunningMode PSURunningMode;
...
struct DispenserInformation {
  ...
  friend std::ostream& operator<<(std::ostream& os,
                                  const DispenserInformation& info);
};
...
struct PowerStackMockConfig {
  ...
  bool verify_hmac =
      true;  // if true verify received goose frames with hmac key

  std::optional<tls_util::MutualTlsServerConfig> tls_config;

  std::function<void(const fusion_charger::goose::PowerRequirementRequest&)>
      power_requirement_request_callback;
  std::function<void(const fusion_charger::goose::StopChargeRequest&)>
...
class PowerStackMock {
 public:
  ...
  std::vector<uint16_t> get_unsolicited_report_data(uint16_t start_address,
                                                    uint16_t quantity);
  std::vector<uint16_t> read_registers(uint16_t start_address,
                                       uint16_t quantity);
  void write_registers(uint16_t start_address,
  ...
  void send_max_rated_current_of_output_port(float current,
                                             uint16_t local_connector_number);
  void send_min_rated_current_of_output_port(float current,
                                             uint16_t local_connector_number);
  void send_max_rated_voltage_of_output_port(float voltage,
                                             uint16_t local_connector_number);
  void send_min_rated_voltage_of_output_port(float voltage,
                                             uint16_t local_connector_number);
  void send_rated_power_of_output_port(float power,
                                       uint16_t local_connector_number);
  ...
  ConnectorCallbackResults get_connector_callback_values(
      uint16_t local_connector_number);
 ...
 private:
  ...
  std::unordered_map<
      uint16_t,
      fusion_charger::modbus_extensions::UnsolicitatedReportRequest::Segment>
      pdu_registers;
  ...
  std::vector<uint16_t> read_and_check(uint16_t start_address,
                                       uint16_t quantity);
  ...
  std::map<uint16_t,
           std::atomic<fusion_charger::goose::PowerRequirementRequest>>
      last_power_requirement_requests;
  std::map<uint16_t, std::atomic<uint32_t>> power_requirement_request_counter =
      {};
  std::map<uint16_t, std::atomic<fusion_charger::goose::StopChargeRequest>>
      last_stop_charge_requests;
  ...
  PowerStackMock(int client_socket,
                 std::optional<std::tuple<SSL*, SSL_CTX*>> openssl_data,
                 std::shared_ptr<modbus_server::ModbusTransport> transport,
                 PowerStackMockConfig config);
};
