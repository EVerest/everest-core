...
MqttClient::MqttClient(std::string mqtt_host, std::string mqtt_port) {
  ...
  MQTTErrors err =
      mqtt_init(&client, sockfd, (uint8_t *)sendbuf, sizeof(sendbuf),
                (uint8_t *)recvbuf, sizeof(recvbuf), NULL);
  ...
  err = mqtt_connect(&client, NULL, NULL, NULL, 0, NULL, NULL,
...
void MqttClient::background_thread_fn() {
  while (!stop_flag) {
    ...
    for (const auto &entry : queue_copy) {
      MQTTErrors err =
          mqtt_publish(&client, entry.topic.c_str(), entry.message.data(),
                       entry.message.size(), MQTT_PUBLISH_QOS_0);
      if (err != MQTT_OK) {
