...
class MqttPowerRequestPublisher {
 ...
 public:
  MqttPowerRequestPublisher(std::shared_ptr<MqttClient> mqtt_client,
                            const std::string& base_topic)
      : mqtt_client(mqtt_client), base_topic(base_topic) {}

  void publish(double voltage, double current,
               uint16_t global_connector_number) {
    const auto data = "{\"voltage\": " + std::to_string(voltage) +
...

    {
      ...
      if (last_publish_data.find(global_connector_number) !=
          last_publish_data.end()) {
    ...
    std::string topic =
        base_topic + std::to_string(global_connector_number) + "/power_request";
 ...
 private:
  ...
  std::unordered_map<uint8_t, std::chrono::steady_clock::time_point>
      publish_deadline;
  std::unordered_map<uint8_t, std::string> last_publish_data;
...
class Mock {
 private:
  ...
  std::chrono::_V2::steady_clock::time_point periodic_update_deadline =
      std::chrono::steady_clock::now();

  int not_sending_capabilities_counter =
      0;  // used to test "capabilities not received" error
  ...
  void periodic_update() {
    ...
    if (not_sending_capabilities_counter > 1) {
      for (int i = 1; i <= used_connectors;
           i++) {  // connector number starts at 1
  ...
  void send_goose_key_on_car_plugged_in(uint8_t local_connector_number) {
    ...
    } else if (car_plugged_in[local_connector_number] &&
...
void on_socket(int socket, void* context) {
  ...
  auto mock = std::make_unique<Mock>(std::unique_ptr<PowerStackMock>(
      PowerStackMock::from_config(*config, socket)));
...
int main(int argc, char* argv[]) {
  ...
  if (std::getenv("FUSION_CHARGER_MOCK_MQTT_HOST") &&
      std::getenv("FUSION_CHARGER_MOCK_MQTT_PORT")) {
    ...
    mqtt_publisher = std::make_shared<MqttPowerRequestPublisher>(
        std::make_shared<MqttClient>(mqtt_host, mqtt_port), mqtt_base_topic);
  ...
  if (mqtt_publisher) {
    config.power_requirement_request_callback =
        [&mqtt_publisher](const PowerRequirementRequest& req) {
          mqtt_publisher->publish(req);
        };

    config.stop_charge_request_callback =
        [&mqtt_publisher](const StopChargeRequest& req) {
          mqtt_publisher->publish(req);
        };
  }
