...
typedef fusion_charger::modbus_driver::raw_registers::ConnectorType
    ConnectorType;
...
struct DispenserConfig {
  ...
  bool allow_unsecured_goose =
      false;  // if set to true allow unsecured goose frames from the PSU, if
              // false only allow secured frames (hmac not verified)
  bool verify_secure_goose_hmac =
