...
typedef fusion_charger::modbus_driver::raw_registers::
    CollectedConnectorRegisters::ContactorStatus ContactorStatus;
...
class Dispenser {
 private:
  ...
  std::atomic<DispenserPsuCommunicationState> psu_communication_state =
  ...
  std::atomic<StopChargeReason> stop_charge_reason =
      StopChargeReason::INSULATION_FAULT;
  ...
  std::shared_ptr<Connector> get_connector(int local_connector_number) {
    ...
    if (local_connector_number > connectors.size()) {
      throw std::runtime_error(
          "Connector number too high. Max local connector number: " +
