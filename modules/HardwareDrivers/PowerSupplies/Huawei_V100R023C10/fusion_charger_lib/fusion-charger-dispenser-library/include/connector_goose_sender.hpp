...
class ConnectorGooseSender {
 ...
 public:
  ConnectorGooseSender(
      ...
      bool secure = false, logs::LogIntf log = logs::log_printf);
   ...
   * frame
   */
  void send_power_requirement(uint16_t connector_no,
   ...
   */
  void send_insulation_detection_voltage_output(uint16_t connector_no,
   ...
   * @param current The requested current
   */
  void send_precharge_voltage_output(uint16_t connector_no, float voltage,
                                     float current);
   ...
   * @param current The requested current
   */
  void send_charging_voltage_output(uint16_t connector_no, float voltage,
                                    float current);
};
