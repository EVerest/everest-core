...
class ConnectorFSM {
  ...
  std::string
      log_prefix;  // Prefix for log messages, can be set in the constructor
   ...
   */
  void transition(std::optional<States> new_state,
 ...
 public:
  ...
  ConnectorFSM(Callbacks callbacks, logs::LogIntf log,
...
class Connector {
  typedef fusion_charger::modbus_driver::raw_registers::WorkingStatus
      WorkingStatus;

  typedef fusion_charger::modbus_driver::raw_registers::
      PsuOutputPortAvailability PsuOutputPortAvailability;

  typedef fusion_charger::modbus_driver::raw_registers::ConnectionStatus
      ConnectionStatus;

  typedef fusion_charger::modbus_driver::ConnectorRegistersConfig
      ConnectorRegistersConfig;
 ...
 public:
   ...
   */
  std::vector<uint8_t> get_hmac_key();
 ...
 private:
  ...
  std::function<void()>
      do_unsolicitated_report_callback;  // callback to dispenser to trigger an
                                         // unsolicitated report

  std::optional<float>
      rated_output_power_psu;  // in kW, set by register callback
   ...
   */
  void on_psu_mac_change(std::vector<uint8_t> mac_address);
