...
struct ConnectorFSM_Fixture : public ::testing::Test {
  ConnectorFSM::Callbacks callbacks{
      ...
      .mode_phase_transition =
          [this](ModePhase mode_phase) { mode_phase_transition_counter++; },
      .any_transition = [this](
                            States state,
                            ModePhase mode_phase) { any_transition_counter++; },
  };
  ConnectorFSM fsm{callbacks, logs::log_printf};

  uint32_t state_transition_counter = 0;
  uint32_t mode_phase_transition_counter = 0;
  uint32_t any_transition_counter = 0;
