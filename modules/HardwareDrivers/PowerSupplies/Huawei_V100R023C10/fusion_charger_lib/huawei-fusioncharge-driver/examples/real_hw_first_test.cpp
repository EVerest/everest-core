...
int main(int argc, char* argv[]) {
  ...
  psu_registers.manufacturer.add_write_callback([](uint16_t value) {
    printf("PSU Manufacturer changed to %d\n", value);
  });
  psu_registers.protocol_version.add_write_callback([](uint16_t value) {
    printf("PSU Protocol version changed to %d\n", value);
  });
  psu_registers.esn_control_board.add_write_callback(
      [](const std::string& value) {
        printf("PSU ESN Control Board changed to %s\n", value.c_str());
      });
  psu_registers.software_version.add_write_callback(
      [](const std::string& value) {
        printf("PSU Software version changed to %s\n", value.c_str());
      });
  psu_registers.hardware_version.add_write_callback(
      [](uint16_t val) { printf("PSU HW version changed to %d\n", val); });

  psu_registers.psu_running_mode.add_write_callback(
      [](SettingPowerUnitRegisters::PSURunningMode value) {
        printf("PSU Running mode changed to %s\n",
               SettingPowerUnitRegisters::psu_running_mode_to_string(value)
                   .c_str());
      });

  connector_registers.hmac_key.add_write_callback([](const uint8_t* value) {
  ...
  connector_registers.psu_port_available.add_write_callback(
      [](PsuOutputPortAvailability value) {
        printf("PSU port available changed to %d\n", (uint16_t)value);
      });

  connector_registers.rated_output_power_psu.add_write_callback(
      [](float value) {
        printf("Rated output power PSU changed to %f\n", value);
      });

  connector_registers.rated_output_power_connector.add_write_callback(
      [](float value) {
        printf("Rated output power connector changed to %f\n", value);
      });
  ...
  server.set_read_holding_registers_request_cb(
      [&register_registry](
          const modbus_server::pdu::ReadHoldingRegistersRequest& req) {
        // printf("Read %04x-%04x\n", req.register_start,
        //  req.register_start + req.register_count);
        auto data =
            register_registry.on_read(req.register_start, req.register_count);
        return modbus_server::pdu::ReadHoldingRegistersResponse(req, data);
      });
  server.set_write_multiple_registers_request_cb(
      [&register_registry](
          const modbus_server::pdu::WriteMultipleRegistersRequest& req) {
        // printf("Write %04x-%04x\n", req.register_start,
        //  req.register_start + req.register_count - 1);
  ...
  server.set_write_single_register_request_cb(
      [&register_registry](
          const modbus_server::pdu::WriteSingleRegisterRequest& req) {
        // printf("Write %04x\n", req.register_address);
        register_registry.on_write(req.register_address,
  ...
  auto unsolicitated_reporter = std::thread([&server, &register_registry,
                                             &closed]() {
    ...
    while (true) {
      try {
        ...
        if (req.has_value()) {
          server.send_unsolicitated_report(req.value(),
                                           std::chrono::seconds(3));
          // printf("Successful unsolicitated report\n");
        }
      } catch (
          modbus_server::transport_exceptions::ConnectionClosedException& e) {
        printf(
            "Unsolicitated reporter noticed an closed connection; "
            "exiting...\n");
        closed = true;
        return;
  ...
  auto goose_thread = std::thread([&eth, &closed, &psu_registers,
                                   &connector_registers]() {
    uint16_t stNum = 1;
    while (true) {
      std::this_thread::sleep_for(std::chrono::seconds(1));
      ...
      report_pdu.requirement_type =
          fusion_charger::goose::RequirementType::Charging;
      ...
      eth.send_packet(frame.serialize(std::vector<uint8_t>(hmac, hmac + 48)));
  ...
  auto poll_thread = std::thread([&pcl, &closed]() {
    try {
      while (true) {
        ...
        modbus_server::transport_exceptions::ConnectionClosedException& e) {
      printf(
          "Poll thread noticed an closed connection; "
          "exiting...\n");
      closed = true;
    }
  });
  ...
  auto dummy_data_changer_thread = std::thread([&connector_registers]() {
    ...
    // collected_connector_registers_data_providers.contactor_status
    //     .update_value(1);
    // printf("👨‍💻 Changed contactor status to 1\n");

    connector_registers.connection_status.update_value(
        ConnectionStatus::FULL_CONNECTED);
    printf("👨‍💻 Changed connection status to full connected\n");

    // collected_connector_registers_data_providers.charging_event_connector
    //     .report(1);
    // printf(
    //     "👨‍💻 Charging event of connector, reported stop to "
    //     "charging\n");

    connector_registers.working_status.update_value(
        WorkingStatus::STANDBY_WITH_CONNECTOR_INSERTED);
    printf(
        "👨‍💻 Update working status to standby with inserted "
        "charger\n");
  });
