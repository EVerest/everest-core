...
TEST(PowerRequirementRequest, from_pdu_real_world_test_1) {
  const char raw_data[] =
      "618197801543432f3024474f24506f77657252657175657374008102"
  ...
  uint8_t data[sizeof(raw_data) / 2];
  for (size_t i = 0; i < sizeof(data); ++i) {
    sscanf(&raw_data[2 * i], "%2hhx", &data[i]);
  }

  ::goose::frame::GoosePDU pdu(std::vector<uint8_t>(data, data + sizeof(data)));
  ...
  EXPECT_EQ(request.requirement_type,
