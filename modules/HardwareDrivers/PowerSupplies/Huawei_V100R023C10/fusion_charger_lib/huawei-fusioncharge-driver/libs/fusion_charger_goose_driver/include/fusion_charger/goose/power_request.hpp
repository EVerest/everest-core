...
enum class RequirementType : uint16_t {
...
struct PowerRequirementRequest {
  uint16_t        charging_connector_no;
  uint16_t        charging_sn = 0xffff;
  ...
  ::goose::frame::GoosePDU to_pdu(
      ::goose::frame::GooseTimestamp timestamp =
          ::goose::frame::GooseTimestamp::now()) const {
    ...
    pdu.apdu_entries[2] =
        utils::make_u16(static_cast<uint16_t>(requirement_type));
    pdu.apdu_entries[3] = utils::make_u16(static_cast<uint16_t>(mode));
  ...
  // todo: test
  ::goose::frame::GooseTimestamp from_pdu(
      const ::goose::frame::GoosePDU& input) {
    if (input.apdu_entries.size() != 8) {
      throw std::runtime_error("Expected 8 APDU entries, got " +
                               std::to_string(input.apdu_entries.size()));
    }

    if (strcmp(input.go_cb_ref, "CC/0$GO$PowerRequest") != 0) {
      throw std::runtime_error("Expected go_cb_ref CC/0$GO$PowerRequest, got " +
    ...
    requirement_type =
        static_cast<RequirementType>(utils::expect_u16(input.apdu_entries[2]));
...
struct PowerRequirementResponse {
  ...
  ::goose::frame::GoosePDU to_pdu(
    ...
    pdu.apdu_entries[2] =
        utils::make_u16(static_cast<uint16_t>(requirement_type));
    pdu.apdu_entries[3] = utils::make_u16(static_cast<uint16_t>(result));
    pdu.apdu_entries[4] = utils::make_u16(static_cast<uint16_t>(mode));
  ...
  ::goose::frame::GooseTimestamp from_pdu(
    ...
    if (input.apdu_entries.size() != 9) {
      throw std::runtime_error("Expected 9 APDU entries, got " +
                               std::to_string(input.apdu_entries.size()));
    }

    if (strcmp(input.go_cb_ref, "CC/0$GO$PowerRequestReply") != 0) {
      throw std::runtime_error(
          "Expected go_cb_ref CC/0$GO$PowerRequestReply, got " +
          std::string(input.go_cb_ref));
    }
