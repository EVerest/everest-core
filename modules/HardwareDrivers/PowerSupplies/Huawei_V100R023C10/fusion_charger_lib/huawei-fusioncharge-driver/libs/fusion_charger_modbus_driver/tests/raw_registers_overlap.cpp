...
TEST(CommonDispenserRegisters, no_internal_overlap) {
  DataProviderHolding<uint16_t> manufacturer(0);
  DataProviderHolding<uint16_t> model(1);
  DataProviderHolding<uint16_t> protocol_version(2);
  DataProviderHolding<uint16_t> hardware_version(2);
  DataProviderStringHolding<48> software_version;

  CommonDispenserRegisters::DataProviders data_providers{
      manufacturer, model, protocol_version, hardware_version,
      software_version};
...
TEST(CommonPowerUnitRegisters, no_internal_overlap) {
  DataProviderHolding<uint16_t> manufacturer(0);
  DataProviderHolding<uint16_t> protocol_version(1);
  DataProviderHolding<uint16_t> hardware_version(2);
  DataProviderStringHolding<48> software_version;
  DataProviderStringHolding<32> esn_control_board;

  CommonPowerUnitRegisters::DataProviders data_providers{
      manufacturer, protocol_version, hardware_version, software_version,
      esn_control_board};
...
TEST(CollectedDispenserRegisters, no_internal_overlap) {
  DataProviderHolding<uint16_t> number_of_charging_connectors(0);
  DataProviderStringHolding<22> esn_dispenser;
  DataProviderHoldingUnsolicitatedReportCallback<uint32_t> time_sync(
      0, []() { return true; });

  CollectedDispenserRegisters::DataProviders data_providers{
      number_of_charging_connectors, esn_dispenser, time_sync};
...
TEST(CollectedConnectorRegisters, no_internal_overlap) {
  ...
  DataProviderHoldingUnsolicitatedReportCallback<WorkingStatus> working_status(
      WorkingStatus::STANDBY, always_true);
  DataProviderHoldingUnsolicitatedReportCallback<ConnectionStatus>
      connection_status(ConnectionStatus::NOT_CONNECTED, always_true);
  DataProviderHolding<uint16_t> connector_no(0);  // 1-12
  DataProviderHolding<float>    contactor_upstream_voltage(0);
  DataProviderMemoryHolding<6>  mac_address;
  // 0 off, 1 on
  DataProviderHoldingUnsolicitatedReportCallback<
      CollectedConnectorRegisters::ContactorStatus>
      contactor_status(CollectedConnectorRegisters::ContactorStatus::OFF,
                       always_true);
  DataProviderHoldingUnsolicitatedReportCallback<
      CollectedConnectorRegisters::ElectronicLockStatus>
      electronic_lock_status(
          CollectedConnectorRegisters::ElectronicLockStatus::UNLOCKED,
          always_true);
  DataProviderHoldingUnsolicitatedReportCallback<
      CollectedConnectorRegisters::ChargingEventConnector>
      charging_event_connector(
          CollectedConnectorRegisters::ChargingEventConnector::START_TO_STOP,
          always_true);

  CollectedConnectorRegisters::DataProviders data_providers{
  ...
  CollectedConnectorRegisters registers(ConnectorOffset::CONNECTOR_1_OFFSET,
                                        data_providers);
...
TEST(SettingPowerUnitRegisters, no_internal_overlap) {
  DataProviderHolding<SettingPowerUnitRegisters::PSURunningMode>
      psu_running_mode(SettingPowerUnitRegisters::PSURunningMode::FAULTY);
  ...
  SettingPowerUnitRegisters::DataProviders data_providers{
...
TEST(SettingConnectorRegisters, no_internal_overlap) {
  ...
  DataProviderHolding<PsuOutputPortAvailability> psu_port_available(
      PsuOutputPortAvailability::NOT_AVAILABLE);

  SettingConnectorRegisters::DataProviders data_providers{
      max_rated_psu_voltage,        max_rated_psu_current,
  ...
  SettingConnectorRegisters registers(ConnectorOffset::CONNECTOR_1_OFFSET,
                                      data_providers);
  ASSERT_NO_THROW(registers.verify_internal_overlap());
}
