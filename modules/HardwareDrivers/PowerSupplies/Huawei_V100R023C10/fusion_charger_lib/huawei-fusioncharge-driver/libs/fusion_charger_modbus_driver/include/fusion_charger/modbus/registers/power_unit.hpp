...
struct PowerUnitRegisters {
  using PSURunningMode =
      raw_registers::SettingPowerUnitRegisters::PSURunningMode;
  ...
  PowerUnitRegisters()
      : manufacturer(0),
        ...
        // todo: determine if psu_running_mode's default is good or not
  ...
  void add_to_registry(
    ...
    raw_registers::CommonPowerUnitRegisters::DataProviders
        common_data_providers{manufacturer, protocol_version, hardware_version,
                              software_version, esn_control_board};

    raw_registers::SettingPowerUnitRegisters::DataProviders
        setting_data_providers{psu_running_mode,           psu_mac,
    ...
    registry.add(std::make_unique<raw_registers::CommonPowerUnitRegisters>(
        common_data_providers));

    registry.add(std::make_unique<raw_registers::SettingPowerUnitRegisters>(
        setting_data_providers));
  }
};
