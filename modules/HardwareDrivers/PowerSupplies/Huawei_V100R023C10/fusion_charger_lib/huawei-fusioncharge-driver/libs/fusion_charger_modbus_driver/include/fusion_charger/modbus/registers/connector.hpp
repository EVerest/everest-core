...
struct ConnectorRegistersConfig {
  ...
  using ContactorStatus =
      raw_registers::CollectedConnectorRegisters::ContactorStatus;
  using ElectronicLockStatus =
      raw_registers::CollectedConnectorRegisters::ElectronicLockStatus;

  uint8_t                               mac_address[6];
  ConnectorType                         type;
  uint16_t                              global_connector_no;
  uint16_t                              connector_number;
...
struct ConnectorRegisters {
  ...
  using ElectronicLockStatus =
      raw_registers::CollectedConnectorRegisters::ElectronicLockStatus;
  using ChargingEventConnector =
      raw_registers::CollectedConnectorRegisters::ChargingEventConnector;
  ...
  DataProviderHoldingUnsolicitatedReportCallback<ConnectionStatus>
                                connection_status;
  DataProviderHolding<uint16_t> connector_no;  // 1-12
  ...
  // unlocked = 0, locked = 1
  DataProviderCallbacksUnsolicitated<ElectronicLockStatus>
      electronic_lock_status;
  DataProviderUnsolicitatedEvent<ChargingEventConnector>
  ...
  // todo: determine what data should be provided via callbacks
  /**
  ...
  ConnectorRegisters(ConnectorRegistersConfig config)
      : connector_number(config.connector_number),
        ...
        // TODO!: always report replace with "when charging"
        ...
        contactor_upstream_voltage(config.get_contactor_upstream_voltage,
                                   utils::ignore_write<float>),
        mac_address(config.mac_address),
        contactor_status(config.get_contactor_status,
                         utils::ignore_write<ContactorStatus>,
                         utils::always_report),
        electronic_lock_status(config.get_electronic_lock_status,
                               utils::ignore_write<ElectronicLockStatus>,
                               utils::always_report),
  ...
  void add_to_registry(
    ...
    ConnectorOffset offset = fusion_charger::modbus_driver::raw_registers::
        offset_from_connector_number(connector_number);

    raw_registers::CollectedConnectorRegisters::DataProviders
        collected_connector_registers{
    ...
    raw_registers::SettingConnectorRegisters::DataProviders
        setting_connector_registers{
    ...
    raw_registers::AlarmConnectorRegisters::DataProviders
        alarm_connector_registers{dc_output_contact_fault,
                                  inverse_connection_dispenser_inlet_cable};

    registry.add(std::make_unique<raw_registers::CollectedConnectorRegisters>(
        offset, collected_connector_registers));
    registry.add(std::make_unique<raw_registers::SettingConnectorRegisters>(
        offset, setting_connector_registers));
    registry.add(std::make_unique<raw_registers::AlarmConnectorRegisters>(
        offset, alarm_connector_registers));
  }
};
