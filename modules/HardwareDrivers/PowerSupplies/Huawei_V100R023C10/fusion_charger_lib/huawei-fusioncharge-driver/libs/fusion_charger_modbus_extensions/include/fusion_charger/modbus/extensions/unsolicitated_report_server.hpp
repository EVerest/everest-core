...
class UnsolicitatedReportBasicServer : public modbus_server::ModbusBasicServer {
 public:
  UnsolicitatedReportBasicServer(
      std::shared_ptr<modbus_server::PDUCorrelationLayerIntf> corr_layer,
   ...
   * error
   */
  std::optional<UnsolicitatedReportResponse> send_unsolicitated_report(
      UnsolicitatedReportRequest& request, std::chrono::milliseconds timeout);
};
