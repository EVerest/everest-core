...
class DummyPDUCorrelationLayer : public modbus_server::PDUCorrelationLayerIntf {
 ...
 public:
  ...
  modbus_server::pdu::GenericPDU request_response(
      const modbus_server::pdu::GenericPDU& request,
  ...
  void request_without_response(
      const modbus_server::pdu::GenericPDU& request) override {
    last_request.push_back(request);
  }
  ...
  std::optional<modbus_server::pdu::GenericPDU> call_on_pdu(
      const modbus_server::pdu::GenericPDU& pdu) {
...
TEST(UnsolicitatedReportBasicServer, send_unsolicitated_report_no_response) {
  ...
  auto response =
      server.send_unsolicitated_report(request, std::chrono::milliseconds(100));
...
TEST(UnsolicitatedReportBasicServer,
     sent_unsolicitated_report_is_defragmented) {
  auto corr_layer = std::make_shared<DummyPDUCorrelationLayer>();
  UnsolicitatedReportBasicServer server(corr_layer);
  UnsolicitatedReportRequest     request;
