...
std::vector<uint8_t> OpenSSLTransport::read_bytes(size_t count) {
  ...
  while (read < count) {
    ...
    if (ret <= 0) {
      ...
      throw OpenSSLTransportException(
          "SSL_read failed with error " +
              std::string(ERR_error_string(err, NULL)),
          err);  // todo: generate string from error code
    }
    read += ret;
  }
...
std::optional<std::vector<uint8_t>> OpenSSLTransport::try_read_bytes(
    size_t count) {
  std::vector<uint8_t> buffer(count);
  size_t               read = 0;
...
void OpenSSLTransport::write_bytes(const std::vector<uint8_t>& bytes) {
  size_t written = 0;
