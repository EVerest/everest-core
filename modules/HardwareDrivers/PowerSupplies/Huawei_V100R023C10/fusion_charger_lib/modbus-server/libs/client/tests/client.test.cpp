...
TEST(ModbusClient, read_holding_registers) {
  ...
  correlation_layer->add_next_answer(
      pdu::GenericPDU(0x03, {0x04, 0x01, 0x02, 0x03, 0x04}));
...
TEST(ModbusClient, write_single_register) {
  ...
  correlation_layer->add_next_answer(
      pdu::GenericPDU(0x06, {0x01, 0x02, 0x03, 0x04}));
...
TEST(ModbusClient, write_multiple_registers) {
  ...
  ASSERT_NO_THROW(correlation_layer->add_next_answer(
      pdu::WriteMultipleRegistersResponse(0xfeed, 0x0002).to_generic()));
...
TEST(ModbusClient, write_multiple_registers_error_response) {
  ...
  ASSERT_THROW(client.write_multiple_registers(0xfeed, {0xc0de, 0xbeef}),
               pdu::PDUException);
}
