...
GenericPDU::GenericPDU(const std::vector<uint8_t>& data_with_function_code)
    : data() {
  if (data_with_function_code.size() < 1) {
    throw EncodingError("GenericPDU", "Data size must be at least 1");
  }
  this->data.insert(this->data.end(), data_with_function_code.begin() + 1,
                    data_with_function_code.end());
  function_code = data_with_function_code[0];
}

GenericPDU::GenericPDU() : function_code(0), data() {}

GenericPDU::GenericPDU(uint8_t function_code, const std::vector<uint8_t>& data)
    : function_code(function_code), data(data) {}

std::vector<uint8_t> GenericPDU::to_vector() const {
...
GenericPDU ErrorPDU::to_generic() const {
  return GenericPDU({(uint8_t)(function_code | 0x80), exception_code});
}

EncodingError::EncodingError(const std::string& encode_class,
                             const std::string& msg)
    : std::runtime_error("Could not encode from " + encode_class + ": " + msg) {
}

DecodingError::DecodingError(const GenericPDU&  original_data,
                             const std::string& decode_class,
                             const std::string& msg)
    : std::runtime_error("Could not decode to " + decode_class + ": " + msg),
...
ErrorPDU::ErrorPDU(uint8_t function_code, uint8_t exception_code)
    : function_code(function_code & 0x7f), exception_code(exception_code) {}

std::string modbus_server::pdu::exception_code_to_string(
    PDUExceptionCode code) {
  switch (code) {
    ...
    default:
      return "Unknown (" + std::to_string((uint8_t)code) + ")";
  }
}
...
PDUException::PDUException(const GenericPDU& pdu) {
  ...
  this->message =
      "PDUException: " +
      exception_code_to_string((PDUExceptionCode)this->exception_code);
}

PDUException::PDUException(PDUExceptionCode exception_code)
    : exception_code((uint8_t)exception_code),
      message("PDUException: " + exception_code_to_string(exception_code)) {}

const char* PDUException::what() const noexcept { return message.c_str(); };

uint8_t PDUException::get_exception_code() const { return exception_code; }
