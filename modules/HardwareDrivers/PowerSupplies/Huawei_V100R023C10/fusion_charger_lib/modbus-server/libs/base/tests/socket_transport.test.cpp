...
TEST(SocketTransport, read_works) {
  ...
  std::vector<uint8_t> read_data = transport.read_bytes(data.size());
...
TEST(SocketTransport, closed_socket_read_throws_err) {
  ...
  ASSERT_THROW(transport.read_bytes(1),
               modbus_server::transport_exceptions::ConnectionClosedException);
