...
void ReadHoldingRegistersRequest::from_generic(const GenericPDU& generic) {
  if (generic.function_code != 0x03) {
    throw DecodingError(generic, "ReadHoldingRegisterRequest",
  ...
  if (register_count == 0) {
    throw DecodingError(generic, "ReadHoldingRegisterRequest",
                        "Register count cannot be zero");
  }

  if (register_count > 125) {
    throw DecodingError(generic, "ReadHoldingRegisterRequest",
                        "Register count too big");
  }
}
...
void ReadHoldingRegistersResponse::from_generic(const GenericPDU& generic) {
  if (generic.function_code != 0x03) {
    throw DecodingError(generic, "ReadHoldingRegisterResponse",
                        "Invalid function code");
  }
  ...
  register_data =
      std::vector<uint8_t>(generic.data.begin() + 1, generic.data.end());
}

GenericPDU ReadHoldingRegistersResponse::to_generic() const {
  if (register_count > 125) {
    throw EncodingError("ReadHoldingRegistersResponse",
                        "Register count too big");
  }

  if (register_data.size() != register_count * 2) {
    throw EncodingError("ReadHoldingRegistersResponse",
                        "Data size (" + std::to_string(register_data.size()) +
  ...
  generic.data.insert(generic.data.end(), register_data.begin(),
                      register_data.end());

  return generic;
}

std::vector<uint16_t> ReadHoldingRegistersResponse::get_register_data() const {
