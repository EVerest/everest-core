...
TEST(PDUCorrelationLayer, request_response_single) {
  ...
  auto response =
      pal.request_response(request, std::chrono::milliseconds(1000));
...
TEST(PDUCorrelationLayer, request_response_parallel_out_of_order) {
  ...
  auto thread_req_1 =
      std::thread([&pal, &request1, &response1, &response1_time]() {
        std::this_thread::sleep_for(std::chrono::milliseconds(5));
        response1 =
            pal.request_response(request1, std::chrono::milliseconds(1000));
        response1_time = std::chrono::steady_clock::now();
      });

  auto thread_req_2 =
      std::thread([&pal, &request2, &response2, &response2_time]() {
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
        response2 =
            pal.request_response(request2, std::chrono::milliseconds(1000));
...
TEST(PDUCorrelationLayer, timeout) {
  ...
  ASSERT_THROW(pal.request_response(request, std::chrono::milliseconds(100)),
               std::runtime_error);
}
...
TEST(PDUCorrelationLayer, on_pdu_callback) {
  ...
  pal.set_on_pdu([](const pdu::GenericPDU& pdu) {
    return pdu::GenericPDU(pdu.function_code, {0x03, 0x04, 0x05});
  });
...
TEST(PDUCorrelationLayer, timeout_works_accurate) {
  ...
  ASSERT_THROW(pal->request_response(request, std::chrono::milliseconds(50)),
               std::runtime_error);
  auto time_after = std::chrono::steady_clock::now();

  ASSERT_TRUE(time_after - time_before <
              std::chrono::milliseconds(60));  // 10ms tolerance
