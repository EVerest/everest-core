...
class ConnectionClosedException : public std::runtime_error {
 public:
  ...
  ConnectionClosedException(const std::string& details)
      : std::runtime_error("Connection closed: " + details) {}
};
...
class ModbusTransport {
 public:
   ...
   */
  virtual std::vector<uint8_t> read_bytes(size_t count) = 0;

  virtual std::optional<std::vector<uint8_t>> try_read_bytes(size_t count) = 0;
...
class ModbusSocketTransport : public ModbusTransport {
 ...
 public:
  ...
  std::vector<uint8_t>                read_bytes(size_t count) override;
  std::optional<std::vector<uint8_t>> try_read_bytes(size_t count) override;
