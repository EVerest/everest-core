...
static_assert(
    std::is_base_of<WriteSingleRegister, WriteSingleRegisterRequest>::value);
static_assert(
    std::is_base_of<WriteSingleRegister, WriteSingleRegisterResponse>::value);
...
TEST(WriteSingleRegister, from_generic_works_correctly) {
  std::vector<uint8_t> data = {0x06, 0xde, 0xad, 0xbe, 0xef};
