...
ModbusTCPProtocol::ModbusTCPProtocol(std::shared_ptr<ModbusTransport> transport, uint16_t unit_id,
...
void ModbusTCPProtocol::send_blocking(const pdu::GenericPDU& pdu, const ModbusProtocol::Context& context) {
    ...
    uint16_t size_in_header = pdu_data.size() + 1; // +1 for the unit id in the header
