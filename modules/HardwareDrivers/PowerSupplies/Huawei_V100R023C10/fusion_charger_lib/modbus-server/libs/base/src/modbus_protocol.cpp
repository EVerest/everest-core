...
ModbusProtocol::ModbusProtocol(std::shared_ptr<ModbusTransport> transport)
    : transport(transport) {}

bool ModbusProtocol::Context::operator==(
    const ModbusProtocol::Context& other) const {
...
ModbusTCPProtocol::ModbusTCPContext::ModbusTCPContext(
    const ModbusProtocol::Context& context) {
...
ModbusTCPProtocol::ModbusTCPContext::ModbusTCPContext()
    : transaction_id(0), protocol_id(0), unit_id(0) {}
...
ModbusTCPProtocol::ModbusTCPProtocol(std::shared_ptr<ModbusTransport> transport,
    ...
    : ModbusProtocol(transport),
      sending_unit_id(unit_id),
...
std::tuple<ModbusProtocol::Context, pdu::GenericPDU>
ModbusTCPProtocol::receive_blocking() {
...
std::optional<
    std::pair<modbus_server::ModbusProtocol::Context, pdu::GenericPDU>>
modbus_server::ModbusTCPProtocol::try_receive() {
  ...
  std::pair<modbus_server::ModbusProtocol::Context, pdu::GenericPDU> result = {
      context.to_context(), pdu::GenericPDU(data)};

  return std::optional(result);
}
void ModbusTCPProtocol::send_blocking(const pdu::GenericPDU&         pdu,
                                      const ModbusProtocol::Context& context) {
  auto     pdu_data = pdu.to_vector();
  uint16_t size_in_header =
      pdu_data.size() + 1;  // +1 for the unit id in the header
