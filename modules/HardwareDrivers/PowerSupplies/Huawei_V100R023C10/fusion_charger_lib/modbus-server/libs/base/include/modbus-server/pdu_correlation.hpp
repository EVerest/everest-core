...
class PDUCorrelationLayerIntf {
 public:
  using On_PDU_Callback_t =
      std::function<std::optional<pdu::GenericPDU>(const pdu::GenericPDU&)>;
 ...
 public:
   ...
   */
  virtual pdu::GenericPDU request_response(
      const pdu::GenericPDU& request, std::chrono::milliseconds timeout) = 0;
   ...
   */
  void set_on_pdu(On_PDU_Callback_t on_pdu) { this->on_pdu = on_pdu; }
};
...
class PDUCorrelationLayer : public PDUCorrelationLayerIntf {
 protected:
  ...
  void on_poll_data(modbus_server::ModbusProtocol::Context context,
                    pdu::GenericPDU                        pdu);

 public:
  PDUCorrelationLayer(std::shared_ptr<ModbusProtocol> protocol)
      : protocol(protocol) {}

  void            blocking_poll() override;
  bool            poll() override;
  pdu::GenericPDU request_response(const pdu::GenericPDU&    request,
