...
void modbus_server::PDUCorrelationLayer::on_poll_data(
    modbus_server::ModbusProtocol::Context context, pdu::GenericPDU pdu) {
  {
...
bool modbus_server::PDUCorrelationLayer::poll() {
  std::optional<
      ...
      data = this->protocol->try_receive();
...
pdu::GenericPDU PDUCorrelationLayer::request_response(
    const pdu::GenericPDU& request, std::chrono::milliseconds timeout) {
  auto context = this->protocol->new_send_context();

  {
    std::lock_guard<std::mutex> lock(this->listening_for_response_mutex);
    this->listening_for_response.push_back(
        {context, request.function_code, std::nullopt});
  }
  ...
  while (std::chrono::steady_clock::now() < end_time) {
    ...
    if (this->listening_for_response[index].pdu.has_value()) {
      ...
      this->listening_for_response.erase(this->listening_for_response.begin() +
                                         index);
...
void PDUCorrelationLayer::request_without_response(
    const pdu::GenericPDU& request) {
  auto context = this->protocol->new_send_context();
  this->protocol->send_blocking(request, context);
}
