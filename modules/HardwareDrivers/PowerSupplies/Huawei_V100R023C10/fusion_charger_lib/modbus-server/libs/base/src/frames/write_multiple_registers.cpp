...
void WriteMultipleRegistersRequest::from_generic(const GenericPDU& generic) {
  ...
  uint8_t byte_count = generic.data[4];

  if (register_count > 0x007b) {
    throw DecodingError(
        generic, "WriteMultipleRegistersRequest",
  ...
  register_data =
      std::vector<uint8_t>(generic.data.begin() + 5, generic.data.end());
}
...
GenericPDU WriteMultipleRegistersRequest::to_generic() const {
  ...
  if (register_data.size() != register_count * 2) {
    throw EncodingError("WriteMultipleRegistersRequest",
                        "Byte count does not match register count");
  }
...
void WriteMultipleRegistersResponse::from_generic(const GenericPDU& generic) {
  if (generic.function_code != 0x10) {
    throw DecodingError(generic, "WriteMultipleRegistersResponse",
                        "Invalid function code");
  }
