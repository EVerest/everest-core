...
std::vector<uint8_t> ModbusSocketTransport::read_bytes(size_t count) {
  ...
  while (read < count) {
    ...
    if (err < 0) {
      throw std::runtime_error("Failed to read bytes, got " +
                               std::to_string(count) +
                               " bytes, err: " + std::to_string(err) +
...
modbus_server::ModbusSocketTransport::try_read_bytes(size_t count) {
  struct pollfd pfd[1];
  ...
  auto error = errno;
  if (result_code < 0) {
    throw std::runtime_error("Failed to poll modbus data, errno: " +
                             std::to_string(error));
  }
...
void ModbusSocketTransport::write_bytes(const std::vector<uint8_t>& bytes) {
  int err = send(this->socket, bytes.data(), bytes.size(), 0);

  // todo: build custom errors for this kind of error
  if (err < 0) {
    throw std::runtime_error("Failed to write bytes, got " +
