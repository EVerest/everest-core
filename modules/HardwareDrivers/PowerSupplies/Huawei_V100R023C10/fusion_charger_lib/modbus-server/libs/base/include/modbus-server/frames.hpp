...
struct GenericPDU {
   ...
   * code
   */
  GenericPDU(const std::vector<uint8_t>& data_with_function_code);
   ...
   */
  GenericPDU(uint8_t function_code, const std::vector<uint8_t>& data);

  /**
   * @brief Convert the PDU to a raw data buffer, including the function code at
   * the beginning
   *
   * @return std::vector<uint8_t> the raw data buffer
   */
...
class DecodingError : public std::runtime_error {
 ...
 public:
  DecodingError(const GenericPDU&  original_data,
...
enum class PDUExceptionCode : uint8_t {
...
class ErrorPDU : public SpecificPDU {
 public:
  ...
  ErrorPDU();
  ErrorPDU(uint8_t function_code, uint8_t exception_code);
...
class PDUException : public std::exception {
 ...
 public:
  ...
  uint8_t get_exception_code() const;
