...
struct ReadHoldingRegistersResponse : public SpecificPDU {
    ...
    ReadHoldingRegistersResponse(const ReadHoldingRegistersRequest& req, std::vector<uint8_t> data) :
     ...
     *
     * @return std::vector<uint16_t> the register data from big-endian format
     * (converted to system endianness)
     */
    std::vector<uint16_t> get_register_data() const;
};
