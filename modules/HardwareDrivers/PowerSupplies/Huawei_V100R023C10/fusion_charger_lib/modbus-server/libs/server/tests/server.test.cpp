...
TEST(ModbusBasicServer, read_holding_registers_test) {
  ...
  server.set_read_holding_registers_request_cb(
      [&callback_called](const pdu::ReadHoldingRegistersRequest& request) {
        callback_called++;
  ...
  std::optional<pdu::GenericPDU> response =
      correlation_layer->call_on_pdu(req.to_generic());
...
TEST(ModbusBasicServer, write_single_register_test) {
  ...
  server.set_write_single_register_request_cb(
      [&callback_called](const pdu::WriteSingleRegisterRequest& request) {
        callback_called++;
...
TEST(ModbusBasicServer, not_serializable_response_sends_error_pdu) {
  ...
  server.set_read_holding_registers_request_cb(
      [&callback_called](const pdu::ReadHoldingRegistersRequest& request) {
        callback_called++;
        return pdu::ReadHoldingRegistersResponse(
            request, {0x01, 0x02,
                      0x03});  // not serializable because of missing 4th byte
      });
...
TEST(ModbusBasicServer, application_server_error_sends_error_pdu) {
  ...
  server.set_read_holding_registers_request_cb(
      [&callback_called](const pdu::ReadHoldingRegistersRequest& request) {
        callback_called++;
        throw ApplicationServerError(
            pdu::PDUExceptionCode::GATEWAY_PATH_UNAVAILABLE);
  ...
  ASSERT_NO_THROW(response_parsed.from_generic(response.value()));
  ASSERT_EQ(response_parsed.function_code, 0x03);
  ASSERT_EQ(response_parsed.exception_code,
            (uint8_t)pdu::PDUExceptionCode::GATEWAY_PATH_UNAVAILABLE);
