...
std::optional<pdu::GenericPDU> ModbusBasicServer::on_pdu_error_handled(const pdu::GenericPDU& input) {
    ...
    try {
        ...
        return pdu::ErrorPDU(input.function_code, (uint8_t)pdu::PDUExceptionCode::ILLEGAL_DATA_VALUE).to_generic();
    ...
    return pdu::ErrorPDU(input.function_code, (uint8_t)pdu::PDUExceptionCode::SERVER_DEVICE_FAILURE).to_generic();
}
...
std::optional<pdu::GenericPDU> ModbusBasicServer::on_pdu(const pdu::GenericPDU& input) {
    ...
    case 0x03: {
        if (!read_registers_request.has_value()) {
            return pdu::ErrorPDU(input.function_code, (uint8_t)pdu::PDUExceptionCode::ILLEGAL_FUNCTION).to_generic();
        }
