...
ModbusBasicServer::ModbusBasicServer(
    std::shared_ptr<PDUCorrelationLayerIntf> pas, logs::LogIntf log)
    : pas(pas), log(log) {
  pas->set_on_pdu(std::bind(&ModbusBasicServer::on_pdu_error_handled, this,
                            std::placeholders::_1));
}

void ModbusBasicServer::set_read_holding_registers_request_cb(
    AlwaysRespondingPDUHandler<pdu::ReadHoldingRegistersRequest,
                               pdu::ReadHoldingRegistersResponse> fn) {
  read_registers_request = fn;
}

void ModbusBasicServer::set_write_multiple_registers_request_cb(
    AlwaysRespondingPDUHandler<pdu::WriteMultipleRegistersRequest,
                               pdu::WriteMultipleRegistersResponse> fn) {
  write_multiple_registers_request = fn;
}

void ModbusBasicServer::set_write_single_register_request_cb(
    AlwaysRespondingPDUHandler<pdu::WriteSingleRegisterRequest,
                               pdu::WriteSingleRegisterResponse> fn) {
  write_single_register_request = fn;
}

std::optional<pdu::GenericPDU> ModbusBasicServer::on_pdu_error_handled(
    const pdu::GenericPDU& input) {
  ...
  try {
    ...
    return pdu::ErrorPDU(input.function_code,
                         (uint8_t)pdu::PDUExceptionCode::ILLEGAL_DATA_VALUE)
        .to_generic();
  ...
  return pdu::ErrorPDU(input.function_code,
                       (uint8_t)pdu::PDUExceptionCode::SERVER_DEVICE_FAILURE)
      .to_generic();
}

std::optional<pdu::GenericPDU> ModbusBasicServer::on_pdu(
    const pdu::GenericPDU& input) {
  switch (input.function_code) {
    case 0x03: {
      if (!read_registers_request.has_value()) {
        return pdu::ErrorPDU(input.function_code,
                             (uint8_t)pdu::PDUExceptionCode::ILLEGAL_FUNCTION)
            .to_generic();
      }
  ...
  log.verbose << "Server: Unknown function code: " +
                     std::to_string(input.function_code);
