...
class ComplexRegisterSubregistry {
 protected:
  std::vector<std::unique_ptr<complex_registers::ComplexRegisterUntyped>>
      registers;
...
class ComplexRegisterRegistry {
 ...
 public:
   ...
   */
  void on_write(uint16_t address, const std::vector<uint8_t>& data);
   ...
   * @return std::vector<uint8_t> The data read from the registers in big-endian
   * (can be converted to uint16 registers in big-endian order)
   */
  std::vector<uint8_t> on_read(uint16_t address, uint16_t size);
