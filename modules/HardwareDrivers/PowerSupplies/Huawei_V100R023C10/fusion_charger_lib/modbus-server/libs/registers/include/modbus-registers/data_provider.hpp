 ...
 */
template <typename T>
class DataProvider : public DataProviderUntyped {
 public:
  /**
   ...
   */
  virtual void on_read(uint8_t* value, size_t len) = 0;
  /**
   ...
   * @param len The length of the value, normally sizeof(T)
   */
  virtual void on_write(uint8_t* value, size_t len) = 0;
};
...
class DataProviderString : public DataProvider<const char*> {};
 ...
 */
template <size_t string_length>
class DataProviderStringHolding : public DataProviderString<string_length> {
 private:
 ...
 public:
  DataProviderStringHolding() { memset(value, 0, string_length); }
  ...
  void on_read(uint8_t* value, size_t len) override {
    memset(value, 0, len);
  ...
  void on_write(uint8_t* value, size_t len) override {
    strlcpy((char*)this->value, (char*)value,
            std::min(string_length + 1, len + 1));
    // todo: more tests
...
class DataProviderMemory : public DataProvider<const uint8_t*> {};
 ...
 */
template <size_t array_size>
class DataProviderMemoryHolding : public DataProviderMemory<array_size> {
 protected:
  uint8_t                                    value[array_size];
  std::vector<std::function<void(uint8_t*)>> callbacks;
 ...
 public:
  DataProviderMemoryHolding() { memset(value, 0, array_size); }
  DataProviderMemoryHolding(const uint8_t* value) {
    memcpy(this->value, value, array_size);
  }
  DataProviderMemoryHolding(const std::vector<uint8_t>& value) {
   ...
   * @param callback The callback to be called
   */
  void add_write_callback(std::function<void(uint8_t*)> callback) {
    callbacks.push_back(callback);
  }
   ...
   * @brief Get the current value
   *
   * @return uint8_t* The current value. Note that this pointer is pointing to
   * private memory
   */
  const uint8_t* get_value() { return value; }
   ...
   */
  size_t get_size() { return array_size; }
   ...
   * @param value The new value. Has to live past the function call.
   */
  void update_value(const uint8_t* value) {
    memcpy(this->value, value, array_size);
  }
};
 ...
 */
template <typename T>
class DataProviderHolding : public DataProvider<T> {
 ...
 public:
   ...
   */
  void update_value(T value) { this->value = value; }
};

template <typename T>
class DataProviderCallbacks : public DataProvider<T> {
 protected:
  std::function<T()>     read_callback;
  std::function<void(T)> write_callback;

 public:
  DataProviderCallbacks(std::function<T()>     read_callback,
                        std::function<void(T)> write_callback)
