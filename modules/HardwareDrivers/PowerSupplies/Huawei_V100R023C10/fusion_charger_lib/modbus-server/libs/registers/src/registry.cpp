...
void ComplexRegisterSubregistry::verify_internal_overlap() {
  ...
  for (auto& reg : registers) {
    for (uint16_t i = 0; i < reg->get_size(); ++i) {
      ...
      if (std::find(used_addresses.begin(), used_addresses.end(), address) !=
          used_addresses.end()) {
...
std::vector<ComplexRegisterUntyped*>
ComplexRegisterSubregistry::get_registers() {
...
void ComplexRegisterRegistry::on_write(uint16_t                    address,
  ...
  for (auto& reg : get_all_registers()) {
    ...
    if (address + data_size <= start_address ||
        address > end_address) {  // todo: check this condition
      continue;
    }
    ...
    if (vector_offset < 0) {
      auto data_offset =
          std::vector<uint8_t>(data.begin() - vector_offset * 2, data.end());
...
std::vector<uint8_t> ComplexRegisterRegistry::on_read(uint16_t address,
                                                      uint16_t size) {
  if (size == 0) {
    return {};
  }
  ...
  for (auto& reg : get_all_registers()) {
    ...
    if (read_start_address_b > data_start_address_b) {
      data.erase(data.begin(),
                 data.begin() + (read_start_address_b - data_start_address_b));
    ...
    if (read_start_address_b + read_size_b < data_end_address_b) {
      data.erase(data.begin() + (read_start_address_b + read_size_b -
                                 data_start_address_b),
                 data.end());
...
void ComplexRegisterRegistry::add(
    std::shared_ptr<ComplexRegisterSubregistry> registry) {
  this->registries.push_back(std::move(registry));
}

std::vector<ComplexRegisterUntyped*>
ComplexRegisterRegistry::get_all_registers() {
