...
class ComplexRegisterUntyped {
 ...
 public:
  ComplexRegisterUntyped(uint16_t start_address, uint16_t size);
   ...
   * uint16_t vector in big-endian order (0x1234 -> {0x12, 0x34}))
   */
  virtual void on_write(uint16_t offset, const std::vector<uint8_t>& data) = 0;

  /**
   * @brief Called when the register is (partially) read from, to be used by a
   *
   * @return std::vector<uint8_t> The complete register data in big-endian order
   * (can be converted to uint16_t vector in big-endian order ({0x12, 0x34} ->
   * 0x1234)). The length of the vector should always be double of \c size
   */
  virtual std::vector<uint8_t> on_read() = 0;

  /**
   * @brief Get the start address of the register in the modbus address space
   *
   * @return uint16_t The start address (0x0000 - 0xFFFF)
   */
   ...
   *
   * @return uint16_t The size of the register in 16-bit words
   */
 ...
 */
template <typename provider_type, size_t buffer_size>
class ComplexRegister : public ComplexRegisterUntyped {
 ...
 public:
  ComplexRegister(uint16_t                                     start_address,
                  data_providers::DataProvider<provider_type>& data_provider,
                  const converters::Converter&                 converter)
      : ComplexRegisterUntyped(start_address, buffer_size / 2),
        data_provider(data_provider),
        converter(converter) {}

  virtual ~ComplexRegister() = default;

  void on_write(uint16_t offset, const std::vector<uint8_t>& data) override {
...
class StringRegister : public ComplexRegister<const char*, string_length> {
 public:
  StringRegister(
      uint16_t                                           start_address,
      data_providers::DataProviderString<string_length>& data_provider,
      const converters::Converter&                       converter)
      : ComplexRegister<const char*, string_length>(
            start_address, data_provider, converter) {}
...
class MemoryRegister : public ComplexRegister<const uint8_t*, array_size> {
 public:
  MemoryRegister(uint16_t                                        start_address,
                 data_providers::DataProviderMemory<array_size>& data_provider,
                 const converters::Converter&                    converter)
      : ComplexRegister<const uint8_t*, array_size>(
            start_address, data_provider, converter) {}
 ...
 */
template <typename T>
class ElementaryRegister : public ComplexRegister<T, sizeof(T)> {
  static_assert(std::is_pointer<T>::value == false);

 public:
  ElementaryRegister(uint16_t                         start_address,
                     data_providers::DataProvider<T>& data_provider,
                     const converters::Converter&     converter)
