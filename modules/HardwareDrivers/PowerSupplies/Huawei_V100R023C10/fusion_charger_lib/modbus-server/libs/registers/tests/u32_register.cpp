...
TEST(ElementaryRegister, register_start_register_size) {
    ...
    DataProviderHolding<uint32_t> provider(0);
    ElementaryRegister<uint32_t> reg(0xc0de, provider, converter);

    EXPECT_EQ(reg.get_start_address(), 0xc0de);
    EXPECT_EQ(reg.get_size(), 2);

    DataProviderHolding<uint64_t> provider2(0);
    ElementaryRegister<uint64_t> reg2(0xdead, provider2, converter);
...
TEST(ElementaryRegister, u32_ABCD_write_exact) {
    DataProviderHolding<uint32_t> provider(0);
    ElementaryRegister<uint32_t> reg(0, provider, ConverterABCD::instance());
...
TEST(ElementaryRegister, i64_ABCD_write_exact) {
    DataProviderHolding<int64_t> provider(0);
    ElementaryRegister<int64_t> reg(0, provider, ConverterABCD::instance());
