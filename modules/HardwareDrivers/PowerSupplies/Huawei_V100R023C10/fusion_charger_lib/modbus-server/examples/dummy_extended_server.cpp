 ...
 public:
  void from_generic(const modbus_server::pdu::GenericPDU& input) override {
    if (input.function_code != 0x41) {
      throw modbus_server::pdu::DecodingError(input, "CustomPDURequest",
...
struct CustomPDUResponse : public modbus_server::pdu::SpecificPDU {
 ...
 public:
  void from_generic(const modbus_server::pdu::GenericPDU& input) override {
    if (input.function_code != 0x41) {
      throw modbus_server::pdu::DecodingError(input, "CustomPDUResponse",
                                              "Invalid function code");
    }
...
class CustomServer : public modbus_server::ModbusBasicServer {
 protected:
  std::optional<AlwaysRespondingPDUHandler<CustomPDURequest, CustomPDUResponse>>
      custom_pdu_cb;

 public:
  CustomServer(std::shared_ptr<modbus_server::PDUCorrelationLayerIntf> pal)
      : modbus_server::ModbusBasicServer(pal) {}

  void set_custom_pdu_cb(
      modbus_server::ModbusBasicServer::
          AlwaysRespondingPDUHandler<CustomPDURequest, CustomPDUResponse> fn) {
    custom_pdu_cb = fn;
  }

 protected:
  std::optional<modbus_server::pdu::GenericPDU> on_pdu(
      const modbus_server::pdu::GenericPDU& input) override {
...
int main() {
  ...
  while (true) {
    ...
    auto transport =
        std::make_shared<modbus_server::ModbusSocketTransport>(client_sock);
    auto protocol =
        std::make_shared<modbus_server::ModbusTCPProtocol>(transport);
