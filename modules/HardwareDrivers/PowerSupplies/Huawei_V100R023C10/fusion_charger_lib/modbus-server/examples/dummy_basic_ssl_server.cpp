...
int main() {
  ...
  SSL_CTX_use_certificate_file(ctx, "../certs/server.crt.pem",
                               SSL_FILETYPE_PEM);
  ...
  while (1) {
    ...
    modbus_server::ModbusBasicServer client =
        modbus_server::ModbusBasicServer(pas);

    client.set_read_holding_registers_request_cb(
        [](modbus_server::pdu::ReadHoldingRegistersRequest request) {
          std::vector<uint8_t> data(request.register_count * 2);

          for (size_t i = 0; i < request.register_count * 2; i++) {
            data[i] = i;
          }

          return modbus_server::pdu::ReadHoldingRegistersResponse(request,
                                                                  data);
        });
