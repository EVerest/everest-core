...
Sender::Sender(std::chrono::milliseconds t0,
...
void Sender::run() {
  ...
  if (!current_packet.has_value()) {
    ...
    current_packet_cv.wait(
        lock, [this] { return stop_flag || current_packet.has_value(); });
    ...
    current_packet_cv.wait_for(lock, wait_time,
                               [this] { return stop_flag || has_new_package; });
...
const uint8_t *Sender::get_mac_address() const {
