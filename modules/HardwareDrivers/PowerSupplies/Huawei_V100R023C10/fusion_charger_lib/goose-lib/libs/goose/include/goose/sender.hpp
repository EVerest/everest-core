...
class SendPacketIntf {
 public:
  ...
  virtual goose_ethernet::EthernetFrame build_packet(
      const PerPacketInfo &info) = 0;
};
...
class SendPacketNormal : public SendPacketIntf {
 ...
 public:
  ...
  goose_ethernet::EthernetFrame build_packet(
      const PerPacketInfo &info) override {
...
class SendPacketSecure : public SendPacketIntf {
 ...
 public:
  SendPacketSecure(goose::frame::SecureGooseFrame frame,
                   std::vector<uint8_t> hmac_key)
...
class Sender : public SenderIntf {
 protected:
  ...
  std::condition_variable
      current_packet_cv;  // Condition variable to notify the sender thread of a
 ...
 public:
   ...
   */
  Sender(std::chrono::milliseconds t0,
         std::vector<std::chrono::milliseconds> ts,
         std::shared_ptr<goose_ethernet::EthernetInterfaceIntf> intf,
         logs::LogIntf log = logs::log_printf);

  Sender(Sender &&other)
      : t0(other.t0), ts(other.ts), intf(other.intf), log(other.log) {}

  const uint8_t *get_mac_address() const;
