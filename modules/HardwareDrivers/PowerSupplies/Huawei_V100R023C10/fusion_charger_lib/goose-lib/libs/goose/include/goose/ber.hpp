...
template <typename T>
std::vector<uint8_t> encode_be(T value) {
...
template <typename T>
T decode_be(const std::vector<uint8_t>& input) {
  T result = 0;
...
struct BEREntry {
  ...
  BEREntry() = default;
  BEREntry(uint8_t tag, std::vector<uint8_t> value) : tag(tag), value(value) {}
   ...
   * @param input
   */
  BEREntry(std::vector<uint8_t>* input);
...
template <typename T>
struct PrimitiveBEREntry {
  T data;
  uint8_t tag;

  PrimitiveBEREntry() = default;
  PrimitiveBEREntry(T data, uint8_t tag) : data(data), tag(tag) {}
  /**
   * @brief Input-modifying decoding constructor; removes read bytes from input
   *
   * @warning This constructor modifies the input vector by removing the read
   ...
   */
  PrimitiveBEREntry(std::vector<uint8_t>& input,
                    std::optional<uint8_t> expected_tag = std::nullopt) {
    ...
    if (entry.value.size() > sizeof(T)) {
      throw std::runtime_error(
          "PrimitiveBEREntry: value size too big mismatch");
    }
...
struct StringBEREntry {
  ...
  StringBEREntry(const std::string& data, uint8_t tag) : data(data), tag(tag) {}
  /**
   * @brief Input-modifying decoding constructor; removes read bytes from input
   *
   * @warning This constructor modifies the input vector by removing the read
   ...
   * @param input BER encoded data
   */
  StringBEREntry(std::vector<uint8_t>& input,
                 std::optional<uint8_t> expected_tag,
  ...
  std::vector<uint8_t> encode() const {
    return BEREntry(tag, std::vector<uint8_t>(data.begin(), data.end()))
        .encode();
  }
};
