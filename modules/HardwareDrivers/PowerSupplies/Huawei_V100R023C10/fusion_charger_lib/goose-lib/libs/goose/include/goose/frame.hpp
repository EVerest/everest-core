...
const uint16_t GOOSE_ETHERTYPE = 0x88B8;
...
struct GooseTimestamp {
     ...
     */
    GooseTimestamp(uint32_t seconds, uint32_t fraction, uint8_t quality_of_time) :
        seconds(seconds), fraction(fraction), quality_of_time(quality_of_time) {
    }

    GooseTimestamp(const std::vector<uint8_t>& raw);

    std::vector<uint8_t> encode() const;
    float to_ms();
    bool operator==(const GooseTimestamp& other) const;

    static GooseTimestamp from_ms(uint64_t ms);
...
struct GoosePDU {
    ...
    uint32_t time_allowed_to_live; // seconds
    ...
    GoosePDU() = default;
    GoosePDU(const std::vector<uint8_t>& pdu);
...
public:
     ...
     * provided
     */
    SecureGooseFrame(const goose_ethernet::EthernetFrame& ethernet_frame, std::optional<std::vector<uint8_t>> hmac_key);
    ...
    goose_ethernet::EthernetFrame serialize(std::vector<uint8_t> hmac_key) const;
};
