...
TEST(SecureGooseFrame, real_world_example) {
  const char data_hex[] =
      "2c52afb6ed180080e11614028100A0C888B80001"
      ...
      "2ba088";
  uint8_t data[sizeof(data_hex) / 2];
...
TEST(SecureGooseFrame, real_world_example_invalid_crc) {
  ...
  goose_ethernet::EthernetFrame ethernet_frame(data, sizeof(data));
  ASSERT_THROW(goose::frame::SecureGooseFrame goose_frame(ethernet_frame),
               std::runtime_error);
}
...
TEST(SecureGooseFrame, encode_decode) {
  ...
  auto serialized =
      goose_frame.serialize(std::vector<uint8_t>(key, key + sizeof(key)));

  auto deserialized = goose::frame::SecureGooseFrame(
      serialized, std::vector<uint8_t>(key, key + sizeof(key)));
  ...
  ASSERT_EQ(deserialized.pdu.time_allowed_to_live,
            goose_frame.pdu.time_allowed_to_live);
...
TEST(SecureGooseFrame, deserialize_different_hmac_key) {
  ...
  // todo: custom exception for invalid hmac?
  serialized.payload[serialized.payload.size() - 1]++;
  ASSERT_THROW(goose::frame::SecureGooseFrame frame(
                   serialized, std::vector<uint8_t>(key, key + sizeof(key))),
               std::runtime_error);
}
