...
TEST(SecureGooseFrame, encode_decode) {
    ...
    auto serialized = goose_frame.serialize(std::vector<uint8_t>(key, key + sizeof(key)));

    auto deserialized = goose::frame::SecureGooseFrame(serialized, std::vector<uint8_t>(key, key + sizeof(key)));
...
TEST(SecureGooseFrame, deserialize_different_hmac_key) {
    ...
    ASSERT_THROW(goose::frame::SecureGooseFrame frame(serialized, std::vector<uint8_t>(key, key + sizeof(key))),
                 std::runtime_error);
}
