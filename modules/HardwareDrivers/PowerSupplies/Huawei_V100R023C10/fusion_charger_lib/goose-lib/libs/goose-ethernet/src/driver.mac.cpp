...
EthernetInterface::EthernetInterface(const char* interface_name) {
  ...
  for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
    ...
    if (strcmp(ifa->ifa_name, interface_name) == 0 &&
      ...
      uint8_t* mac_address = (unsigned char*)LLADDR(sdl);
  ...
  if (!found) {
    ...
    throw std::runtime_error(
        "cannot get interface address; interface not found");
  }

  // add filter to ignore outgoing packets (sent by us)
  char filter_exp[sizeof("not ether src XX:XX:XX:XX:XX:XX")];
  snprintf(filter_exp, sizeof(filter_exp),
           "not ether src %02X:%02X:%02X:%02X:%02X:%02X", this->mac_address[0],
           this->mac_address[1], this->mac_address[2], this->mac_address[3],
           this->mac_address[4], this->mac_address[5]);
...
void EthernetInterface::send_packet_raw(const uint8_t* packet, size_t size) {
  int ret = pcap_sendpacket(pcap, (uint8_t*)packet, size);
  if (ret != 0) {
...
std::vector<uint8_t> EthernetInterface::receive_packet_raw() {
  pcap_pkthdr* header;
  const uint8_t* data;
  while (1) {
    ...
    switch (ret) {
      case 1:
        return std::vector<uint8_t>(data, data + header->caplen);
      case 0:
        // timeout, try again
        continue;
      case -1:
        throw std::runtime_error("pcap_next_ex failed: " +
                                 std::string(pcap_geterr(pcap)));
      case -2:
...
EthernetInterface::~EthernetInterface() { pcap_close(pcap); }
