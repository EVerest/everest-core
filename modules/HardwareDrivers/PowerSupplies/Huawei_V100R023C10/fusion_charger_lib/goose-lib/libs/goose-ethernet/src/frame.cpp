...
EthernetFrame::EthernetFrame(const uint8_t* data, size_t size) {
  ...
  if (ethertype == 0x8100) {
    ...
    payload = std::vector<uint8_t>(data + 18, data + size);
  } else {
    eth_802_1q_tag = std::nullopt;
    payload = std::vector<uint8_t>(data + 14, data + size);
  }

  // todo: check if redundant
  if (payload.size() < 42) {
    throw DeserializeError(
        "Ethernet frame payload too short (payload size < 42)");
  }
}

EthernetFrame::EthernetFrame(const std::vector<uint8_t>& data)
    : EthernetFrame(data.data(), data.size()) {}
...
std::vector<uint8_t> EthernetFrame::serialize() const {
  ...
  size_t package_size =
      14 + payload.size() + (eth_802_1q_tag.has_value() ? 4 : 0);
...
bool EthernetFrame::ethertype_is_length() { return ethertype <= 1500; }
