...
EthernetInterface::EthernetInterface(const char *interface_name) {
  ...
  if (this->fd == -1) {
    throw std::runtime_error("Failed to open socket for ethernet interface: " +
  ...
  // setup filter for GOOSE packets
  {
    ...
    int ret = setsockopt(this->fd, SOL_SOCKET, SO_ATTACH_FILTER, &socket_filter,
                         sizeof(socket_filter));
    if (ret < 0) {
      throw std::runtime_error("Failed to set socket filter, errno: " +
                               std::to_string(errno));
    }
  }
...
std::optional<std::vector<uint8_t>> EthernetInterface::receive_packet_raw() {
  ...
  auto error = errno;
  if (result_code < 0) {
    throw std::runtime_error("Failed to poll ethernet frame, errno: " +
                             std::to_string(error));
  }
  ...
  if (ret == -1) {
    throw std::runtime_error("Failed to receive packet, errno: " +
                             std::to_string(errno));
  }
