...
PowerModuleStatus::PowerModuleStatus(const std::vector<uint8_t>& raw) {
    ...
    if (raw.size() >= 8) {
        ...
        if (data_type == WinlineProtocol::DATA_TYPE_INTEGER && error_code == WinlineProtocol::ERROR_NORMAL) {
            ...
            EVLOG_warning << "Winline: Invalid status response - DataType: 0x" << std::hex
                          << static_cast<int>(data_type) << ", ErrorCode: 0x" << static_cast<int>(error_code);
        }
    } else {
        EVLOG_warning << "Winline: PowerModuleStatus received insufficient data (size: " << raw.size() << ")";
    }
}
...
ReadVoltage::ReadVoltage(const std::vector<uint8_t>& raw) {
    if (raw.size() >= 8) {
        ...
        if (data_type == WinlineProtocol::DATA_TYPE_FLOAT && error_code == WinlineProtocol::ERROR_NORMAL) {
            ...
            EVLOG_warning << "Winline: Invalid voltage response - DataType: 0x" << std::hex
                          << static_cast<int>(data_type) << ", ErrorCode: 0x" << static_cast<int>(error_code);
        }
    }
}
...
ReadCurrent::ReadCurrent(const std::vector<uint8_t>& raw) {
    if (raw.size() >= 8) {
        ...
        if (data_type == WinlineProtocol::DATA_TYPE_FLOAT && error_code == WinlineProtocol::ERROR_NORMAL) {
            ...
            EVLOG_warning << "Winline: Invalid current response - DataType: 0x" << std::hex
                          << static_cast<int>(data_type) << ", ErrorCode: 0x" << static_cast<int>(error_code);
        }
    }
}
...
ReadGroupInfo::ReadGroupInfo(const std::vector<uint8_t>& raw) {
    if (raw.size() >= 8) {
        ...
        if (data_type == WinlineProtocol::DATA_TYPE_INTEGER && error_code == WinlineProtocol::ERROR_NORMAL) {
            ...
            EVLOG_warning << "Winline: Invalid group info response - DataType: 0x" << std::hex
                          << static_cast<int>(data_type) << ", ErrorCode: 0x" << static_cast<int>(error_code);
        }
    }
}
...
ReadRatedOutputPower::ReadRatedOutputPower(const std::vector<uint8_t>& raw) {
    if (raw.size() >= 8) {
        ...
        if (data_type == WinlineProtocol::DATA_TYPE_FLOAT && error_code == WinlineProtocol::ERROR_NORMAL) {
            ...
            EVLOG_warning << "Winline: Invalid power response - DataType: 0x" << std::hex << static_cast<int>(data_type)
...
ReadRatedOutputCurrent::ReadRatedOutputCurrent(const std::vector<uint8_t>& raw) {
    if (raw.size() >= 8) {
        ...
        if (data_type == WinlineProtocol::DATA_TYPE_FLOAT && error_code == WinlineProtocol::ERROR_NORMAL) {
            ...
            EVLOG_warning << "Winline: Invalid rated current response - DataType: 0x" << std::hex
                          << static_cast<int>(data_type) << ", ErrorCode: 0x" << static_cast<int>(error_code);
        }
    }
}
...
ReadCurrentLimitPoint::ReadCurrentLimitPoint(const std::vector<uint8_t>& raw) {
    if (raw.size() >= 8) {
        ...
        if (data_type == WinlineProtocol::DATA_TYPE_FLOAT && error_code == WinlineProtocol::ERROR_NORMAL) {
            ...
            EVLOG_warning << "Winline: Invalid current limit point response - DataType: 0x" << std::hex
                          << static_cast<int>(data_type) << ", ErrorCode: 0x" << static_cast<int>(error_code);
        }
    }
}
...
ReadDCBoardTemperature::ReadDCBoardTemperature(const std::vector<uint8_t>& raw) {
    if (raw.size() >= 8) {
        ...
        if (data_type == WinlineProtocol::DATA_TYPE_FLOAT && error_code == WinlineProtocol::ERROR_NORMAL) {
            ...
            EVLOG_warning << "Winline: Invalid DC board temperature response - DataType: 0x" << std::hex
                          << static_cast<int>(data_type) << ", ErrorCode: 0x" << static_cast<int>(error_code);
        }
    }
}
...
ReadAmbientTemperature::ReadAmbientTemperature(const std::vector<uint8_t>& raw) {
    if (raw.size() >= 8) {
        ...
        if (data_type == WinlineProtocol::DATA_TYPE_FLOAT && error_code == WinlineProtocol::ERROR_NORMAL) {
            ...
            EVLOG_warning << "Winline: Invalid ambient temperature response - DataType: 0x" << std::hex
                          << static_cast<int>(data_type) << ", ErrorCode: 0x" << static_cast<int>(error_code);
        }
    }
}
...
ReadDCDCVersion::ReadDCDCVersion(const std::vector<uint8_t>& raw) {
    if (raw.size() >= 8) {
        ...
        if (data_type == WinlineProtocol::DATA_TYPE_INTEGER && error_code == WinlineProtocol::ERROR_NORMAL) {
            ...
            EVLOG_warning << "Winline: Invalid DCDC version response - DataType: 0x" << std::hex
                          << static_cast<int>(data_type) << ", ErrorCode: 0x" << static_cast<int>(error_code);
        }
    }
}
...
ReadPFCVersion::ReadPFCVersion(const std::vector<uint8_t>& raw) {
    if (raw.size() >= 8) {
        ...
        if (data_type == WinlineProtocol::DATA_TYPE_INTEGER && error_code == WinlineProtocol::ERROR_NORMAL) {
            ...
            EVLOG_warning << "Winline: Invalid PFC version response - DataType: 0x" << std::hex
                          << static_cast<int>(data_type) << ", ErrorCode: 0x" << static_cast<int>(error_code);
        }
    }
}
