...
void powermeterImpl::set_device_time() {
    ...
    std::vector<uint8_t> slip_msg_set_device_time =
...
void powermeterImpl::set_device_charge_point_id(ast_app_layer::UserIdType id_type, std::string charge_point_id) {
    ...
    std::vector<uint8_t> slip_msg_set_charge_point_id =
        std::move(this->slip.package_single(this->config.powermeter_device_id, set_charge_point_id_cmd));
...
void powermeterImpl::read_device_data() {
    {
        ...
        std::vector<uint8_t> slip_msg_read_device_data = std::move(this->slip.package_multi(
    ...
    }
    {
        ...
        std::vector<uint8_t> slip_msg_read_device_data_2 = std::move(this->slip.package_multi(
            this->config.powermeter_device_id,
...
void powermeterImpl::get_device_public_key() {
    {
        ...
        std::vector<uint8_t> slip_msg_get_device_public_keys = std::move(this->slip.package_multi(
            this->config.powermeter_device_id,
            {get_device_public_key_cmd, get_device_public_key_asn1_cmd, get_device_public_key_str16_cmd}));
        ...
        // std::vector<uint8_t> slip_msg_get_device_public_key =
        // std::move(this->slip.package_single(this->config.powermeter_device_id, get_device_public_key_cmd));
...
void powermeterImpl::request_device_type() {
    ...
    std::vector<uint8_t> slip_msg_device_type =
...
void powermeterImpl::error_diagnostics(uint8_t addr) {
    ...
    std::vector<uint8_t> slip_msg_last_log_entry =
    ...
    std::vector<uint8_t> slip_msg_last_system_errors =
        std::move(this->slip.package_single(this->config.powermeter_device_id, last_system_errors_cmd));
    ...
    source_requested = ast_app_layer::ErrorSource::SYSTEM;
    std::vector<uint8_t> slip_msg_last_critical_system_errors =
        std::move(this->slip.package_single(this->config.powermeter_device_id, last_critical_system_errors_cmd));
    ...
    std::vector<uint8_t> slip_msg_last_communication_errors =
        std::move(this->slip.package_single(this->config.powermeter_device_id, last_comm_errors_cmd));
    ...
    category_requested = ast_app_layer::ErrorCategory::LAST_CRITICAL;
    source_requested = ast_app_layer::ErrorSource::COMMUNICATION;
    std::vector<uint8_t> slip_msg_last_critical_communication_errors =
        std::move(this->slip.package_single(this->config.powermeter_device_id, last_critical_comm_errors_cmd));
...
void powermeterImpl::read_diagnostics_data() {
    ...
    // part 1 - basic info
    {
        ...
        std::vector<uint8_t> slip_msg_get_diagnostics_data_1 = std::move(this->slip.package_multi(
            this->config.powermeter_device_id, {get_charge_point_id_cmd, get_device_type_cmd, get_hardware_version_cmd,
    ...
    // part 2 - log stats
    {
        ...
        std::vector<uint8_t> slip_msg_get_diagnostics_data_2 =
    ...
    // part 3 - HW/SW info
    {
        ...
        std::vector<uint8_t> slip_msg_get_diagnostics_data_3 = std::move(this->slip.package_multi(
            this->config.powermeter_device_id,
...
void powermeterImpl::readRegisters() {
    ...
    std::vector<uint8_t> slip_msg_read_registers = std::move(this->slip.package_multi(
        this->config.powermeter_device_id,
        {get_voltage_cmd, get_current_cmd, get_import_power_cmd, export_power_cmd, get_total_power_cmd}));
        ...
        if ((i + part_len - 1) <= response_size) {
            ...
            case (int)ast_app_layer::CommandType::GET_ERRORS: {
                ...
                for (uint8_t i = 0; i < 5; i++) {
                    j = i * 10;
                    logging_obj.source[(uint8_t)source_requested].category[(uint8_t)category_requested].error[i].id =
                        get_u32(part_data, j);
                    logging_obj.source[(uint8_t)source_requested]
                        .category[(uint8_t)category_requested]
                        .error[i]
                    ...
                    EVLOG_info << "Error #" << (int)i << " for source (" << (int)source_requested << ") and category ("
                               << (int)category_requested << "):\nID:      " << get_u32(part_data, j)
            ...
            case (int)ast_app_layer::CommandType::AB_HW_VERSION: {
                uint16_t delimiter_pos = 0;
                for (uint16_t i = 0; i < part_data_len; i++) {
                    if (part_data[i] == '|') {
                        delimiter_pos = i;
                    }
                }
...
ast_app_layer::CommandResult powermeterImpl::receive_response() {
    ...
    this->serial_device.rx(response, ast_app_layer::PM_AST_SERIAL_RX_INITIAL_TIMEOUT_MS,
...
powermeterImpl::handle_start_transaction(types::powermeter::TransactionReq& value) {
    ...
    app_layer.create_command_start_transaction(user_id_status, user_id_type, user_id_data,
                                               this->config.gmt_offset_quarter_hours, data_vect);
    std::vector<uint8_t> slip_msg_start_transaction =
        std::move(this->slip.package_single(this->config.powermeter_device_id, data_vect));
...
types::powermeter::TransactionStopResponse powermeterImpl::handle_stop_transaction(std::string& transaction_id) {
    ...
    std::vector<uint8_t> slip_msg_stop_transaction =
        std::move(this->slip.package_single(this->config.powermeter_device_id, data_vect));
...
std::string powermeterImpl::get_meter_ocmf() {
    ...
    std::vector<uint8_t> slip_msg_get_last_ocmf =
        std::move(this->slip.package_single(this->config.powermeter_device_id, data_vect));
