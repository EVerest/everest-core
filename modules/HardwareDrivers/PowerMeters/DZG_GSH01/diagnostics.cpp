...
void to_json(json& j, const Logging& k) {
    ...
    for (uint8_t n = 0; n < 5; n++) {
        j["errors"]["system"]["last"][n]["id"] = k.source[(uint8_t)app_layer::ErrorSource::SYSTEM]
                                                     .category[(uint8_t)app_layer::ErrorCategory::LAST]
                                                     .error[n]
                                                     .id;
        j["errors"]["system"]["last"][n]["priority"] = k.source[(uint8_t)app_layer::ErrorSource::SYSTEM]
                                                           .category[(uint8_t)app_layer::ErrorCategory::LAST]
                                                           .error[n]
                                                           .priority;
        j["errors"]["system"]["last"][n]["counter"] = k.source[(uint8_t)app_layer::ErrorSource::SYSTEM]
                                                          .category[(uint8_t)app_layer::ErrorCategory::LAST]
                                                          .error[n]
                                                          .counter;
    }
    ...
    for (uint8_t n = 0; n < 5; n++) {
        j["errors"]["system"]["last_critical"][n]["id"] =
            k.source[(uint8_t)app_layer::ErrorSource::SYSTEM]
                .category[(uint8_t)app_layer::ErrorCategory::LAST_CRITICAL]
                .error[n]
                .id;
    ...
    for (uint8_t n = 0; n < 5; n++) {
        j["errors"]["communication"]["last"][n]["id"] = k.source[(uint8_t)app_layer::ErrorSource::COMMUNICATION]
                                                            .category[(uint8_t)app_layer::ErrorCategory::LAST]
                                                            .error[n]
                                                            .id;
        j["errors"]["communication"]["last"][n]["priority"] = k.source[(uint8_t)app_layer::ErrorSource::COMMUNICATION]
                                                                  .category[(uint8_t)app_layer::ErrorCategory::LAST]
                                                                  .error[n]
                                                                  .priority;
        j["errors"]["communication"]["last"][n]["counter"] = k.source[(uint8_t)app_layer::ErrorSource::COMMUNICATION]
                                                                 .category[(uint8_t)app_layer::ErrorCategory::LAST]
                                                                 .error[n]
                                                                 .counter;
    }
    ...
    for (uint8_t n = 0; n < 5; n++) {
        j["errors"]["communication"]["last_critical"][n]["id"] =
            k.source[(uint8_t)app_layer::ErrorSource::COMMUNICATION]
                .category[(uint8_t)app_layer::ErrorCategory::LAST_CRITICAL]
                .error[n]
                .id;
