...
*/
int SerialDevice::rx(std::vector<uint8_t>& rxbuf, std::optional<int> initial_timeout_ms,
    ...
    while (true) {
        ...
        if (rv == -1) {                           // error in select function call
            ...
            if (bytes_read_total >= rxbuf.capacity()) {
                ...
                EVLOG_info
                    << "No buffer space left, but more to read. (Did you mean to set \"ignore_echo\" to \"false\"?)";
                break;
            }

            rxbuf.resize(rxbuf.capacity());
            int bytes_read =
                read(this->fd, (uint8_t*)(&rxbuf[0] + bytes_read_total), (size_t)(rxbuf.capacity() - bytes_read_total));
...
int SerialDevice::tx_rx_blocking(std::vector<uint8_t>& request, std::vector<uint8_t>& rxbuf,
