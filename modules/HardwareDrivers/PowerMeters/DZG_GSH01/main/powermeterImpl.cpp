...
inline uint16_t get_u16(std::vector<uint8_t>& vec, uint8_t start_index) {
    if (vec.size() < start_index + 1)
        return 0;
    return std::move(((uint16_t)vec[start_index + 1] << 8) | (uint16_t)vec[start_index]);
}
...
inline uint32_t get_u32(std::vector<uint8_t>& vec, uint8_t start_index) {
    if (vec.size() < start_index + 3)
        return 0;
    return std::move(((uint32_t)vec[start_index + 3] << 24) | ((uint32_t)vec[start_index + 2] << 16) |
                     ((uint32_t)vec[start_index + 1] << 8) | (uint32_t)vec[start_index]);
}
...
inline uint64_t get_u64(std::vector<uint8_t>& vec, uint8_t start_index) {
    if (vec.size() < start_index + 7)
        return 0;
    return std::move(((uint64_t)vec[start_index + 7] << 56) | ((uint64_t)vec[start_index + 6] << 48) |
                     ((uint64_t)vec[start_index + 5] << 40) | ((uint64_t)vec[start_index + 4] << 32) |
                     ((uint64_t)vec[start_index + 3] << 24) | ((uint64_t)vec[start_index + 2] << 16) |
                     ((uint64_t)vec[start_index + 1] << 8) | (uint64_t)vec[start_index]);
}
...
void powermeterImpl::ready() {
    ...
    while (!connected) {
        connected = this->serial_device.open_device(config.serial_port, config.baudrate, config.ignore_echo,
                                                    config.num_of_retries);
        if (!connected &&
            !(this->error_state_monitor->is_error_active("powermeter/CommunicationFault", "Communication timed out"))) {
            this->communication_timeout = true;
            EVLOG_error << "Failed to communicate with the powermeter due to serial port : " << config.serial_port
                        << " errors.";
            auto error = this->error_factory->create_error("powermeter/CommunicationFault", "Communication timed out",
    ...
    this->communication_timeout = false;
    ...
    const uint64_t CHARGING_STATUS = 0x20000; // according to documentation it is the bit 17th
    ...
    if (this->config.publish_device_data) {
        std::thread([this] {
            while (true) {
                get_status_word();
                if (this->device_data_obj.ab_status & CHARGING_STATUS) {
                    this->no_charging_done = false;
                    this->charging_in_progress = true;
                } else {
                    this->charging_in_progress = false;
                }
                read_device_data();
                std::this_thread::sleep_for(std::chrono::seconds(1));
...
void powermeterImpl::time_sync() {
    ...
    uint64_t diff =
        abs((uint32_t)std::chrono::duration_cast<std::chrono::seconds>(timepoint.time_since_epoch()).count() -
...
void powermeterImpl::get_device_time() {
    ...
    std::vector<uint8_t> slip_msg = std::move(this->slip.package_single(this->config.powermeter_device_id, data_vect));
...
void powermeterImpl::set_device_time() {
    ...
    std::vector<uint8_t> slip_msg =
        std::move(this->slip.package_single(this->config.powermeter_device_id, set_device_time_cmd));
    this->send_receive(slip_msg);
}
...
void powermeterImpl::get_meter_bus_address() {
    ...
    std::vector<uint8_t> slip_msg = std::move(this->slip.package_single(0xFF, data_vect));
    this->send_receive(slip_msg);
}
...
void powermeterImpl::set_meter_bus_address(uint8_t old_bus_address, uint8_t new_bus_address) {
    ...
    std::vector<uint8_t> slip_msg = std::move(this->slip.package_single(old_bus_address, set_meter_bus_address_cmd));
    this->send_receive(slip_msg);
}
...
void powermeterImpl::read_device_data() {
    {
        ...
        std::vector<uint8_t> slip_msg;
        if (this->no_charging_done) {
            slip_msg = std::move(this->slip.package_multi(this->config.powermeter_device_id,
                                                          {get_time_cmd,
        ...
        } else if (this->charging_in_progress) {
            slip_msg = std::move(this->slip.package_multi(this->config.powermeter_device_id,
                                                          {get_time_cmd, get_total_start_import_energy_cmd,
                                                           // get_total_stop_import_energy_cmd,
                                                           get_total_transaction_duration_cmd, get_ocmf_stats_cmd,
                                                           get_last_transaction_ocmf_cmd}));
        } else {
            slip_msg = std::move(this->slip.package_multi(
...
void powermeterImpl::set_application_operation_mode(app_layer::ApplicationBoardMode mode) {
    ...
    std::vector<uint8_t> slip_msg =
        std::move(this->slip.package_single(this->config.powermeter_device_id, set_operation_mode_cmd));
    this->send_receive(slip_msg);
}
...
void powermeterImpl::set_line_loss_impedance(uint16_t ll_impedance) {
    ...
    std::vector<uint8_t> slip_msg =
        std::move(this->slip.package_single(this->config.powermeter_device_id, set_ll_impedance_cmd));
    this->send_receive(slip_msg);
}
...
void powermeterImpl::error_diagnostics(uint8_t addr) {
    ...
    std::vector<uint8_t> slip_msg =
        std::move(this->slip.package_single(this->config.powermeter_device_id, last_log_entry_cmd));
    this->send_receive(slip_msg);
}
...
void powermeterImpl::read_diagnostics_data() {
    // part 1 - basic info
    {
        ...
        std::vector<uint8_t> slip_msg = std::move(
            this->slip.package_multi(this->config.powermeter_device_id,
    ...
    // part 2 - log stats
    {
        ...
        std::vector<uint8_t> slip_msg =
            std::move(this->slip.package_single(this->config.powermeter_device_id, get_log_stats_cmd));
        this->send_receive(slip_msg);
    }
    ...
    // part 3 - application/metering info
    {
        ...
        std::vector<uint8_t> slip_msg = std::move(this->slip.package_multi(
            this->config.powermeter_device_id,
            {get_application_fw_version_cmd, get_application_fw_checksum_cmd, get_application_fw_hash_cmd,
             get_application_mode_cmd, get_metering_fw_version_cmd, get_metering_fw_checksum_cmd,
...
void powermeterImpl::readRegisters() {
    ...
    std::vector<uint8_t> slip_msg = std::move(
        this->slip.package_multi(this->config.powermeter_device_id, {get_time_cmd, get_voltage_cmd, get_current_cmd,
...
app_layer::CommandResult powermeterImpl::process_response(const std::vector<uint8_t>& response_message) {
    ...
    while ((i + 4) <= response_size) {
        ...
        if ((i + part_len - 1) <= response_size) {
            ...
            case (int)app_layer::CommandType::APP_MODE: {
                ...
                if (mode == (uint8_t)app_layer::ApplicationBoardMode::ASSEMBLY) {
                    device_diagnostics_obj.dev_info.application.mode = "Assembly";
                } else if (mode == (uint8_t)app_layer::ApplicationBoardMode::APPLICATION) {
...
app_layer::CommandResult powermeterImpl::handle_response(std::vector<uint8_t>& response) {
    app_layer::CommandResult retval = app_layer::CommandResult::OK;

    if (response.size() >= 5) {
        app_layer::CommandResult result{};
        auto ret = this->slip.unpack(response, config.powermeter_device_id);
...
void powermeterImpl::send_receive(std::vector<uint8_t>& request) {
    ...
    while (app_layer::CommandResult::TIMEOUT == result) {
        ...
        this->serial_device.tx_rx_blocking(request, response, app_layer::PM_GSH01_SERIAL_RX_INITIAL_TIMEOUT_MS,
...
powermeterImpl::handle_start_transaction(types::powermeter::TransactionReq& value) {
    ...
    if (false == this->communication_timeout) {
        time_sync();
        cleanup_dangling_transaction();

        this->start_transact_result = app_layer::CommandResult::PENDING;
        ...
        std::vector<uint8_t> slip_msg =
            std::move(this->slip.package_single(this->config.powermeter_device_id, data_vect));

        this->start_transaction_msg_status = MessageStatus::SENT;
        this->send_receive(slip_msg);

        if (this->start_transaction_msg_status != MessageStatus::RECEIVED) {
            this->start_transact_result = app_layer::CommandResult::TIMEOUT;
        }

        if (this->start_transact_result != app_layer::CommandResult::OK) {
...
types::powermeter::TransactionStopResponse powermeterImpl::do_stop_transaction(const std::string& transaction_id) {
    ...
    this->stop_transact_result = app_layer::CommandResult::PENDING;
    ...
    if (this->stop_transaction_msg_status != MessageStatus::RECEIVED) {
        this->stop_transact_result = app_layer::CommandResult::TIMEOUT;
    }

    if (this->stop_transact_result != app_layer::CommandResult::OK) {
        r.status = types::powermeter::TransactionRequestStatus::UNEXPECTED_ERROR;
    } else {
...
std::string powermeterImpl::get_meter_ocmf() {
    ...
    if (this->get_transaction_values_msg_status != MessageStatus::RECEIVED) {
        this->stop_transact_result = app_layer::CommandResult::TIMEOUT;
    }

    if (this->stop_transact_result != app_layer::CommandResult::OK) {
        std::stringstream ss;
        ss << "Error: command failed with code " << int(this->stop_transact_result) << ". ("
           << command_result_to_string(this->stop_transact_result) << ")";
        return ss.str();
    }
