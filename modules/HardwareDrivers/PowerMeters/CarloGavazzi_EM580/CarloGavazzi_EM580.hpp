...
public:
  ...
  CarloGavazzi_EM580(const ModuleInfo &info,
                     std::unique_ptr<powermeterImplBase> p_main,
                     std::unique_ptr<serial_communication_hubIntf> r_modbus,
                     Conf &config)
      : ModuleBase(info), p_main(std::move(p_main)),
        r_modbus(std::move(r_modbus)), config(config){};
