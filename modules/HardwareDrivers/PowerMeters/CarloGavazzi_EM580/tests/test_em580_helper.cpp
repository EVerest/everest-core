...
TEST(EM580Helper, MaxPayloadBytesForWords) {
    ...
    EXPECT_EQ(modbus_utils::max_payload_bytes_for_words(1), 1);     // 2 bytes total, reserve NUL => 1
    EXPECT_EQ(modbus_utils::max_payload_bytes_for_words(126), 251); // 252 bytes total, reserve NUL => 251
}
...
TEST(EM580Helper, DecodeDeviceStateErrorsReturnsMatchingMessages) {
    // Bits 0 and 13 correspond to V1N over-range and Measure module internal fault.
    const auto state = static_cast<uint16_t>((1U << 0U) | (1U << 13U));
...
TEST(EM580Helper, OcmfWaitForReadyTimesOutAfterMaxRetries) {
    ...
    // kNonReadyReads non-ready reads => retries becomes kNonReadyReads and (retries > max_retries(10)) => false
#pragma unroll
