...
public:
  void push_fetch_response(std::int32_t address, std::uint16_t register_count,
                           transport::DataVector response) {
    scripted_fetch_[Key{address, register_count}].push_back(
        std::move(response));
  }

  const std::vector<FetchCall> &fetch_calls() const { return fetch_calls_; }

  const std::vector<WriteCall> &write_calls() const { return write_calls_; }

  transport::DataVector fetch(std::int32_t address,
                              std::uint16_t register_count) override {
    ...
    if (iter == scripted_fetch_.end() || iter->second.empty()) {
      throw std::runtime_error(
          "FakeModbusTransport: no scripted fetch response for address/count");
    }
  ...
  void
  write_multiple_registers(std::int32_t address,
                           const std::vector<std::uint16_t> &data) override {
...
transport::DataVector u16_be(std::uint16_t value) {
  ...
  const auto high = static_cast<std::uint8_t>(
      (static_cast<std::uint32_t>(value) >> kByteBits) & kByteMask);
  const auto low =
      static_cast<std::uint8_t>(static_cast<std::uint32_t>(value) & kByteMask);
...
TEST(EM580PowermeterImpl, StartTransactionHappyPathCountsWrites) {
  ...
  transport->push_fetch_response(
      em580::registers::MODBUS_OCMF_STATE_ADDRESS, 1,
      u16_be(em580::registers::MODBUS_OCMF_STATE_NOT_READY));

  auto *transport_ptr = transport.get();
  module::main::powermeterImpl::TestAccess::set_modbus_transport(
      impl, std::move(transport));
  ...
  const auto resp =
      module::main::powermeterImpl::TestAccess::start_transaction(impl, req);
...
TEST(EM580PowermeterImpl,
     StopTransactionPendingClosedTransactionMismatchReturnsError) {
  static Everest::PtrContainer<module::CarloGavazzi_EM580> dummy_mod;
  auto conf = make_test_conf();
  module::main::powermeterImpl impl(nullptr, dummy_mod, conf);
  ...
  transport->push_fetch_response(
      em580::registers::MODBUS_OCMF_STATE_SIZE_ADDRESS, 1, u16_be(1));
...
{"SA":"ECDSA-brainpoolP384r1-SHA256","SD":"00"})";
  transport->push_fetch_response(
      em580::registers::MODBUS_OCMF_STATE_FILE_ADDRESS, 1, bytes(ocmf_file));

  auto *transport_ptr = transport.get();
  module::main::powermeterImpl::TestAccess::set_modbus_transport(
      impl, std::move(transport));
  module::main::powermeterImpl::TestAccess::set_pending_closed_transaction(
      impl, true);

  std::string requested_uuid = "12345678-1234-5678-1234-567812345678";
  const auto resp = module::main::powermeterImpl::TestAccess::stop_transaction(
      impl, requested_uuid);
  EXPECT_EQ(resp.status,
            types::powermeter::TransactionRequestStatus::UNEXPECTED_ERROR);
...
TEST(
    EM580PowermeterImpl,
    StopTransactionEmptyIdWithoutPendingClosedTransactionCleansUpAndReturnsOk) {
  static Everest::PtrContainer<module::CarloGavazzi_EM580> dummy_mod;
  auto conf = make_test_conf();
  module::main::powermeterImpl impl(nullptr, dummy_mod, conf);
  ...
  module::main::powermeterImpl::TestAccess::set_modbus_transport(
      impl, std::move(transport));
  module::main::powermeterImpl::TestAccess::set_transaction_id(
      impl, "12345678-1234-5678-1234-567812345678");
  module::main::powermeterImpl::TestAccess::set_pending_closed_transaction(
      impl, false);
...
TEST(EM580PowermeterImpl,
     StartTransactionSpuriousReadyStateDoesCleanupAndNoTransactionWrites) {
  static Everest::PtrContainer<module::CarloGavazzi_EM580> dummy_mod;
  auto conf = make_test_conf();
  module::main::powermeterImpl impl(nullptr, dummy_mod, conf);
  ...
  transport->push_fetch_response(
      em580::registers::MODBUS_OCMF_STATE_SIZE_ADDRESS, 1, u16_be(1));
  transport->push_fetch_response(
      em580::registers::MODBUS_OCMF_STATE_FILE_ADDRESS, 1,
  ...
  // writes).
  EXPECT_EQ(transport_ptr->write_calls().size(), 1U);
  EXPECT_EQ(transport_ptr->write_calls()[0].address,
            em580::registers::MODBUS_OCMF_STATE_ADDRESS);
}
...
TEST(EM580PowermeterImpl, StopTransactionUnknownIdReturnsErrorAndNoWrites) {
  ...
  module::main::powermeterImpl::TestAccess::set_pending_closed_transaction(
      impl, false);
  module::main::powermeterImpl::TestAccess::set_transaction_id(
      impl, "11111111-1111-1111-1111-111111111111");
