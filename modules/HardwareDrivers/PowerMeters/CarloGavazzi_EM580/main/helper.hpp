...
constexpr int MODBUS_BASE_ADDRESS = 300001;

constexpr int MODBUS_SIGNATURE_TYPE_ADDRESS = 309472; // 24FFh: Signature type (UINT16)
constexpr int MODBUS_PUBLIC_KEY_ADDRESS = 309473;     // 2500h: Public key (UINT16[130])
// DER formatted public key (Table 4.20/4.21), mandatory to read whole block from 2600h.
constexpr int MODBUS_PUBLIC_KEY_DER_ADDRESS = 309729;    // 2600h: Public key DER (read-only)
constexpr int MODBUS_PUBLIC_KEY_DER_WORD_COUNT_256 = 46; // 2600h..262Dh (92 bytes, DER length 0x5A + 2)
constexpr int MODBUS_PUBLIC_KEY_DER_WORD_COUNT_384 = 62; // 2600h..263Dh (124 bytes, DER length 0x7A + 2)

constexpr int MODBUS_SIGNED_MAP_ADDRESS = 302049;
constexpr int MODBUS_SIGNED_MAP_SIGNATURE_ADDRESS = 302126;

constexpr int MODBUS_REAL_TIME_VALUES_ADDRESS = 300001;
constexpr int MODBUS_REAL_TIME_VALUES_COUNT = 80; // Registers 300001-300080 (0x50 = 80 words)

constexpr int MODBUS_TEMPERATURE_ADDRESS = 300776; // Internal Temperature

constexpr int MODBUS_FIRMWARE_MEASURE_MODULE_ADDRESS = 300771;       // Measure module firmware version/revision
constexpr int MODBUS_FIRMWARE_COMMUNICATION_MODULE_ADDRESS = 300772; // Communication module firmware version/revision
constexpr int MODBUS_SERIAL_NUMBER_START_ADDRESS = 320481;           // Serial number (7 registers: 320481-320487)
constexpr int MODBUS_SERIAL_NUMBER_REGISTER_COUNT = 7;               // 7 UINT16 registers = 14 bytes
constexpr int MODBUS_PRODUCTION_YEAR_ADDRESS = 320488;               // Production year (1 UINT16 register)

// Device state register (Table 4.30, Section 4.3.6)
constexpr int MODBUS_DEVICE_STATE_ADDRESS = 320499; // 5012h: Device state (UINT16 bitfield)

// Time synchronization registers
constexpr int MODBUS_UTC_TIMESTAMP_ADDRESS = 328723;   // UTC Timestamp for synchronization (INT64, 4 words)
constexpr int MODBUS_TIMEZONE_OFFSET_ADDRESS = 328722; // Local time delta in minutes (INT16, 1 word)

// OCMF Transaction registers (Table 4.34)
constexpr int MODBUS_OCMF_IDENTIFICATION_STATUS_ADDRESS = 328673;      // 7000h: OCMF Ident. Status (UINT16)
constexpr int MODBUS_OCMF_IDENTIFICATION_LEVEL_ADDRESS = 328674;       // 7001h: OCMF Ident. Level (UINT16)
constexpr int MODBUS_OCMF_IDENTIFICATION_FLAGS_START_ADDRESS = 328675; // 7002h: OCMF Ident. Flags 1-4 (4 UINT16)
constexpr int MODBUS_OCMF_IDENTIFICATION_FLAGS_COUNT = 4;              // 4 flags
constexpr int MODBUS_OCMF_IDENTIFICATION_TYPE_ADDRESS = 328679;        // 7006h: OCMF Ident. Type (UINT16)
constexpr int MODBUS_OCMF_IDENTIFICATION_DATA_START_ADDRESS = 328680;  // 7007h: OCMF Ident. Data (CHAR[40] = 20 words)
constexpr int MODBUS_OCMF_IDENTIFICATION_DATA_WORD_COUNT = 20;         // 40 bytes = 20 words
constexpr int MODBUS_OCMF_CHARGING_POINT_ID_TYPE_ADDRESS =
    328700; // 701Bh: OCMF Charging point identifier type (UINT16)
constexpr int MODBUS_OCMF_CHARGING_POINT_ID_START_ADDRESS = 328701;   // 701Ch: OCMF CPI (CHAR[40] = 20 words)
constexpr int MODBUS_OCMF_CHARGING_POINT_ID_WORD_COUNT = 20;          // 40 bytes = 20 words
constexpr int MODBUS_OCMF_SESSION_MODALITY_ADDRESS = 328727;          // 7036h: OCMF Session Modality (UINT16)
constexpr uint16_t MODBUS_OCMF_SESSION_MODALITY_CHARGING_VEHICLE = 0; // Charging vehicle
constexpr uint16_t MODBUS_OCMF_SESSION_MODALITY_VEHICLE_TO_GRID = 1;  // Vehicle to grid
constexpr uint16_t MODBUS_OCMF_SESSION_MODALITY_BIDIRECTIONAL = 2;    // Bidirectional

// Tariff text register (Table 4.32)
// 326881 (6900h): Tariff text (CHAR[252] = 126 words)
constexpr int MODBUS_OCMF_TARIFF_TEXT_ADDRESS = 326881;               // 6900h: Tariff text (CHAR[252] = 126 words)
constexpr int MODBUS_OCMF_TARIFF_TEXT_WORD_COUNT = 126;               // 252 bytes = 126 words (CHAR[252])
constexpr int MODBUS_OCMF_TRANSACTION_ID_GENERATION_ADDRESS = 328417; // 6F00h: OCMF Transaction ID Generation (UINT16)

// Tariff update register (Table 4.33)
constexpr int MODBUS_OCMF_TARIFF_UPDATE_ADDRESS = 327085; // 69CCh: Tariff update (UINT16)

// OCMF Command register (Table 4.35)
// The register is UINT16 containing the ASCII code (e.g. 'B', 'E', 'A').
constexpr int MODBUS_OCMF_COMMAND_ADDRESS = 328737;  // 7040h: OCMF Command Data (UINT16)
constexpr uint16_t MODBUS_OCMF_COMMAND_START = 0x42; // Start transaction ('B')
constexpr uint16_t MODBUS_OCMF_COMMAND_END = 0x45;   // End transaction ('E')
constexpr uint16_t MODBUS_OCMF_COMMAND_ABORT = 0x41; // Abort transaction ('A')

// OCMF State / status registers (Table 4.39 and related)
constexpr int MODBUS_OCMF_STATE_ADDRESS = 328929;               // 7100h: OCMF State (UINT16)
constexpr int MODBUS_OCMF_TRANSACTION_ID_ADDRESS = 328931;      // 7102h: OCMF Transaction ID (UINT32)
constexpr uint16_t MODBUS_OCMF_STATE_NOT_READY = 0;             // Not ready
constexpr uint16_t MODBUS_OCMF_STATE_RUNNING = 1;               // Running
constexpr uint16_t MODBUS_OCMF_STATE_READY = 2;                 // Ready
constexpr uint16_t MODBUS_OCMF_STATE_CORRUPTED = 3;             // Corrupted
constexpr int MODBUS_OCMF_STATE_SIZE_ADDRESS = 328930;          // 7101h: OCMF Size (UINT16)
constexpr int MODBUS_OCMF_STATE_FILE_ADDRESS = 328945;          // 7110h: OCMF File (max theoretically 2031 words)
constexpr int MODBUS_OCMF_STATE_FILE_WORD_COUNT = 2031;         // 2031 words = 4062 bytes
constexpr int MODBUS_OCMF_CHARGING_STATUS_ADDRESS = 328742;     // 7045h: Charging status (UINT16)
constexpr int MODBUS_OCMF_LAST_TRANSACTION_ID_ADDRESS = 328762; // 7059h: Last transaction id (CHAR[])
constexpr int MODBUS_OCMF_LAST_TRANSACTION_ID_WORD_COUNT = 14;  // 14 bytes = 7 words
constexpr int MODBUS_OCMF_TIME_SYNC_STATUS_ADDRESS = 328769;    // 7060h: Time synchronization status (UINT16)
...
/// Converts a string to a big-endian Modbus CHAR array (vector of UINT16 words) that is **0-terminated**
/// and contains only the **used** part (i.e. no full fixed-length padding).
///
/// - Max capacity is `max_words * 2` bytes.
/// - Ensures a terminating `\\0` byte is present within the returned data.
/// - If `str` is too long, it is truncated to fit `max_words * 2 - 1` bytes (+ 1 byte terminator).
inline std::vector<uint16_t> string_to_modbus_char_array(const std::string& str, std::size_t max_words) {
    ...
    std::vector<uint16_t> data(words_to_write, 0);
...
/// Confirm OCMF file read by writing NOT_READY (0) into the OCMF state register.
inline void confirm_file_read(transport::AbstractModbusTransport& modbus_transport) {
    std::vector<uint16_t> ocmf_confirmation_data = {em580::registers::MODBUS_OCMF_STATE_NOT_READY};
...
inline bool wait_for_ready(transport::AbstractModbusTransport& modbus_transport,
                           ...
                           int max_retries = 10) {
    uint16_t state = em580::registers::MODBUS_OCMF_STATE_NOT_READY;
...
inline std::vector<std::string> decode_device_state_errors(uint16_t device_state) {
    struct BitError {
        const char* message;
        uint16_t bit;
    };
    ...
    for (const auto& err : errors) {
        if ((device_state & static_cast<uint16_t>(1U << err.bit)) != 0U) {
