...
transport::DataVector
SerialCommHubTransport::fetch(std::int32_t address,
                              std::uint16_t register_count) {
  return retry_with_config([this, address, register_count]() {
    transport::DataVector response;
    response.reserve(static_cast<std::size_t>(register_count) *
                     2U); // this is a uint8_t vector
    ...
    while (remaining_register_to_read > 0) {
      const std::uint16_t register_to_read =
          remaining_register_to_read > MAX_READ_REGISTERS_PER_MESSAGE
              ? MAX_READ_REGISTERS_PER_MESSAGE
              : remaining_register_to_read;

      types::serial_comm_hub_requests::Result serial_com_hub_result =
          m_serial_hub.call_modbus_read_input_registers(
              static_cast<int>(m_device_id), static_cast<int>(read_address),
              register_to_read);

      // Check for communication errors
      if (serial_com_hub_result.status_code ==
          types::serial_comm_hub_requests::StatusCodeEnum::Timeout) {
        throw transport::ModbusTimeoutException(
            "Modbus read timeout: Packet receive timeout");
      } else if (serial_com_hub_result.status_code !=
      ...
      static_assert(
          std::is_same_v<std::int32_t,
                         decltype(types::serial_comm_hub_requests::Result::
...
void SerialCommHubTransport::write_multiple_registers(
    std::int32_t address, const std::vector<std::uint16_t> &data) {
  retry_with_config_void([this, address, &data]() {
    ...
    while (offset < data.size()) {
      ...
      const std::size_t chunk_size =
          remaining > static_cast<std::size_t>(MAX_WRITE_REGISTERS_PER_MESSAGE)
      ...
      types::serial_comm_hub_requests::StatusCodeEnum status =
          m_serial_hub.call_modbus_write_multiple_registers(
              static_cast<int>(m_device_id),
              static_cast<int>(write_address +
                               static_cast<std::int32_t>(offset)),
              data_raw);

      if (status == types::serial_comm_hub_requests::StatusCodeEnum::Timeout) {
        throw transport::ModbusTimeoutException(
            "Modbus write timeout: Packet receive timeout");
      } else if (status !=
                 types::serial_comm_hub_requests::StatusCodeEnum::Success) {
        std::string error_msg =
