...
public:
  explicit ModbusTimeoutException(const std::string &message)
...
public:
  ...
  virtual transport::DataVector fetch(std::int32_t address,
                                      std::uint16_t register_count) = 0;
  virtual void
  write_multiple_registers(std::int32_t address,
                           const std::vector<std::uint16_t> &data) = 0;
};
...
private:
  ...
  auto retry_with_config(Func &&func) -> decltype(std::forward<Func>(func)()) {
    const bool is_initial = m_initial_connection_mode.load();
    const int max_retries =
        is_initial ? m_initial_retry_count : m_normal_retry_count;
    const int delay_ms =
        is_initial ? m_initial_retry_delay_ms : m_normal_retry_delay_ms;
    ...
    while (infinite_retries || attempt <= max_retries) {
      try {
        ...
        const bool should_retry =
            infinite_retries ? true : attempt < max_retries;
        if (should_retry) {
          if (infinite_retries) {
            EVLOG_warning << "Modbus operation failed (attempt " << attempt
                          << ", infinite retries): " << e.what()
          ...
          EVLOG_error << "Modbus operation failed after " << attempt
                      << " attempts: " << e.what();
          // Raise CommunicationFault error for timeout errors
          if (m_error_handler) {
            m_error_handler("Modbus communication error: " +
                            std::string(e.what()));
          }
...
public:
  ...
  SerialCommHubTransport(serial_communication_hubIntf &serial_hub,
                         TransportConfig config)
      : SerialCommHubTransport(serial_hub, config, nullptr, nullptr) {}

  SerialCommHubTransport(serial_communication_hubIntf &serial_hub,
                         TransportConfig config, ErrorHandler error_handler,
                         ClearErrorHandler clear_error_handler)
      : m_serial_hub(serial_hub), m_device_id(config.device_id),
