...
// (300001-300080)
constexpr std::size_t ENERGY_IMPORT =
    104; // 300053 (0034h) - kWh (+) TOT, byte offset 104 (52*2)
constexpr std::size_t ENERGY_EXPORT =
    156; // 300079 (004Eh) - kWh (-) TOT, byte offset 156 (78*2)
} // namespace Offsets
...
constexpr float ENERGY_KWH_TO_WH =
    100.0F; // Value weight: kWh*10, convert to Wh (kWh*10 * 100 = Wh)
...
void powermeterImpl::init() {
  ...
  transport::ErrorHandler error_handler =
      [this](const std::string &error_message) {
        ...
        if (!this->error_state_monitor->is_error_active(
  ...
  p_modbus_transport = std::make_unique<transport::SerialCommHubTransport>(
      *mod->r_modbus, transport_config, error_handler, clear_error_handler);
}

void powermeterImpl::read_signature_config() {
  EVLOG_info << "Read the signature public key...";

  enum SignatureType { SIGNATURE_256_BIT, SIGNATURE_384_BIT, SIGNATURE_NONE };

  auto read_signature_type = [this]() {
    transport::DataVector data =
        p_modbus_transport->fetch(MODBUS_SIGNATURE_TYPE_ADDRESS, 1);
    return static_cast<SignatureType>(
        modbus_utils::to_uint16(data, modbus_utils::ByteOffset{0}));
  };

  auto read_public_key_in_hex = [this](int lengthInBits) {
    const transport::DataVector data = p_modbus_transport->fetch(
        MODBUS_PUBLIC_KEY_ADDRESS,
        static_cast<std::uint16_t>((lengthInBits >> 3) + 1));
    // Table 4.18/4.19: last byte is unused and always 0x00; keep the 0x04
    // prefix, drop the unused byte.
    return modbus_utils::to_hex_string(
        data, modbus_utils::ByteOffset{0},
  ...
  auto read_public_key_der_in_hex = [this](std::uint16_t der_word_count) {
    ...
    const transport::DataVector der_data = p_modbus_transport->fetch(
        MODBUS_PUBLIC_KEY_DER_ADDRESS, der_word_count);
    std::size_t der_len = der_data.size();
    if (der_data.size() >= 2 && der_data[0] == 0x30) {
      // DER header is: 0x30 <len> ...
      der_len = std::min<std::size_t>(
          der_data.size(), static_cast<std::size_t>(2 + der_data[1]));
    }
    return modbus_utils::to_hex_string(der_data, modbus_utils::ByteOffset{0},
                                       modbus_utils::ByteLength{der_len});
  };
  ...
  default:
    ...
    throw std::runtime_error(
        "no signature keys are configured, device is not eichrecht compliant");
  }
  EVLOG_info << "Signature type detected: " << signature_type_string;

  this->m_public_key_hex =
      read_public_key_in_hex(this->m_public_key_length_in_bits);
...
void powermeterImpl::read_firmware_versions() {
  ...
  transport::DataVector measure_fw_data =
      p_modbus_transport->fetch(MODBUS_FIRMWARE_MEASURE_MODULE_ADDRESS, 1);
  std::uint16_t measure_fw_value =
      modbus_utils::to_uint16(measure_fw_data, modbus_utils::ByteOffset{0});
  ...
  m_measure_module_firmware_version =
      fmt::format("{}.{}.{}", major, minor, revision);
  EVLOG_info << "Measure module firmware version: "
             << m_measure_module_firmware_version;

  // Read communication module firmware version/revision (register 300772)
  transport::DataVector comm_fw_data = p_modbus_transport->fetch(
      MODBUS_FIRMWARE_COMMUNICATION_MODULE_ADDRESS, 1);
  std::uint16_t comm_fw_value =
      modbus_utils::to_uint16(comm_fw_data, modbus_utils::ByteOffset{0});
  ...
  m_communication_module_firmware_version =
      fmt::format("{}.{}.{}", major, minor, revision);
  EVLOG_info << "Communication module firmware version: "
             << m_communication_module_firmware_version;
}
...
void powermeterImpl::read_serial_number() {
  ...
  transport::DataVector serial_data = p_modbus_transport->fetch(
      MODBUS_SERIAL_NUMBER_START_ADDRESS, MODBUS_SERIAL_NUMBER_REGISTER_COUNT);
  ...
  transport::DataVector year_data =
      p_modbus_transport->fetch(MODBUS_PRODUCTION_YEAR_ADDRESS, 1);
  std::uint16_t production_year =
      modbus_utils::to_uint16(year_data, modbus_utils::ByteOffset{0});
...
void powermeterImpl::read_transaction_state_and_id() {
  transport::DataVector state_data =
      p_modbus_transport->fetch(MODBUS_OCMF_STATE_ADDRESS, 1);
  std::uint16_t ocmf_state =
      modbus_utils::to_uint16(state_data, modbus_utils::ByteOffset{0});
...
void powermeterImpl::ready() {
  ...
  live_measure_thread_ = std::thread([this] {
    ...
    while (!stop_requested_.load()) {
      const auto measurement_interval =
          std::chrono::milliseconds{config.live_measurement_interval_ms};
      const auto device_state_interval =
          std::chrono::milliseconds{config.device_state_read_interval_ms};
      try {
        if (device_not_configured.load()) {
          configure_device();
          device_not_configured = false;
          last_device_state_read =
              std::chrono::steady_clock::time_point{}; // force state read
        }
        ...
        EVLOG_error << "Failed to communicate with the device, try again in "
                    << config.communication_error_pause_delay_s
                    << " seconds: " << e.what();
        device_not_configured = true;
        {
          ...
          stop_cv_.wait_for(
              lock,
              std::chrono::seconds{config.communication_error_pause_delay_s},
              [this] { return stop_requested_.load(); });
        }
      }
      {
        std::unique_lock<std::mutex> lock(stop_mutex_);
        stop_cv_.wait_for(lock, measurement_interval,
                          [this] { return stop_requested_.load(); });
      }
    }
  });
...
void powermeterImpl::write_transaction_registers(
    const types::powermeter::TransactionReq &transaction_req) {
  ...
  p_modbus_transport->write_multiple_registers(
      MODBUS_OCMF_IDENTIFICATION_STATUS_ADDRESS, status_data);
  ...
  if (transaction_req.identification_level.has_value()) {
    identification_level_value =
        ocmf::level_to_value(transaction_req.identification_level.value());
  }
  std::vector<std::uint16_t> level_data = {identification_level_value};
  p_modbus_transport->write_multiple_registers(
      MODBUS_OCMF_IDENTIFICATION_LEVEL_ADDRESS, level_data);

  // 3. Write OCMF Identification Flags (registers 328675-328678, 7002h-7005h) -
  // up to 4 flags
  std::vector<std::uint16_t> flags_data(MODBUS_OCMF_IDENTIFICATION_FLAGS_COUNT,
                                        0);
  for (size_t i = 0; i < transaction_req.identification_flags.size() &&
                     i < MODBUS_OCMF_IDENTIFICATION_FLAGS_COUNT;
       ++i) {
    flags_data[i] =
        ocmf::flag_to_value(transaction_req.identification_flags[i]);
  }
  p_modbus_transport->write_multiple_registers(
      MODBUS_OCMF_IDENTIFICATION_FLAGS_START_ADDRESS, flags_data);

  // 4. Write OCMF Identification Type (register 328679, 7006h)
  std::uint16_t identification_type_value =
      ocmf::type_to_value(transaction_req.identification_type);
  std::vector<std::uint16_t> type_data = {identification_type_value};
  p_modbus_transport->write_multiple_registers(
      MODBUS_OCMF_IDENTIFICATION_TYPE_ADDRESS, type_data);
  ...
  modbus_utils::log_truncation_warning_if_needed(
      "OCMF Identification Data", client_id_str,
      MODBUS_OCMF_IDENTIFICATION_DATA_WORD_COUNT);
  std::vector<std::uint16_t> id_data =
      modbus_utils::string_to_modbus_char_array(
          client_id_str, MODBUS_OCMF_IDENTIFICATION_DATA_WORD_COUNT);
  p_modbus_transport->write_multiple_registers(
      MODBUS_OCMF_IDENTIFICATION_DATA_START_ADDRESS, id_data);
  ...
  p_modbus_transport->write_multiple_registers(
      MODBUS_OCMF_CHARGING_POINT_ID_TYPE_ADDRESS, id_type_data);

  // 7. Write OCMF Charging point identifier (registers 328701-328720,
  // 701Ch-702Fh) - CHAR[40] = 20 words (evse_id)
  modbus_utils::log_truncation_warning_if_needed(
      "OCMF Charging Point Identifier (EVSE ID)", transaction_req.evse_id,
      MODBUS_OCMF_CHARGING_POINT_ID_WORD_COUNT);
  std::vector<std::uint16_t> evse_id_data =
      modbus_utils::string_to_modbus_char_array(
          transaction_req.evse_id, MODBUS_OCMF_CHARGING_POINT_ID_WORD_COUNT);
  p_modbus_transport->write_multiple_registers(
      MODBUS_OCMF_CHARGING_POINT_ID_START_ADDRESS, evse_id_data);
  ...
  const std::string tariff_text = transaction_req.tariff_text.value_or("") +
                                  tt_marker + transaction_req.transaction_id;
  modbus_utils::log_truncation_warning_if_needed(
      "OCMF Tariff Text (TT)", tariff_text, MODBUS_OCMF_TARIFF_TEXT_WORD_COUNT);
  const std::vector<std::uint16_t> tariff_text_data =
      modbus_utils::string_to_modbus_char_array(
          tariff_text, MODBUS_OCMF_TARIFF_TEXT_WORD_COUNT);
  p_modbus_transport->write_multiple_registers(MODBUS_OCMF_TARIFF_TEXT_ADDRESS,
                                               tariff_text_data);
}

std::string powermeterImpl::read_ocmf_file() {
  transport::DataVector size_data =
      p_modbus_transport->fetch(MODBUS_OCMF_STATE_SIZE_ADDRESS, 1);
  std::uint16_t size =
      modbus_utils::to_uint16(size_data, modbus_utils::ByteOffset{0});
  if (size == 0) {
    throw std::runtime_error("OCMF file size is 0");
  }
  transport::DataVector file_data =
      p_modbus_transport->fetch(MODBUS_OCMF_STATE_FILE_ADDRESS, size);
...
void powermeterImpl::clear_transaction_states() {
  ...
  if (ocmf_state == MODBUS_OCMF_STATE_READY) {
    ...
    std::vector<std::uint16_t> ocmf_confirmation_data = {
        MODBUS_OCMF_STATE_NOT_READY};
    p_modbus_transport->write_multiple_registers(MODBUS_OCMF_STATE_ADDRESS,
                                                 ocmf_confirmation_data);
    EVLOG_info << "Cleanup done.";
  }
}

types::powermeter::TransactionStartResponse
powermeterImpl::handle_start_transaction(
    types::powermeter::TransactionReq &treq) {
  try {
    EVLOG_info << "Starting transaction with transaction id: "
               << treq.transaction_id << " evse id: " << treq.evse_id
               << " identification status: " << treq.identification_status
               << " identification type: "
               << types::powermeter::ocmfidentification_type_to_string(
                      treq.identification_type)
               << " identification level: "
               << types::powermeter::ocmfidentification_level_to_string(
                      treq.identification_level.value_or(
                          types::powermeter::OCMFIdentificationLevel::NONE))
               << " identification data: "
               << treq.identification_data.value_or("")
    ...
    if (ocmf_state != MODBUS_OCMF_STATE_NOT_READY) {
      EVLOG_warning
          << "Spurious transaction detected, clearing transaction states ...";
    ...
    std::vector<std::uint16_t> session_modality_data = {
        MODBUS_OCMF_SESSION_MODALITY_CHARGING_VEHICLE};
    p_modbus_transport->write_multiple_registers(
        MODBUS_OCMF_SESSION_MODALITY_ADDRESS, session_modality_data);
...
powermeterImpl::handle_stop_transaction(std::string &transaction_id) {
  EVLOG_info << "Stopping transaction with transaction id: "
             << (transaction_id.empty() ? "empty" : transaction_id);
  ...
  if (transaction_id.empty()) {
    ...
    try {
      if (!m_pending_closed_transaction and m_transaction_active.load()) {
        ...
        p_modbus_transport->write_multiple_registers(
            MODBUS_OCMF_COMMAND_ADDRESS, command_data);
  ...
  try {
    if (m_pending_closed_transaction) {
      ...
      const auto ocmf_file_transaction_id_opt =
          ocmf::extract_transaction_id_from_ocmf_record(ocmf_file);
      if (!ocmf_file_transaction_id_opt.has_value()) {
        EVLOG_error
            << "Failed to extract transaction id from OCMF file TT field";
        return {types::powermeter::TransactionRequestStatus::UNEXPECTED_ERROR,
                {},
                {},
                "Failed to extract transaction id from OCMF file"};
      }
      const std::string &ocmf_file_transaction_id =
          *ocmf_file_transaction_id_opt;
      ...
      auto signed_meter_value =
          types::units_signed::SignedMeterValue{ocmf_file, "", "OCMF"};
      ...
      auto signed_meter_value =
          types::units_signed::SignedMeterValue{ocmf_data, "", "OCMF"};
      signed_meter_value.public_key.emplace(m_public_key_hex);
      ...
      EVLOG_error << "No open transaction or unknown transaction id: "
                  << transaction_id;
      return {types::powermeter::TransactionRequestStatus::UNEXPECTED_ERROR,
              {},
              {},
...
void powermeterImpl::read_powermeter_values() {
  ...
  transport::DataVector data = p_modbus_transport->fetch(
      MODBUS_REAL_TIME_VALUES_ADDRESS, MODBUS_REAL_TIME_VALUES_COUNT);
  ...
  voltage_V.L1 =
      Factors::VOLTAGE * static_cast<float>(modbus_utils::to_int32(
                             data, modbus_utils::ByteOffset{Offsets::V_L1_N}));
  voltage_V.L2 =
      Factors::VOLTAGE * static_cast<float>(modbus_utils::to_int32(
                             data, modbus_utils::ByteOffset{Offsets::V_L2_N}));
  voltage_V.L3 =
      Factors::VOLTAGE * static_cast<float>(modbus_utils::to_int32(
                             data, modbus_utils::ByteOffset{Offsets::V_L3_N}));
  ...
  current_A.L1 =
      Factors::CURRENT * static_cast<float>(modbus_utils::to_int32(
                             data, modbus_utils::ByteOffset{Offsets::A_L1}));
  current_A.L2 =
      Factors::CURRENT * static_cast<float>(modbus_utils::to_int32(
                             data, modbus_utils::ByteOffset{Offsets::A_L2}));
  current_A.L3 =
      Factors::CURRENT * static_cast<float>(modbus_utils::to_int32(
                             data, modbus_utils::ByteOffset{Offsets::A_L3}));
  ...
  power_W.L1 =
      Factors::POWER * static_cast<float>(modbus_utils::to_int32(
                           data, modbus_utils::ByteOffset{Offsets::W_L1}));
  power_W.L2 =
      Factors::POWER * static_cast<float>(modbus_utils::to_int32(
                           data, modbus_utils::ByteOffset{Offsets::W_L2}));
  power_W.L3 =
      Factors::POWER * static_cast<float>(modbus_utils::to_int32(
                           data, modbus_utils::ByteOffset{Offsets::W_L3}));
  power_W.total =
      Factors::POWER * static_cast<float>(modbus_utils::to_int32(
                           data, modbus_utils::ByteOffset{Offsets::W_SYS}));
  ...
  VAR.L1 = Factors::REACTIVE_POWER *
           static_cast<float>(modbus_utils::to_int32(
               data, modbus_utils::ByteOffset{Offsets::VAR_L1}));
  VAR.L2 = Factors::REACTIVE_POWER *
           static_cast<float>(modbus_utils::to_int32(
               data, modbus_utils::ByteOffset{Offsets::VAR_L2}));
  VAR.L3 = Factors::REACTIVE_POWER *
           static_cast<float>(modbus_utils::to_int32(
               data, modbus_utils::ByteOffset{Offsets::VAR_L3}));
  VAR.total = Factors::REACTIVE_POWER *
              static_cast<float>(modbus_utils::to_int32(
                  data, modbus_utils::ByteOffset{Offsets::VAR_SYS}));
  powermeter.VAR = VAR;
  ...
  frequency_Hz.L1 = Factors::FREQUENCY *
                    static_cast<float>(modbus_utils::to_int16(
                        data, modbus_utils::ByteOffset{Offsets::FREQUENCY}));
  ...
  std::int16_t phase_sequence = modbus_utils::to_int16(
      data, modbus_utils::ByteOffset{Offsets::PHASE_SEQUENCE});
  if (phase_sequence == -1) {
    powermeter.phase_seq_error =
        true; // L1-L3-L2 is considered an error (counter-clockwise)
  ...
  powermeter.energy_Wh_import.total =
      ...
      static_cast<float>(modbus_utils::to_int32(
          data, modbus_utils::ByteOffset{Offsets::ENERGY_IMPORT}));
  ...
  energy_Wh_export.total =
      Factors::ENERGY_KWH_TO_WH *
      static_cast<float>(modbus_utils::to_int32(
          data, modbus_utils::ByteOffset{Offsets::ENERGY_EXPORT}));
...
void powermeterImpl::dump_device_state() {
  try {
    ...
    transport::DataVector charging_status_data =
        p_modbus_transport->fetch(MODBUS_OCMF_CHARGING_STATUS_ADDRESS, 1);
    std::uint16_t charging_status = modbus_utils::to_uint16(
        charging_status_data, modbus_utils::ByteOffset{0});

    // 3. Last transaction id (register 328723 / 7059h, CHAR[])
    transport::DataVector last_tx_data =
        p_modbus_transport->fetch(MODBUS_OCMF_LAST_TRANSACTION_ID_ADDRESS,
    ...
    std::uint16_t time_sync_status = modbus_utils::to_uint16(
        time_sync_status_data, modbus_utils::ByteOffset{0});

    // 5. OCMF command (last written command value)
    transport::DataVector cmd_data =
        p_modbus_transport->fetch(MODBUS_OCMF_COMMAND_ADDRESS, 1);
    std::uint16_t raw_cmd =
        modbus_utils::to_uint16(cmd_data, modbus_utils::ByteOffset{0});
    ...
    transport::DataVector tx_def_data = p_modbus_transport->fetch(
        MODBUS_OCMF_TRANSACTION_ID_GENERATION_ADDRESS, 1);
    std::uint16_t tx_def =
        modbus_utils::to_uint16(tx_def_data, modbus_utils::ByteOffset{0});
    ...
    EVLOG_info << "  Time synchronization status (device, raw): "
               << time_sync_status;
    EVLOG_info << "  Last OCMF command (raw): 0x" << std::hex << raw_cmd
               << " ('" << cmd_char << "')";
    EVLOG_info << "  Transaction ID definition (OCMF): 0x" << std::hex
               << tx_def;
  } catch (const std::exception &e) {
...
void powermeterImpl::synchronize_time() {
  ...
  data.push_back(
      static_cast<std::uint16_t>(timestamp & 0xFFFF)); // LSW: bits 0-15
  data.push_back(
      static_cast<std::uint16_t>((timestamp >> 16) & 0xFFFF)); // bits 16-31
  data.push_back(
      static_cast<std::uint16_t>((timestamp >> 32) & 0xFFFF)); // bits 32-47
  data.push_back(static_cast<std::uint16_t>((timestamp >> 48) &
                                            0xFFFF)); // MSW: bits 48-63

  // Write UTC timestamp to register 328723 (4 words for INT64)
  p_modbus_transport->write_multiple_registers(MODBUS_UTC_TIMESTAMP_ADDRESS,
                                               data);
...
void powermeterImpl::set_timezone(int offset_minutes) {
  ...
  p_modbus_transport->write_multiple_registers(MODBUS_TIMEZONE_OFFSET_ADDRESS,
                                               data);

  EVLOG_info << "Timezone set to: " << (offset_minutes >= 0 ? "+" : "")
...
void powermeterImpl::time_sync_thread() {
  ...
  while (!stop_requested_.load()) {
    {
      std::unique_lock<std::mutex> lock(stop_mutex_);
      stop_cv_.wait_until(lock, next_sync_time,
                          [this] { return stop_requested_.load(); });
    }
    ...
    if (!is_transaction_active()) {
      ...
      EVLOG_info
          << "Time synchronization deferred: charging session in progress";
      m_pending_time_sync = true;
    }
...
void powermeterImpl::read_device_state() {
  ...
  transport::DataVector state_data =
      p_modbus_transport->fetch(MODBUS_DEVICE_STATE_ADDRESS, 1);
  std::uint16_t device_state =
      modbus_utils::to_uint16(state_data, modbus_utils::ByteOffset{0});

  // Check for error bits and raise VendorError if any are set
  const auto error_messages =
      device_state_utils::decode_device_state_errors(device_state);
  ...
  if (!error_messages.empty()) {
    ...
    error_description +=
        " (device state: 0x" + fmt::format("{:04X}", device_state) + ")";

    EVLOG_error << "Device state error: " << error_description;
    auto error = this->error_factory->create_error(
        "powermeter/VendorError", "DeviceStateError", error_description);
    raise_error(error);
  } else {
