...
bool CarSimulation::iso_start_v2g_session(const CmdArguments& arguments, bool three_phases) {
    ...
    EVLOG_debug << "energy mode: " << energy_mode << " payment option: " << payment_option;

    if (energy_mode == constants::AC) {
        sim_data.energy_mode = EnergyMode::AC;
        if (three_phases == false) {
            r_ev[0]->call_start_charging(types::iso15118::EnergyTransferMode::AC_single_phase_core, payment_option);
            charge_mode = ChargeMode::AC;
        } else {
            r_ev[0]->call_start_charging(types::iso15118::EnergyTransferMode::AC_three_phase_core, payment_option);
            charge_mode = ChargeMode::ACThreePhase;
        }
    } else if (energy_mode == constants::DC) {
        r_ev[0]->call_start_charging(types::iso15118::EnergyTransferMode::DC_extended, payment_option);
