...
void start_eebus_grpc_api(std::filesystem::path binary_path, int port, std::filesystem::path cert_file, std::filesystem::path key_file) {
...
void EEBUS::init() {
    ...
    if (this->config.manage_eebus_grpc_api_binary) {
        this->eebus_grpc_api_thread = std::thread(
            ...
            this->config_validator->get_eebus_grpc_api_binary_path(),
