...
ControlServiceCalls::ControlServiceCalls(
    const std::shared_ptr<control_service::ControlService::Stub> stub
) : stub(stub) {
    this->entity_address = common_types::CreateEntityAddress(
...
void ControlServiceCalls::call_start_setup() {
    control_service::EmptyResponse response;
    control_service::CallStartSetup(
        this->stub,
...
void ControlServiceCalls::call_register_remote_ski(const std::string& remote_ski) {
    ...
    request = control_service::CreateRegisterRemoteSkiRequest(
        remote_ski
    );
    control_service::EmptyResponse response;
...
void ControlServiceCalls::subscribe_use_case_events(
    ...
    control_service::SubscribeUseCaseEventsRequest request = control_service::CreateSubscribeUseCaseEventsRequest(
        &this->entity_address,
        &this->use_case
    );
