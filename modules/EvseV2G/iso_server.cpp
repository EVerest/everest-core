...
static enum v2g_event handle_iso_session_setup(struct v2g_connection* conn) {
    ...
    char buffer[iso2_evccIDType_BYTES_SIZE * 3 - 1 + 1] = {'\0'}; /* format: (%02x:) * n - (1x ':') + (1x NULL) */
    enum v2g_event next_event = V2G_EVENT_NO_EVENT;

    /* format EVCC ID */
    for (std::size_t i = 0; i < req->EVCCID.bytesLen; i++) {
        if (i > 0)
            buffer[i * 3 - 1] = ':';
        snprintf(&buffer[i * 3], std::size(buffer) - i * 3, "%02" PRIX8, req->EVCCID.bytes[i]);
    }
