...
public:
    ...
    types::reservation::ReservationResult make_reservation(const std::optional<uint32_t> evse_id, const types::reservation::Reservation& reservation);
    ...
    void
    register_reservation_cancelled_callback(const std::function<void(const std::optional<uint32_t> evse_id)>& callback);
...
private: // Functions
    bool has_evse_connector_type(const std::vector<EvseConnectorType> evse_connectors,
                                 const types::evse_manager::ConnectorTypeEnum connector_type);
    types::reservation::ReservationResult get_evse_state(const uint32_t evse_id,
                           const std::map<uint32_t, types::reservation::Reservation> evse_specific_reservations);
    types::reservation::ReservationResult is_connector_available(const uint32_t evse_id, const types::evse_manager::ConnectorTypeEnum connector_type);
    ...
    bool is_reservation_possible(const std::optional<types::evse_manager::ConnectorTypeEnum> global_reservation_type,
                                 const std::map<uint32_t, types::reservation::Reservation>& evse_specific_reservations);
    void set_reservation_timer(const types::reservation::Reservation& reservation, const std::optional<uint32_t> evse_id);
    types::reservation::ReservationResult get_reservation_evse_connector_state();
