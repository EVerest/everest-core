...
void powermeter_ACImpl::run_meter_loop() {
    ...
    while (true) {
        ...
        uint64_t timestamp =
            std::chrono::duration_cast<std::chrono::milliseconds>(date::utc_clock::now().time_since_epoch()).count();
...
void powermeter_ACImpl::init_modbus_client() {
...
uint32_t powermeter_ACImpl::read_power_in() {
    std::vector<uint8_t> bytevector = this->modbus_client->read_holding_register(
        config.power_unit_id, config.power_in_register, config.power_in_length);
    modbus::utils::print_message_hex(bytevector);
    return sunspec::conversion::bytevector_to_uint32(bytevector);
}

uint32_t powermeter_ACImpl::read_power_out() {
    std::vector<uint8_t> bytevector = this->modbus_client->read_holding_register(
        config.power_unit_id, config.power_out_register, config.power_out_length);
    return sunspec::conversion::bytevector_to_uint32(bytevector);
}

uint32_t powermeter_ACImpl::read_energy_in() {
    std::vector<uint8_t> bytevector = this->modbus_client->read_holding_register(
        config.energy_unit_id, config.energy_in_register, config.energy_in_length);
    return sunspec::conversion::bytevector_to_uint32(bytevector);
}

uint32_t powermeter_ACImpl::read_energy_out() {
    std::vector<uint8_t> bytevector = this->modbus_client->read_holding_register(
        config.energy_unit_id, config.energy_out_register, config.energy_out_length);
    return sunspec::conversion::bytevector_to_uint32(bytevector);
}
