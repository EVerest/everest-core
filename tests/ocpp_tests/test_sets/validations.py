...
def validate_standard_start_transaction(meta_data, msg, exp_payload):
    ...
    success = msg.payload['connectorId'] == exp_payload.connector_id and (msg.payload['idTag'] == exp_payload.id_tag or exp_payload.id_tag == None) \
        and msg.payload['meterStart'] == exp_payload.meter_start and 'timestamp' in msg.payload
...
def validate_standard_stop_transaction(meta_data, msg, exp_payload):
    ...
    success = 'meterStop' in msg.payload and \
        msg.payload['reason'] == exp_payload.reason and \
        (msg.payload['transactionId'] == exp_payload.transaction_id or
         msg.payload['transactionId'] == STANDARD_TRANSACTION_ID)
...
def validate_meter_values(messages, periodic_measurands, clock_aligned_measurands,
                          periodic_interval, clock_aligned_interval):

    periodic_meter_values = []
    clock_aligned_meter_values = []
    for msg in messages:
        if msg.payload['meterValue'][0]['sampledValue'][0]['context'] == "Sample.Periodic":
            periodic_meter_values.extend(msg.payload['meterValue'])
        elif msg.payload['meterValue'][0]['sampledValue'][0]['context'] == "Sample.Clock":
...
def validate_composite_schedule(meta_data, msg, exp_payload: call_result.GetCompositeSchedulePayload):
    return msg.payload['status'] == exp_payload.status and \
        msg.payload['connectorId'] == exp_payload.connector_id and \
        msg.payload['chargingSchedule']['chargingRateUnit'] == exp_payload.charging_schedule.charging_rate_unit and \
        validate_duration(msg.payload['chargingSchedule']['duration'], exp_payload.charging_schedule.duration) and \
        validate_charging_schedule_periods(msg.payload['chargingSchedule']['chargingSchedulePeriod'],
                                           exp_payload.charging_schedule.charging_schedule_period
                                           )


def validate_duration(duration, exp_duration):
    return duration == exp_duration or duration - 2 == exp_duration or duration + 2 == exp_duration
    ...
    if success:
        ...
        for i in range(len(exp_periods)):
            if periods[i]['limit'] != exp_periods[i].limit:
                return False
            elif periods[i]['startPeriod'] != exp_periods[i].start_period and \
                periods[i]['startPeriod'] != exp_periods[i].start_period+1 and \
                    periods[i]['startPeriod'] != exp_periods[i].start_period-1:
                return False
            elif exp_periods[i].number_phases != None and periods[i]['numberPhases'] != exp_periods[i].number_phases:
                return False
            i += 1
        return True
    else:
...
def validate_boot_notification(meta_data, msg, exp_payload):
    return msg.payload['chargeBoxSerialNumber'] == exp_payload.charge_box_serial_number and \
        msg.payload['chargePointModel'] == exp_payload.charge_point_model and \
        msg.payload['chargePointVendor'] == exp_payload.charge_point_vendor
...
def validate_notify_report_data_201(meta_data, msg, exp_payload):
    ...
    for payload in exp_payload.report_data:
        ...
        if el != None:
           if msg.payload['requestId'] == exp_payload.request_id and \
            payload.variable_attribute.type == el['variableAttribute'][0]['type'] and \
            payload.variable_attribute.value == el['variableAttribute'][0]['value']:
                found_items += 1
...
def find_report_data(report_data_element, report_data_list):
    for el in report_data_list:
        if el['component']['name'] == report_data_element.component.name and el['variable']['name'] == report_data_element.variable.name:
            #check if evse id has to be checked
            if report_data_element.component.evse != None:
                if report_data_element.component.evse.id == el['component']['evse']['id']:
                    return el
...
def validate_data_transfer_pnc_get_15118_ev_certificate(meta_data, msg, exp_payload):
    return msg.payload['vendorId'] == exp_payload.vendor_id and \
        msg.payload['messageId'] == exp_payload.message_id and \
        'action' in msg.payload['data'] and \
        'exiRequest' in msg.payload['data'] and \
        'iso15118SchemaVersion' in msg.payload['data']
...
def validate_data_transfer_sign_certificate(meta_data, msg, exp_payload):
    ...
    try:
        return msg.payload['vendorId'] == exp_payload.vendor_id and \
            msg.payload['messageId'] == exp_payload.message_id and \
            'certificateType' in data and \
            'csr' in data and \
            data['certificateType'] == "V2GCertificate" and \
            crypto.load_certificate_request(crypto.FILETYPE_PEM, data['csr'])
    except:
        return False

async def wait_for_callerror_and_validate(meta_data, charge_point,
    ...
    if (meta_data.validation_mode == ValidationMode.EASY and
        validate_call_error_against_old_messages(meta_data, exp_payload)):
        logging.debug(
    ...
    logging.info(
        f"Timeout while waiting for CallError message")
...
def validate_call_error(msg, exp_payload):
    if (msg.message_type_id == 4):
        ...
        if (msg.error_code == exp_payload):
            ...
            logging.error(f"Wrong error code \"{msg.error_code}\" expected \"{exp_payload}\"")
            return False
    return False
