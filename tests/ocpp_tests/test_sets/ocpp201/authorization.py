...
async def test_authorize_01(charge_point_v201: ChargePoint201, test_controller: TestController, test_utility: TestUtility):
...
@ pytest.mark.asyncio
@pytest.mark.ocpp_version("ocpp2.0.1")
async def test_c09(charge_point_v201: ChargePoint201, central_system_v201: CentralSystem, test_controller: TestController, test_utility: TestUtility):
    """
    C09.FR.03
    C09.FR.04
    C09.FR.05
    ...
    r: call_result201.SetVariablesPayload = await charge_point_v201.set_config_variables_req("AuthCtrlr", "MasterPassGroupId", "00000000")
    set_variable_result: SetVariableResultType = SetVariableResultType(
        **r.set_variable_result[0])
    assert set_variable_result.attribute_status == SetVariableStatusType.accepted
    ...
    def get_token_info(token: str):
        if token in accepted_tags:
            return IdTokenInfoType(status=AuthorizationStatusType.accepted,
                                   group_id_token=IdTokenType(id_token="123", type=IdTokenTypeEnum.central))
        else:
    ...
    def on_authorize(**kwargs):
        ...
        return call_result201.AuthorizePayload(id_token_info=get_token_info(msg_token.id_token))
    ...
    def on_transaction_event(**kwargs):
        ...
        if msg.id_token != None:
            msg_token = IdTokenType(**msg.id_token)
            return call_result201.TransactionEventPayload(id_token_info=get_token_info(msg_token.id_token))
        else:
    ...
    assert await wait_for_and_validate(test_utility, charge_point_v201, "StatusNotification", {"connectorStatus": "Available", "evseId": 2})
    ...
    assert await wait_for_and_validate(test_utility, charge_point_v201, "StatusNotification", {"connectorStatus": "Available", "evseId": 1})
...
@ pytest.mark.asyncio
@pytest.mark.ocpp_version("ocpp2.0.1")
async def test_c10_c11_c12(charge_point_v201: ChargePoint201, test_controller: TestController, test_utility: TestUtility):
    ...
    r: call_result201.SetVariablesPayload = await charge_point_v201.set_config_variables_req("AuthCtrlr", "LocalPreAuthorize", "false")
    set_variable_result: SetVariableResultType = SetVariableResultType(
        **r.set_variable_result[0])
    assert set_variable_result.attribute_status == SetVariableStatusType.accepted

    # Set AuthCacheLifeTime to 1s
    r: call_result201.SetVariablesPayload = await charge_point_v201.set_config_variables_req("AuthCacheCtrlr", "LifeTime", "1")
    set_variable_result: SetVariableResultType = SetVariableResultType(
        **r.set_variable_result[0])
    assert set_variable_result.attribute_status == SetVariableStatusType.accepted
...
@pytest.mark.asyncio
@pytest.mark.ocpp_version("ocpp2.0.1")
async def test_c15(charge_point_v201: ChargePoint201, central_system_v201: CentralSystem, test_controller: TestController, test_utility:TestUtility):
    """
    C15.FR.01
    C15.FR.02
    C15.FR.03
    ...
    C15.FR.08
    """
    log.info(" ########### Test case C15: Offline Authorization of unknown Id ###########")
    ...
    def on_transaction_event(**kwargs):
        ...
        if msg.id_token != None:
            if (stop_tx_on_invalid_id != None):
                return call_result201.TransactionEventPayload(id_token_info=IdTokenInfoType(status=AuthorizationStatusType.unknown))
            else:
                return call_result201.TransactionEventPayload(id_token_info=IdTokenInfoType(status=AuthorizationStatusType.accepted))
        else:
            return call_result201.TransactionEventPayload()
    ...
    central_system_v201.function_overrides.append(('on_transaction_event', on_transaction_event))
    ...
    log.debug("==============================================C15.FR.08 ====================================")
    log.debug("The Charging Station rejects the unknown IdToken if OfflineTxForUnknownIdEnabled is set False ")
    # Disable offline authorization for unknown ID
    r: call_result201.SetVariablesPayload = await charge_point_v201.set_config_variables_req("AuthCtrlr","OfflineTxForUnknownIdEnabled","false")
    ...
    # C15.FR.08
    log.debug("==============================================C15.FR.08 ====================================")
    log.debug("The Charging Station accepts the unknown IdToken if OfflineTxForUnknownIdEnabled is set True ")
