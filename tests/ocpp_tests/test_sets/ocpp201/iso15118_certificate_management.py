...
class CertificateHashDataChainEntry:
    ...
    def from_dict(data: Dict):
        return CertificateHashDataChainEntry(
            ...
            child_certificate_hash_data=[CertificateHashData(**d) for d in data.get("child_certificate_hash_data", [])]
        )
...
class CertificateHashDataChain:
    ...
    def from_list(data: list[dict]):
        return CertificateHashDataChain(entries=[CertificateHashDataChainEntry.from_dict(d) for d in data])
...
class TestIso15118CertificateManagementOcppIntegration:
    ...
    @pytest.mark.parametrize("skip_implementation",
                             [{"ProbeModuleConnectorA":["set_get_certificate_response"], "ProbeModuleConnectorB":["set_get_certificate_response"]}])
    @pytest.mark.ocpp_config_adaptions(GenericOCPP201ConfigAdjustment([
    (OCPP201ConfigVariableIdentifier("ISO15118Ctrlr", "ContractCertificateInstallationEnabled", "Actual"), True)]))
    ...
    async def test_m1_m2_certificate_request_ev(self, action, response_status,
        ...
        for connector_id in connectors:
            ...
            probe_module.implement_command(connector_id,
        ...
        for connector_index, calling_connector_id in enumerate(connectors):
            ...
            mock_certificate_installation_req = base64.b64encode(
                f"{calling_connector_id} mock Raw CertificateInstallationReq or CertificateUpdateReq message as exi stream"
                .encode("utf-8")
            ).decode("utf-8")

            mock_iso15118_schema_version = f"{calling_connector_id} mock Schema Version"

            probe_module.publish_variable(calling_connector_id, "iso15118_certificate_request",
            ...
            assert called_mock.mock_calls == [
                mock_call(
                    {'certificate_response': {'certificate_action': action,
                                              'exi_response': exi_response,
    ...
    @pytest.mark.asyncio
    async def test_m3_get_installed_certificates(self, central_system: CentralSystem,
        ...
        def security_module_get_certs_mock(args):
            return {
                "status": "Accepted",
                "certificate_hash_data_chain": [copy.deepcopy(mock_certificate_hash_data_chain_data[certificate_type])
                                                for
                                                certificate_type
                                                in args["certificate_types"]]
            }
        ...
        probe_module.implement_command("ProbeModuleSecurity",
                                       "get_installed_certificates", security_module_mock)
        ...
        # Act: request certs
        ocpplib_result: call_result201.GetInstalledCertificateIdsPayload = await chargepoint_with_pm.get_installed_certificate_ids_req(
            certificate_type=[GetCertificateIdUseType.csms_root_certificate,
                              GetCertificateIdUseType.v2g_certificate_chain])

        # Verfiy
        assert ocpplib_result == call_result201.GetInstalledCertificateIdsPayload(status='Accepted',
        ...
        assert security_module_mock.mock_calls == [
            mock_call({'certificate_types': ['CSMSRootCertificate', 'V2GCertificateChain']})]

    @pytest.mark.parametrize("skip_implementation",
                             [{"ProbeModuleSecurity":["get_installed_certificates"]}])
    async def test_m3_get_installed_certificates_not_found(self, central_system: CentralSystem,
                                                           probe_module: ProbeModule):
        """
        ...
        # Verfiy
        assert ocpplib_result == call_result201.GetInstalledCertificateIdsPayload(status='NotFound',
    ...
    async def test_m4_delete(self, response_status, central_system: CentralSystem,
                             probe_module: ProbeModule,
                             test_utility: TestUtility):
        """
        ...
        probe_module.implement_command("ProbeModuleSecurity", "delete_certificate", security_module_mock)
        ...
        await chargepoint_with_pm.delete_certificate_req(certificate_hash_data=copy.deepcopy(cert_hash_data))
        assert await wait_for_and_validate(test_utility, chargepoint_with_pm, "DeleteCertificate",
                                           call_result201.DeleteCertificatePayload(response_status))
        assert security_module_mock.mock_calls == [mock_call({'certificate_hash_data': cert_hash_data})]
    ...
    @pytest.mark.parametrize("evse_security_response_status, chargepoint_response_status",
                             [("Accepted", "Accepted"),
                              ("WriteError", "Failed"),
                              ("InvalidFormat", "Rejected"),
                              ])
    async def test_m5_install(self, evse_security_response_status, chargepoint_response_status,
                              test_config: OcppTestConfiguration,
                              central_system: CentralSystem,
                              probe_module: ProbeModule, test_utility: TestUtility):
        """
        ...
        probe_module.implement_command("ProbeModuleSecurity", "install_ca_certificate", security_module_mock)
        ...
        assert await wait_for_and_validate(test_utility, chargepoint_with_pm, "InstallCertificate",
                                           call_result201.InstallCertificatePayload(chargepoint_response_status))

        assert security_module_mock.mock_calls == [
            mock_call({'certificate': request['certificate'], 'certificate_type': 'CSMS'})]
...
class TestIso15118CertificateManagementE2E:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ('CSMSRootCertificate', GetCertificateIdUseType.csms_root_certificate, "csms/CSMS_ROOT_CA.pem"),
            ('ManufacturerRootCertificate', GetCertificateIdUseType.manufacturer_root_certificate, "mf/MF_ROOT_CA.pem"),
            ('V2GRootCertificate', GetCertificateIdUseType.v2g_root_certificate, "v2g/V2G_ROOT_CA.pem"),
            ('MORootCertificate', GetCertificateIdUseType.mo_root_certificate, "mo/MO_ROOT_CA.pem"),
        ]
    )
    async def test_m3_retrieve_installed_certificates(self,
                                                      certificate_type, use_type, certificate_file,
                                                      test_config: OcppTestConfiguration,
                                                      charge_point_v201: ChargePoint201):
        search_path = test_config.certificate_info.csms_root_ca.parent.parent

        result: call_result201.GetInstalledCertificateIdsPayload = await charge_point_v201.get_installed_certificate_ids_req(
        ...
        assert result == call_result201.GetInstalledCertificateIdsPayload(
            ...
            certificate_hash_data_chain=[{'certificate_hash_data': CertificateHashDataGenerator.get_hash_data(
                certificate_path=search_path / certificate_file
            ),
                'certificate_type': certificate_type}]
        )

    async def test_m3_retrieve_installed_certificates_all_types(self,
                                                                test_config: OcppTestConfiguration,
                                                                charge_point_v201: ChargePoint201):
        """
        ...
        expected_certificate_hash_data_chain_set = {
                ...
                ('CSMSRootCertificate', GetCertificateIdUseType.csms_root_certificate, "csms/CSMS_ROOT_CA.pem"),
                ('ManufacturerRootCertificate', GetCertificateIdUseType.manufacturer_root_certificate,
                 "mf/MF_ROOT_CA.pem"),
                ('MORootCertificate', GetCertificateIdUseType.mo_root_certificate, "mo/MO_ROOT_CA.pem"),
                ('V2GRootCertificate', GetCertificateIdUseType.v2g_root_certificate, "v2g/V2G_ROOT_CA.pem"),
            ]
                                                   } | {CertificateHashDataChainEntry.from_dict(d) for d in
                                                        self._get_v2g_certificate_chain(search_path)}

        assert {CertificateHashDataChainEntry.from_dict(d) for d in
                result.certificate_hash_data_chain} == expected_certificate_hash_data_chain_set

    async def test_m3_retrieve_installed_certificates_not_found(self,
                                                                tmp_path,
        ...
        for f in list((tmp_path / "certs/ca/mo").glob("*.pem")) + list((tmp_path / "certs/ca/mo").glob("*.der")):
            f.unlink()
    ...
    def _get_v2g_certificate_chain(self, ca_cert_path: Path):
        ...
        exp_hashdata_ca_1 = CertificateHashDataGenerator.get_hash_data(
            certificate_path=sub_ca_1_path,
            issuer_certificate_path=root_ca_path
        )
        exp_hashdata_ca_2 = CertificateHashDataGenerator.get_hash_data(
            certificate_path=sub_ca_2_path,
            issuer_certificate_path=sub_ca_1_path
        )
        exp_hashdata_leaf = CertificateHashDataGenerator.get_hash_data(
            certificate_path=leaf_cert_path,
            issuer_certificate_path=sub_ca_2_path
        )
    ...
    async def test_m3_retrieve_v2g_certificate_chain(self,
                                                     test_config: OcppTestConfiguration,
                                                     charge_point_v201: ChargePoint201):
        """
        ...
        resulting_chain = CertificateHashDataChain.from_list(result.certificate_hash_data_chain)
        expected_chain = CertificateHashDataChain.from_list(self._get_v2g_certificate_chain(
    ...
    async def test_m4_delete_and_retrieve_certificates(self, certificate_type, use_type, certificate_file, test_config,
                                                       charge_point_v201: ChargePoint201):
        certificate_search_path = test_config.certificate_info.csms_root_ca.parent.parent
        ...
        deletion_result: call_result201.GetInstalledCertificateIdsPayload = await charge_point_v201.delete_certificate_req(
        ...
        verification_result: call_result201.GetInstalledCertificateIdsPayload = await charge_point_v201.get_installed_certificate_ids_req(
            certificate_type=[use_type])

        assert verification_result.status == GetInstalledCertificateStatusType.notFound

    async def test_m4_reject_deletion_of_charging_station_certificate(self, test_config, charge_point_v201):
        """
                M_ISO_15118_CertificateManagement_CS - TC_M_23_CS-Delete a certificate from a Charging Station
        """

        cso_certificate = test_config.certificate_info.csms_root_ca.parent.parent.parent / "client" / "csms" / "CSMS_RSA.pem"
        issuer_certificate = test_config.certificate_info.csms_root_ca.parent.parent.parent / "ca" / "csms" / "CSMS_ROOT_CA.pem"
        certificate_for_deletion_hash_data = CertificateHashDataGenerator.get_hash_data(
    ...
    async def test_m4_allow_deletion_of_secc_leaf_certificate(self, test_config, charge_point_v201):
        """
                M_ISO_15118_CertificateManagement_CS - TC_M_23_CS-Delete a certificate from a Charging Station
        """

        cso_certificate = test_config.certificate_info.csms_root_ca.parent.parent.parent / "client" / "cso" / "SECC_LEAF.pem"
        issuer_certificate = test_config.certificate_info.csms_root_ca.parent.parent.parent / "ca" / "cso" / "CPO_SUB_CA2.pem"
        certificate_for_deletion_hash_data = CertificateHashDataGenerator.get_hash_data(
            certificate_path=cso_certificate, issuer_certificate_path=issuer_certificate
        )
    ...
    async def test_m5_install_ca_certificate(self, example_certificate,
                                             certificate_type, ocpp_certificate_type,
                                             charge_point_v201: ChargePoint201):
        certificate, cert_hash_data = example_certificate["certificate"], example_certificate["certificate_hash_data"]

        certificates_before: call_result201.GetInstalledCertificateIdsPayload = await charge_point_v201.get_installed_certificate_ids_req(
            certificate_type=[ocpp_certificate_type])

        logging.info(f"Installing certificate (serial: {cert_hash_data['serial_number']}) as {ocpp_certificate_type}")
        res: call_result201.InstallCertificatePayload = await charge_point_v201.install_certificate_req(
            certificate_type=ocpp_certificate_type,
        ...
        assert {CertificateHashDataChainEntry.from_dict(d) for d in
                verification_result.certificate_hash_data_chain} == {
                   CertificateHashDataChainEntry.from_dict({
                       'certificate_hash_data': cert_hash_data,
                       'certificate_type': certificate_type})} | {CertificateHashDataChainEntry.from_dict(d) for d in
                                                                  certificates_before.certificate_hash_data_chain}

    async def test_m5_reject_installation_of_expired_certificate(self, charge_point_v201):
...
-----END CERTIFICATE-----"""
    ...
    async def test_m4_delete_installed_certificates(self, certificate_type, ocpp_certificate_type, example_certificate,
                                                    test_config,
                                                    charge_point_v201: ChargePoint201):
        certificate, cert_hash_data = example_certificate["certificate"], example_certificate["certificate_hash_data"]
        ...
        installation_result: call_result201.InstallCertificatePayload = await charge_point_v201.install_certificate_req(
            certificate_type=ocpp_certificate_type,
            certificate=certificate)
        assert installation_result == call_result201.InstallCertificatePayload(status='Accepted')

        certificates_after_install: call_result201.GetInstalledCertificateIdsPayload = await charge_point_v201.get_installed_certificate_ids_req(
            certificate_type=[ocpp_certificate_type])

        assert cert_hash_data in [c["certificate_hash_data"] for c in
                                  certificates_after_install.certificate_hash_data_chain]
        ...
        certificates_after_delete: call_result201.GetInstalledCertificateIdsPayload = await charge_point_v201.get_installed_certificate_ids_req(
            certificate_type=[ocpp_certificate_type])

        assert certificates_after_delete.status == GetInstalledCertificateStatusType.accepted
        assert certificates_before.certificate_hash_data_chain == certificates_after_delete.certificate_hash_data_chain
