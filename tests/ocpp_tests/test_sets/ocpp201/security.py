...
from everest.testing.core_utils._configuration.libocpp_configuration_helper import _OCPP201NetworkConnectionProfileAdjustment
...
class _BaseTest:
    ...
    def _setup_csms_mock(self,
                         csms_mock: Mock,
                         test_data: _CertificateSigningTestData):
        status = "Accepted" if test_data.csr_accepted else "Rejected"
        csms_mock.on_sign_certificate.side_effect = lambda csr: call_result.SignCertificatePayload(status=status)

    def _get_expected_csr_data(self, certificate_type: str, ocpp_config_reader: OCPPConfigReader):
        if certificate_type == "ChargingStationCertificate":

            return {
                'certificate_type': 'CSMS',
                'common': ocpp_config_reader.get_variable("InternalCtrlr", "ChargeBoxSerialNumber"),
                'country': ocpp_config_reader.get_variable("ISO15118Ctrlr", "ISO15118CtrlrCountryName"),
                'organization': ocpp_config_reader.get_variable("SecurityCtrlr", "OrganizationName"),
                'use_tpm': False}
        else:
            return {
                ...
                'common': ocpp_config_reader.get_variable("InternalCtrlr", "ChargeBoxSerialNumber"),
                'country': ocpp_config_reader.get_variable("ISO15118Ctrlr", "ISO15118CtrlrCountryName"),
                'organization': ocpp_config_reader.get_variable("ISO15118Ctrlr", "ISO15118CtrlrOrganizationName"),
                'use_tpm': False}
...
@pytest.mark.parametrize("skip_implementation",
                             [{"ProbeModuleSecurity":["generate_certificate_signing_request", "update_leaf_certificate"]}])
...
class TestSecurityOCPPIntegration(_BaseTest):
    ...
    def _setup_security_module_mocks(self,
                                     probe_module: ProbeModule,
                                     test_data: _CertificateSigningTestData) -> _SecurityModuleMocks:
        security_generate_certificate_signing_request_mock = Mock()
        security_generate_certificate_signing_request_mock.side_effect = lambda arg: test_data.csr
        probe_module.implement_command("ProbeModuleSecurity",
                                       "generate_certificate_signing_request",
                                       security_generate_certificate_signing_request_mock)

        security_update_leaf_certificate_mock = Mock()
        security_update_leaf_certificate_mock.side_effect = lambda \
                arg: "Accepted" if test_data.signed_certificate_valid else "InvalidCertificateChain"
        probe_module.implement_command("ProbeModuleSecurity",
    ...
    @pytest.mark.parametrize("certificate_type", ["ChargingStationCertificate", "V2GCertificate"])
    async def test_A02_update_charging_station_certificate_by_csms_request(self, certificate_type,
                                                                           probe_module,
                                                                           ocpp_config_reader,
        ...
        trigger_result: call_result.TriggerMessagePayload = await chargepoint_with_pm.trigger_message_req(
        ...
        await self._wait_for_mock_called(security_mocks.generate_certificate_signing_request)
        ...
        assert csms_mock.on_sign_certificate.mock_calls == [mock_call(csr=test_data.csr)]

        # Act II: CSMS sends signed result to ChargePoint
        signed_result: call_result.CertificateSignedPayload = await chargepoint_with_pm.certificate_signed_req(
        ...
        assert security_mocks.update_leaf_certificate.mock_calls == [
            mock_call({'certificate_chain': test_data.signed_certificate,
                       'certificate_type': 'CSMS' if certificate_type == "ChargingStationCertificate" else "V2G"})]

    async def test_A02_update_charging_station_certificate_by_csms_request_retry(self, probe_module,
                                                                                 ocpp_config_reader,
                                                                                 central_system: CentralSystem):
        ...
        repeat_times = ocpp_config_reader.get_variable("SecurityCtrlr", "CertSigningRepeatTimes")
        ...
        assert csms_mock.on_sign_certificate.mock_calls == repeat_times * [mock_call(csr=test_data.csr)]
        security_mocks.update_leaf_certificate.assert_not_called()

    async def test_A04_rejected_security_event_notification(self, probe_module,
                                                            ocpp_config_reader,
                                                            central_system: CentralSystem):
        ...
        await self._wait_for_mock_called(csms_mock.on_security_event_notification,
                                         call=mock_call(timestamp=ANY, tech_info='InvalidCertificateChain', type='InvalidChargingStationCertificate'))

        # test A02.FR.20 - no repeated request
        await asyncio.sleep(0.3)  # wait the minimum time between two retries and a little more
...
class TestSecurityOCPPE2E(_BaseTest):
    ...
    def _parse_certificate_request(cls, csr: str) -> _ParsedCSR:
        ...
        if request.subject.get_attributes_for_oid(x509.NameOID.EMAIL_ADDRESS):
            email_address = request.subject.get_attributes_for_oid(x509.NameOID.EMAIL_ADDRESS)[0].value
        return cls._ParsedCSR(csr,
                              request.subject.get_attributes_for_oid(x509.NameOID.COMMON_NAME)[0].value,
                              request.subject.get_attributes_for_oid(x509.NameOID.ORGANIZATION_NAME)[0].value,
    ...
    async def test_A02_update_charging_station_certificate_by_csms_request(self, certificate_type,
        ...
        # # Act CSMS triggers SignChargingStationCertificate
        trigger_result: call_result.TriggerMessagePayload = await charge_point.trigger_message_req(
        ...
        await self._wait_for_mock_called(csms_mock.on_sign_certificate)
        assert csms_mock.on_sign_certificate.mock_calls == [mock_call(csr=ANY,certificate_type=certificate_type)]
        received_csr_data = self._parse_certificate_request(csms_mock.on_sign_certificate.mock_calls[0].kwargs["csr"])
        expected_csr_data = self._get_expected_csr_data(certificate_type, ocpp_config_reader)
        ...
        if certificate_type == "ChargingStationCertificate":
            signed_certificate = CertificateHelper.sign_certificate_request(
                ...
                issuer_certificate_path=Path(__file__).parent / "../everest-aux/certs/ca/csms/CSMS_ROOT_CA.pem",
                issuer_private_key_path=Path(__file__).parent / "../everest-aux/certs/ca/csms/CSMS_ROOT_CA.key",
            ...
            signed_certificate = CertificateHelper.sign_certificate_request(
                received_csr_data.csr,
                issuer_certificate_path=Path(__file__).parent / "../everest-aux/certs/ca/cso/CPO_SUB_CA2.pem",
                issuer_private_key_path=Path(__file__).parent / "../everest-aux/certs/client/csms/CPO_SUB_CA2.key",
                issuer_private_key_passphrase="123456"
            )
            signed_certificate += (Path(
                __file__).parent / "../everest-aux/certs/ca/cso/CPO_SUB_CA2.pem").read_text() + "\n"
            signed_certificate += (Path(
                __file__).parent / "../everest-aux/certs/ca/cso/CPO_SUB_CA1.pem").read_text() + "\n"

        # Act II: CSMS sends signed result to ChargePoint
        signed_result: call_result.CertificateSignedPayload = await charge_point.certificate_signed_req(
    ...
    async def test_A02_update_charging_station_certificate_by_csms_request_invalid_as_expired(self,
                                                                                              ocpp_config_reader,
                                                                                              central_system: CentralSystem,
                                                                                              charge_point: ChargePoint201):
        ...
        await self._wait_for_mock_called(csms_mock.on_security_event_notification,
                                         call=mock_call(timestamp=ANY, tech_info='Expired', type='InvalidChargingStationCertificate'))

    async def test_A02_update_charging_station_certificate_by_csms_request_invalid_due_to_wrong_ca(self,
                                                                                                   ocpp_config_reader,
                                                                                                   central_system: CentralSystem,
                                                                                                   charge_point: ChargePoint201):
        csms_mock = central_system.mock
        ...
        signed_certificate = CertificateHelper.sign_certificate_request(
            received_csr_data.csr,
            issuer_certificate_path=Path(__file__).parent / "../everest-aux/certs/ca/mo/MO_ROOT_CA.pem",
            issuer_private_key_path=Path(__file__).parent / "../everest-aux/certs/client/mo/MO_ROOT_CA.key",
            issuer_private_key_passphrase="123456"
        )
    ...
    def assert_websocket_client_sslproto_certificate_equals_certificate(self, websocket_client_cert: dict[str, Any],
                                                                        certificate: str):
        x509_cert = crypto.load_certificate(crypto.FILETYPE_PEM, certificate.encode("utf-8"))

        def _compare_websocket_and_cert_components(websocket_components, cert_components):
            websocket_cert_subject_dict = {k: v for ((k, v),) in websocket_components}
            cert_subject_dict = {k: v for (k, v) in cert_components}
            for websocket_key, cert_key in [("countryName", b"C"), ("commonName", b"CN"), ("organizationName", b"O"),
                                            ("domainComponent", b"DC")]:
                assert websocket_cert_subject_dict.get(websocket_key, "") == cert_subject_dict.get(cert_key,
                                                                                                   b"").decode(
                    "utf-8")

        _compare_websocket_and_cert_components(websocket_client_cert["subject"],
                                               x509_cert.get_subject().get_components())
        _compare_websocket_and_cert_components(websocket_client_cert["issuer"], x509_cert.get_issuer().get_components())
        assert int(websocket_client_cert["serialNumber"], 16) == x509_cert.get_serial_number()
        assert datetime.strptime(websocket_client_cert["notBefore"], "%b %d %H:%M:%S %Y GMT") == datetime.strptime(
            x509_cert.get_notBefore().decode("utf-8"), "%Y%m%d%H%M%SZ")

    @pytest.mark.csms_tls(verify_client_certificate=True)
    @pytest.mark.ocpp_config_adaptions(_OCPP201NetworkConnectionProfileAdjustment(None, None, 3))
    async def test_A02_use_newest_certificate_after_installation(self,
                                                                 central_system: CentralSystem,
                                                                 charge_point: ChargePoint201):
        ...
        expected_original_certificate = (
                Path(__file__).parent / "../everest-aux/certs/client/csms/CSMS_RSA.pem").read_text()
        self.assert_websocket_client_sslproto_certificate_equals_certificate(original_certificate,
                                                                             expected_original_certificate)
        ...
        async def wait_for_reconnect():
            while len(central_system.ws_server.websockets) < 1 or central_system.ws_server.websockets == {
                old_connection}:
        ...
        self.assert_websocket_client_sslproto_certificate_equals_certificate(new_certificate, signed_certificate)

    @pytest.mark.ocpp_config_adaptions(_OCPP201NetworkConnectionProfileAdjustment(None, None, 3))
    @pytest.mark.csms_tls(verify_client_certificate=True)
    async def test_A02_use_newest_certificate_according_to_validity(self,
        ...
        cert_directory = Path(
            everest_core.everest_config["active_modules"]["evse_security"]["config_module"]["csms_leaf_cert_directory"])
        key_directory = Path(
            everest_core.everest_config["active_modules"]["evse_security"]["config_module"]["csms_leaf_key_directory"])

        ca_certificate = Path(__file__).parent / "../everest-aux/certs/ca/csms/CSMS_ROOT_CA.pem"
        ...
        for cert_index, (cert_name, relative_valid_time) in enumerate(
            ...
            cert_req, cert_key = CertificateHelper.generate_certificate_request(cert_name)
            cert = CertificateHelper.sign_certificate_request(cert_req,
        ...
        new_connection = next(iter(central_system.ws_server.websockets))
        new_certificate = new_connection.transport.get_extra_info("peercert")
        self.assert_websocket_client_sslproto_certificate_equals_certificate(new_certificate,
                                                                             certificates["Cert2-newest"])
    ...
    (OCPP201ConfigVariableIdentifier("InternalCtrlr", "V2GCertificateExpireCheckInitialDelaySeconds", "Actual"), 0)]))
    @pytest.mark.ocpp_config_adaptions(GenericOCPP201ConfigAdjustment([
    (OCPP201ConfigVariableIdentifier("InternalCtrlr", "ClientCertificateExpireCheckInitialDelaySeconds", "Actual"), 0)]))
    @pytest.mark.ocpp_config_adaptions(_OCPP201NetworkConnectionProfileAdjustment(None, None, 3))
    async def test_A03_install_new_if_expired(self,
                                              certificate_type,
                                              everest_core: EverestCore,
                                              test_controller: TestController,
                                              central_system: CentralSystem,
        ...
        if certificate_type == "ChargingStation":
            cert_directory = Path(
                everest_core.everest_config["active_modules"]["evse_security"]["config_module"][
            ...
            ca_certificate = Path(__file__).parent / "../everest-aux/certs/ca/cso/CPO_SUB_CA2.pem"
            ca_key = Path(__file__).parent / "../everest-aux/certs/client/csms/CPO_SUB_CA2.key"
            ca_passphrase = "123456"
        ...
        cert_req, cert_key = CertificateHelper.generate_certificate_request("almost expired")
        cert = CertificateHelper.sign_certificate_request(cert_req,
                                                          issuer_certificate_path=ca_certificate,
                                                          issuer_private_key_path=ca_key,
                                                          issuer_private_key_passphrase=ca_passphrase,
    ...
    async def test_A01(self, central_system: CentralSystem, charge_point_v201: ChargePoint201):
        # Disable AuthCacheCtrlr
        r: call_result.SetVariablesPayload = await charge_point_v201.set_config_variables_req("SecurityCtrlr",
                                                                                              "BasicAuthPassword",
                                                                                              "BEEFDEADBEEFDEAD")
