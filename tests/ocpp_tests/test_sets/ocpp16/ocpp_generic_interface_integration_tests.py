...
async def _env(everest_core, test_controller, central_system: CentralSystem, skip_implementation, overwrite_implementation):
    ...
    def _add_pm_command_mock(implementation_id, command, value, skip_implementation):
        ...
        if not skip:
            ...
            probe_module_command_mocks.setdefault(implementation_id, {})[command] = Mock()
            probe_module_command_mocks[implementation_id][command].return_value = value
            probe_module.implement_command(implementation_id=implementation_id, command_name=command,
    ...
    for idx, evse_manager in enumerate(["evse_manager", "evse_manager_b"]):
        _add_pm_command_mock(evse_manager, "get_evse", {"id": idx + 1, "connectors": [{"id": 1}]}, skip_implementation)
        _add_pm_command_mock(evse_manager, "enable_disable", True, skip_implementation)
        _add_pm_command_mock(evse_manager, "authorize_response", None, skip_implementation)
        _add_pm_command_mock(evse_manager, "withdraw_authorization", None, skip_implementation)
        _add_pm_command_mock(evse_manager, "reserve", False, skip_implementation)
        _add_pm_command_mock(evse_manager, "cancel_reservation", None, skip_implementation)
        ...
        _add_pm_command_mock(evse_manager, "stop_transaction", True, skip_implementation)
        _add_pm_command_mock(evse_manager, "force_unlock", True, skip_implementation)
        _add_pm_command_mock(evse_manager, "set_get_certificate_response", None, skip_implementation)
        _add_pm_command_mock(evse_manager, "external_ready_to_start_charging", True, skip_implementation)
    _add_pm_command_mock("security", "get_leaf_expiry_days_count", 42, skip_implementation)
    _add_pm_command_mock("security", "get_v2g_ocsp_request_data", {"ocsp_request_data_list": []}, skip_implementation)
    _add_pm_command_mock("security", "get_mo_ocsp_request_data", {"ocsp_request_data_list": []}, skip_implementation)
    _add_pm_command_mock("security", "install_ca_certificate", "Accepted", skip_implementation)
    _add_pm_command_mock("security", "delete_certificate", "Accepted", skip_implementation)
    _add_pm_command_mock("security", "update_leaf_certificate", "Accepted", skip_implementation)
    _add_pm_command_mock("security", "verify_certificate", "Valid", skip_implementation)
    _add_pm_command_mock("security", "get_installed_certificates", {"status": "Accepted", "certificate_hash_data_chain": []}, skip_implementation)
    _add_pm_command_mock("security", "update_ocsp_cache", None, skip_implementation)
    _add_pm_command_mock("security", "is_ca_certificate_installed", False, skip_implementation)
    _add_pm_command_mock("security", "generate_certificate_signing_request", {"status": "Accepted"}, skip_implementation)
    _add_pm_command_mock("security", "get_leaf_certificate_info", {"status": "Accepted"}, skip_implementation)
    ...
    _add_pm_command_mock("security", "get_all_valid_certificates_info", {"status": "NotFound", "info": []}, skip_implementation)
    ...
    _add_pm_command_mock("reservation", "cancel_reservation", "Accepted", skip_implementation)
    ...
    _add_pm_command_mock("system", "allow_firmware_installation", None, skip_implementation)
    ...
    yield _OCPP16GenericInterfaceIntegrationEnvironment(csms_mock, central_system, everest_core, probe_module,
                                                        probe_module_command_mocks,
...
class TestOCPP16GenericInterfaceIntegration:
    ...
    async def test_command_security_event(self, _env):
        res = await _env.probe_module.call_command("ocpp", "security_event",
                                                   {"event": {"type": "SecurityLogWasCleared",
                                                    "info": "integration_test_security_info",
                                                    "critical": True, "timestamp": "2024-01-01T12:00:00"}})
        assert res is None
        await wait_for_mock_called(_env.csms_mock.on_security_event_notification,
                                   mock_call(tech_info='integration_test_security_info', timestamp=ANY,
                                             type='SecurityLogWasCleared'))
        assert len(_env.csms_mock.on_security_event_notification.mock_calls) == 2 # we expect 2 because of the StartupOfTheDevice

    @pytest.mark.ocpp_config_adaptions(
        GenericOCPP16ConfigAdjustment([("Custom", "ExampleConfigurationKey", "test_value")]))
    async def test_command_get_variables(self, _env):
        res = await _env.probe_module.call_command("ocpp", "get_variables",
                                                   {"requests": [{'component_variable': {
        ...
        assert res == [{'attribute_type': 'Actual',
                        'component_variable': {'component': {'name': ''}, 'variable': {'name': 'ChargePointId'}},
                        'status': 'Accepted', 'value': 'cp001'},
                       {'component_variable': {'component': {'name': ''}, 'variable': {'name': 'UNKNOWN'}},
                        'status': 'UnknownVariable'},
                       {'attribute_type': 'Actual',
                        'component_variable': {'component': {'name': ''},
                                               'variable': {
                                                   'name': 'ExampleConfigurationKey'}},
                        'status': 'Accepted', 'value': 'test_value'}
                       ]

    @pytest.mark.ocpp_config_adaptions(
        GenericOCPP16ConfigAdjustment([("Custom", "ExampleConfigurationKey", "test_value")]))
    async def test_command_set_variables(self, _env):
        res = await _env.probe_module.call_command("ocpp", "set_variables",
                                                   {"requests": [{'component_variable': {
                                                       'component': {'name': 'IGNORED'},
        ...
        assert res == [
            {'component_variable': {'component': {'name': 'IGNORED'}, 'variable': {'name': 'RetryBackoffRandomRange'}},
             'status': 'Rejected'},
            {'component_variable': {'component': {'name': ''}, 'variable': {'name': 'UNKNOWN'}}, 'status': 'Rejected'},
            {'component_variable': {'component': {'name': ''},
                                    'variable': {'instance': 'TO_BE_IGNORED', 'name': 'ExampleConfigurationKey'}},
             'status': 'Accepted'}]

        # Verify value changed
        check = await _env.probe_module.call_command("ocpp", "get_variables",
                                                     {"requests": [{'component_variable': {
                                                         'component': {'name': ''},
                                                         'variable': {'name': 'ExampleConfigurationKey'}}
                                                     }
                                                     ]})
        assert check == [{'attribute_type': 'Actual', 'component_variable': {'component': {'name': ''}, 'variable': {
            'name': 'ExampleConfigurationKey'}}, 'status': 'Accepted', 'value': 'unittest changed value'}]
    ...
    async def test_command_monitor_variables(self, _env):
        ...
        _env.probe_module.subscribe_variable("ocpp", "event_data", event_data_subscription_mock)
        ...
        # enable monitoring
        res = await _env.probe_module.call_command("ocpp", "monitor_variables",
                                                   {"component_variables": [{
                                                       'component': {'name': 'IGNORED'},
        ...
        await wait_for_mock_called(event_data_subscription_mock,
                                   mock_call({'actual_value': '42', 'component_variable': {
                                       'component': {'name': 'IGNORED'}, 'variable': {'name': 'HeartbeatInterval'}},
                                              'event_id': ANY,
    ...
    async def test_subscribe_charging_schedules(self, _env):
        ...
        _env.probe_module.subscribe_variable("ocpp", "charging_schedules", subscription_mock)
        ...
        await _env.charge_point.set_charging_profile_req(
            SetChargingProfilePayload(connector_id=0, cs_charging_profiles={
                ...
                "chargingSchedule": {"chargingRateUnit": "A", "chargingSchedulePeriod": [
                    {"limit": 32.0, "startPeriod": 0}]}}))
        await wait_for_mock_called(subscription_mock,
                                   mock_call(
                                       {'schedules': [
                                           ...
                                           ], 'evse': 0, 'duration': ANY, 'start_schedule': ANY},
                                           {'charging_rate_unit': 'A', 'charging_schedule_period': [
                                                {
                                                    "limit": 32,
                                                    "number_phases": 3,
                                           ...
                                           ], 'evse': 1, 'duration': ANY, 'start_schedule': ANY},
                                           {'charging_rate_unit': 'A', 'charging_schedule_period': [
                                                {
                                                    "limit": 32,
                                                    "number_phases": 3,
                                           ...
                                           ], 'evse': 2, 'duration': ANY, 'start_schedule': ANY}]}))
    ...
    @pytest.mark.parametrize("overwrite_implementation",
                             [{"security":{"update_leaf_certificate": "InvalidSignature"}}])
    async def test_subscribe_security_event(self, _env):
        subscription_mock = Mock()
        _env.probe_module.subscribe_variable("ocpp", "security_event", subscription_mock)
        ...
        await wait_for_mock_called(subscription_mock,
                                   mock_call({'info': 'InvalidSignature', 'type': 'InvalidChargePointCertificate'}))
    ...
    async def test_change_availability_request_connector(self, _env):
        ...
        res = await _env.probe_module.call_command("ocpp", "change_availability",
                                                   {"request": {
        ...
        await wait_for_mock_called(_env.probe_module_command_mocks["evse_manager"]["enable_disable"],
                                   call=call({'cmd_source': {'enable_priority': 5000, 'enable_source': 'CSMS', 'enable_state': 'Disable'}, 'connector_id': 0}))  # as currently implemented in disable_evse callback in OCPP module
        ...
        await wait_for_mock_called(_env.probe_module_command_mocks["evse_manager_b"]["enable_disable"],
                                   call=call({'cmd_source': {'enable_priority': 5000, 'enable_source': 'CSMS', 'enable_state': 'Disable'}, 'connector_id': 0}))  # as currently implemented in disable_evse callback in OCPP module
    ...
    async def test_change_availability_request_evse(self, _env):
        ...
        assert res == {'status': 'Accepted'}
        await wait_for_mock_called(_env.probe_module_command_mocks["evse_manager"]["enable_disable"],
                                   call=call({'cmd_source': {'enable_priority': 5000, 'enable_source': 'CSMS', 'enable_state': 'Disable'}, 'connector_id': 0}))
        assert len(_env.probe_module_command_mocks["evse_manager"]["enable_disable"].mock_calls) == 1
    ...
    async def test_change_availability_request_failed(self, _env):
        ...
        assert res == {'status': 'Rejected',
                       'status_info': {'additional_info': ANY,  # No connector id specified;
        ...
        assert res == {'status': 'Rejected',
                       'status_info': {'additional_info': ANY,  # Invalid connector id specified
                                       'reason_code': 'InvalidInput'}}
