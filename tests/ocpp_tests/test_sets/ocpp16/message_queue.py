...
@pytest.mark.asyncio
async def test_call_error_to_transaction_message(test_config: OcppTestConfiguration, charge_point_v16: ChargePoint16, test_utility: TestUtility, test_controller: TestController):

    setattr(charge_point_v16, 'on_start_transaction', None)
    charge_point_v16.route_map = create_route_map(charge_point_v16)

    await charge_point_v16.change_configuration_req(key="TransactionMessageAttempts", value="3")
    ...
    assert await wait_for_and_validate(test_utility, charge_point_v16, "StopTransaction", {"reason": "EVDisconnected"})
...
@pytest.mark.asyncio
async def test_security_event_delivery_after_reconnect(everest_core, test_controller, central_system: CentralSystem):
    ...
    csms_mock.on_security_event_notification.reset_mock(); # Since on boot we expect a count of security events
    await probe_module.call_command("ocpp", "security_event", {"type": "SecurityLogWasCleared", "info": "test_info"})
    ...
    await wait_for_mock_called(csms_mock.on_security_event_notification, mock_call(tech_info='test_info', timestamp=ANY, type='SecurityLogWasCleared'), 10)
