...
def validate_authorize_req(authorize_req: call201.AuthorizePayload, contains_contract, contains_ocsp):
    return (authorize_req.certificate != None) == contains_contract and \
            (authorize_req.iso15118_certificate_hash_data != None) == contains_ocsp

@pytest.mark.skip("Plug and charge tests do currently interfere when they are run in parallel with other tests")
class TestPlugAndCharge:

    @pytest.mark.asyncio
    @pytest.mark.source_certs_dir(Path(__file__).parent.parent / "everest-aux/certs")
    async def test_contract_installation_and_authorization_01(self, request, central_system_v16: CentralSystem, charge_point_v16: ChargePoint16,
                                                            test_controller: TestController, test_config, test_utility: TestUtility):
        """
        ...
        r: call.DataTransferPayload = call.DataTransferPayload(**await wait_for_and_validate(test_utility, charge_point_v16,
                                        "DataTransfer", {
                                            "messageId": "Authorize",
                                            "vendorId": "org.openchargealliance.iso15118pnc",
                                        }))

        authorize_req: call201.AuthorizePayload = call201.AuthorizePayload(**camel_to_snake_case(json.loads(r.data)))
    ...
    @pytest.mark.asyncio
    async def test_contract_installation_and_authorization_02(self, request, central_system_v16: CentralSystem, charge_point_v16: ChargePoint16,
    ...
    @pytest.mark.asyncio
    @pytest.mark.source_certs_dir(Path(__file__).parent.parent / "everest-aux/certs")
    async def test_contract_installation_and_authorization_03(self, request, central_system_v16: CentralSystem, charge_point_v16: ChargePoint16,
                                                            test_controller: TestController, test_config, test_utility: TestUtility):
        """
        Test for contract installation on the vehicle and succeeding authorization and charging process
        """

        await charge_point_v16.change_configuration_req(key="CentralContractValidationAllowed", value="true")
    ...
    @pytest.mark.asyncio
    async def test_eim_01(self, test_config, charge_point_v16: ChargePoint16, test_controller: TestController, test_utility: TestUtility):
        """
    ...
    @pytest.mark.asyncio
    @pytest.mark.everest_core_config(get_everest_config_path_str('everest-config-sil-iso no-tls.yaml'))
    async def test_eim_02(self, charge_point_v16: ChargePoint16, test_controller: TestController, test_utility: TestUtility):
        """
    ...
    @pytest.mark.asyncio
    async def test_pnc_reject(self, test_config, central_system_v16: CentralSystem, charge_point_v16: ChargePoint16, test_controller: TestController, test_utility: TestUtility):
        """
    ...
    @ pytest.mark.asyncio
    async def test_pnc_get_installed_certificate_ids(self, charge_point_v16: ChargePoint16):
        """
    ...
    @ pytest.mark.asyncio
    async def test_pnc_install_certificate(self, request, charge_point_v16: ChargePoint16):

        v2g_root_ca_path = Path(request.config.getoption(
            "--everest-prefix")) / "etc/everest/certs/ca/v2g/V2G_ROOT_CA.pem"
        try:
            os.remove(v2g_root_ca_path)
        except Exception:
            pass

        with open(Path(os.path.dirname(__file__)).parent / "everest-aux/certs/ca/v2g/V2G_ROOT_CA.pem", "r") as f:
    ...
    @ pytest.mark.asyncio
    async def test_pnc_sign_certificate_and_trigger_message(self, test_utility, charge_point_v16: ChargePoint16):
        trigger_message_req = {
        ...
        csr = await wait_for_and_validate(test_utility, charge_point_v16,
                                        "DataTransfer", call.DataTransferPayload(
                                            data=json.dumps(remove_nones(
                                                snake_to_camel_case(asdict(call201.SignCertificatePayload(
                                                    csr="",
        ...
        data_transfer_response = await charge_point_v16.data_transfer_req(vendor_id="org.openchargealliance.iso15118pnc", message_id="CertificateSigned", data=json.dumps(certificate_signed_req))
