...
macro(_trailbook_ev_generate_rst_from_manifest_generate_command)
    ...
    get_filename_component(MANIFEST_DIRECTORY "${args_MANIFEST_FILE}" DIRECTORY)
    get_filename_component(INDEX_PROPERTY_DIRECTORY "${MANIFEST_DIRECTORY}" DIRECTORY)
    ...
    set(INDEX_TOC_GLOB_EXPRESSION "    *")

    while(NOT CURRENT_RELATIVE_FOLDER STREQUAL "")
        ...
        if (NOT ${is_create_index_cmd_added})
            message(WARNING "ADDING DEFINITION OF add_custom_command FOR THE INDEX FILE OF ${CURRENT_DEST_DIRECTORY} FOR ${INDEX_FILE}")
            add_custom_command(
                OUTPUT
                    "${INDEX_FILE}"
                COMMAND ${CMAKE_COMMAND} -E make_directory "${CURRENT_DEST_DIRECTORY}"
                COMMAND ${CMAKE_COMMAND} -E touch "${INDEX_FILE}"
                COMMAND ${CMAKE_COMMAND} -E echo "=========================================================" > "${INDEX_FILE}"
                COMMAND ${CMAKE_COMMAND} -E echo "${CURRENT_FOLDER_NAME}" >> ${INDEX_FILE}
                COMMAND ${CMAKE_COMMAND} -E echo "=========================================================" >> "${INDEX_FILE}"
                COMMAND ${CMAKE_COMMAND} -E echo "" >> "${INDEX_FILE}"
                COMMAND ${CMAKE_COMMAND} -E echo ".. toctree::" >> "${INDEX_FILE}"
                COMMAND ${CMAKE_COMMAND} -E echo "    :maxdepth: 1" >> "${INDEX_FILE}"
                COMMAND ${CMAKE_COMMAND} -E echo "    :glob:" >> "${INDEX_FILE}"
                COMMAND ${CMAKE_COMMAND} -E echo "" >> "${INDEX_FILE}"
                COMMAND ${CMAKE_COMMAND} -E echo "${INDEX_TOC_GLOB_EXPRESSION}" >> "${INDEX_FILE}"
                COMMAND ${CMAKE_COMMAND} -E echo "" >> "${INDEX_FILE}"
                DEPENDS
                    ${GENERATED_FILE}
                    trailbook_${args_TRAILBOOK_NAME}_stage_prepare_sphinx_source_after
            ...
            message(WARNING "SKIPPING DEFINITION OF add_custom_command FOR THE INDEX FILE OF ${CURRENT_FOLDER_NAME}")
        endif()
