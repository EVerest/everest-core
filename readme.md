...
This branch was created using `git subtree` to create a single branch with
the EVerest source dependencies.

```
git subtree add --squash -P <subdirectory> git@github.com:EVerest/<repository>.git <version>
```
...
for i in *
do
  git subtree pull --squash -P $i git@github.com:EVerest/$i.git main
done
```
